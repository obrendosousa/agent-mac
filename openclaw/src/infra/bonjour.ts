...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CHAPPIE_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Chappie";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Chappie\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.CHAPPIE_MDNS_HOSTNAME?.trim() ||
    process.env.CHAPPIEDBOT_MDNS_HOSTNAME?.trim() ||
    "chappie";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
