...
const TEMP_DIR_PREFIX = "chappie-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("chappie-install-source-utils-");
  const archivePath = path.join(cwd, "chappie-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("chappie-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-chappie-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "chappie-plugin@1.2.3",
          name: "chappie-plugin",
          version: "1.2.3",
          filename: "chappie-plugin-1.2.3.tgz",
    ...
    const result = await runPack("chappie-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "chappie-plugin",
        version: "1.2.3",
        resolvedSpec: "chappie-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "chappie-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "chappie-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nchappie-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"chappie-plugin@1.2.3","name":"chappie-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "chappie-plugin",
        version: "1.2.3",
        resolvedSpec: "chappie-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@chappie/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@chappie/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@chappie/whatsapp");
      expect(result.error).toContain("docs.chappie.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("chappie-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "chappie-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@chappie/plugin-demo@2.0.0",
            filename: "chappie-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@chappie/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "chappie-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@chappie/plugin-demo@2.0.0",
      },
    });
  });
