...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevChappieHome = process.env.CHAPPIE_HOME;
    const prevHome = process.env.HOME;
    process.env.CHAPPIE_HOME = "/srv/chappie-home";
    process.env.HOME = "/home/other";
    const chappieHome = path.join(path.resolve("/srv/chappie-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", chappieHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevChappieHome === undefined) {
        delete process.env.CHAPPIE_HOME;
      } else {
        process.env.CHAPPIE_HOME = prevChappieHome;
      }
