...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.CHAPPIE_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.CHAPPIE_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.CHAPPIE_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.CHAPPIE_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.CHAPPIE_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set CHAPPIE_APNS_PRIVATE_KEY_P8 or CHAPPIE_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading CHAPPIE_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
