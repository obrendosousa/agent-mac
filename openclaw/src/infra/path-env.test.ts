...
let ensureChappieCliOnPath: typeof import("./path-env.js").ensureChappieCliOnPath;

describe("ensureChappieCliOnPath", () => {
  const envKeys = [
    "PATH",
    "CHAPPIE_PATH_BOOTSTRAPPED",
    "CHAPPIE_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureChappieCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/chappie-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "chappie");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureChappieCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling chappie exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.CHAPPIE_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.CHAPPIE_PATH_BOOTSTRAPPED = "1";
    ensureChappieCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "chappie");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.CHAPPIE_PATH_BOOTSTRAPPED;
      delete process.env.CHAPPIE_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.CHAPPIE_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/chappie-path/case-linuxbrew");
