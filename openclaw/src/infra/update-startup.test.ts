...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CHAPPIE_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.CHAPPIE_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveChappiePackageRoot } = await import("./chappie-root.js"));
    ...
    vi.mocked(resolveChappiePackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveChappiePackageRoot).mockResolvedValue("/opt/chappie");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/chappie/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
