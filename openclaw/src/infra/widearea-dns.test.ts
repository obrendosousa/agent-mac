...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "chappie.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Chappie)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "chappie.internal", expected: "chappie.internal." },
    { value: "chappie.internal.", expected: "chappie.internal." },
    { value: "  chappie.internal  ", expected: "chappie.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { CHAPPIE_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { CHAPPIE_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("chappie.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "chappie.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/chappie",
    });

    expect(txt).toContain(`$ORIGIN chappie.internal.`);
    ...
    expect(txt).toContain(`_chappie-gw._tcp IN PTR studio-london._chappie-gw._tcp`);
    expect(txt).toContain(`studio-london._chappie-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Chappie)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/chappie`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN chappie.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._chappie-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Chappie)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("chappie.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("chappie.internal."),
