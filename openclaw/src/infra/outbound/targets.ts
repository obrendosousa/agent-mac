...
export function resolveSessionDeliveryTarget(params: {
   ...
   *
   * @see https://github.com/chappie/chappie/issues/24152
   */
...
export function resolveOutboundTarget(params: {
  ...
  if (params.channel === INTERNAL_MESSAGE_CHANNEL) {
    return {
      ok: false,
      error: new Error(
        `Delivering to WebChat is not supported via \`${formatCliCommand("chappie agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
      ),
    };
  }
