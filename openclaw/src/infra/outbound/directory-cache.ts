...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ChappieConfig | null = null;
  ...
  get(key: string, cfg: ChappieConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ChappieConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ChappieConfig): void {
