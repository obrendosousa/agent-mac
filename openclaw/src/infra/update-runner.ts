...
const DEFAULT_PACKAGE_NAME = "chappie";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["chappie", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "chappie.mjs");
    ...
    const doctorStep = await runStep(
      step("chappie doctor", doctorArgv, gitRoot, { CHAPPIE_UPDATE_IN_PROGRESS: "1" }),
    );
