...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CHAPPIE_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*chappie-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "chappie";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "chappie");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "chappie-gw");
  ...
  records.push(`_chappie-gw._tcp IN PTR ${instanceLabel}._chappie-gw._tcp`);
  records.push(`${instanceLabel}._chappie-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._chappie-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; chappie-content-hash: ${contentHash}\n${contentBody}`;
}
