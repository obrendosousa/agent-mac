...
describe("shell env fallback", () => {
  ...
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ CHAPPIE_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ CHAPPIE_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("uses the same truthy env parsing for deferred fallback", () => {
    ...
    expect(shouldDeferShellEnvFallback({ CHAPPIE_DEFER_SHELL_ENV_FALLBACK: "false" })).toBe(false);
    expect(shouldDeferShellEnvFallback({ CHAPPIE_DEFER_SHELL_ENV_FALLBACK: "yes" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ CHAPPIE_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        CHAPPIE_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
  });
