...
import { POSIX_CHAPPIE_TMP_DIR, resolvePreferredChappieTmpDir } from "./tmp-chappie-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredChappieTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `chappie-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredChappieTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_CHAPPIE_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredChappieTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredChappieTmpDir", () => {
  it("prefers /tmp/chappie when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_CHAPPIE_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/chappie when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_CHAPPIE_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/chappie when /tmp/chappie is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/chappie when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/chappie exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_CHAPPIE_TMP_DIR) {
  ...
  it("falls back when /tmp/chappie is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/chappie is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/chappie is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/chappie permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_CHAPPIE_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_CHAPPIE_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_CHAPPIE_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/chappie after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_CHAPPIE_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_CHAPPIE_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_CHAPPIE_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_CHAPPIE_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Chappie temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "chappie");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredChappieTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_CHAPPIE_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Chappie temp dir/);
  });
});
