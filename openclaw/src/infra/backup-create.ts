...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Chappie config file was found to back up."
        : "No local Chappie state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-backup-"));
