  ...
  describe("resolveApnsRelayConfigFromEnv", () => {
    ...
    it("lets env overrides win and clamps tiny timeout values", () => {
      const resolved = resolveApnsRelayConfigFromEnv(
        {
          CHAPPIE_APNS_RELAY_BASE_URL: " https://relay-override.example.com/base/ ",
          CHAPPIE_APNS_RELAY_TIMEOUT_MS: "999",
    ...
    it("allows loopback http URLs for alternate truthy env values", () => {
      const resolved = resolveApnsRelayConfigFromEnv({
        CHAPPIE_APNS_RELAY_BASE_URL: "http://[::1]:8787",
        CHAPPIE_APNS_RELAY_ALLOW_HTTP: "yes",
        CHAPPIE_APNS_RELAY_TIMEOUT_MS: "nope",
      } as NodeJS.ProcessEnv);
    ...
    it.each([
      {
        ...
        env: { CHAPPIE_APNS_RELAY_BASE_URL: "ftp://relay.example.com" },
        expected: "unsupported protocol",
      },
      {
        name: "http non-loopback host",
        env: {
          CHAPPIE_APNS_RELAY_BASE_URL: "http://relay.example.com",
          CHAPPIE_APNS_RELAY_ALLOW_HTTP: "true",
        },
        expected: "loopback hosts",
      },
      {
        name: "query string",
        env: { CHAPPIE_APNS_RELAY_BASE_URL: "https://relay.example.com/path?debug=1" },
        expected: "query and fragment are not allowed",
      },
      {
        name: "userinfo",
        env: { CHAPPIE_APNS_RELAY_BASE_URL: "https://user:pass@relay.example.com/path" },
