...
export function triggerChappieRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.CHAPPIE_SYSTEMD_UNIT,
      process.env.CHAPPIE_PROFILE,
    );
  ...
  const label =
    process.env.CHAPPIE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CHAPPIE_PROFILE);
