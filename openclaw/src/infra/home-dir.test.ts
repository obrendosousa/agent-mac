...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers CHAPPIE_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for CHAPPIE_HOME", () => {
    const result = resolveRequiredHomeDir(
      { CHAPPIE_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when CHAPPIE_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ CHAPPIE_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { CHAPPIE_HOME: "/srv/chappie-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/chappie-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/chappie-home " },
      expected: "/srv/chappie-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/chappie-home/docs"));
  });
