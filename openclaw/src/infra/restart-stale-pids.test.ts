...
function createChappieBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "chappie-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createChappieBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses chappie-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "chappie-gateway" },
          { pid: process.pid, cmd: "chappie-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'chappie'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple chappie pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "chappie-gateway" },
          { pid: pid2, cmd: "chappie-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-chappie pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks chappie processes — non-chappie listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty chappie stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty chappie stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + chappie pid in stdout — container-restricted lsof reports partial results
          return createChappieBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "chappie-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'chappie'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("chappie")
      // inside the mid-loop flush: a non-chappie cmd between two entries must not
      // be pushed, but the following chappie entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-chappie entry first, then chappie entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid chappie entry must still be found.
      const stdout = `p0\ncchappie-gateway\np${stalePid}\ncchappie-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero chappie pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-chappie stdout (line 145)", () => {
    it("treats status 1 + non-chappie stdout as port-free (not an chappie process)", () => {
      // status 1 + non-empty stdout where no chappie pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill chappie pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-chappie output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no chappie pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
