...
function candidateBinDirs(opts: EnsureChappiePathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `chappie` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "chappie");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CHAPPIE_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "chappie"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `chappie` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureChappieCliOnPath(opts: EnsureChappiePathOpts = {}) {
  if (isTruthyEnvValue(process.env.CHAPPIE_PATH_BOOTSTRAPPED)) {
    return;
  }
