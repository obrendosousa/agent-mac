...
async function runExtractedArchiveFailureCase(configureArchive: () => void) {
  vi.spyOn(installSource, "withTempDir").mockImplementation(
    async (_prefix, fn) => await fn("/tmp/chappie-install-flow"),
  );
  configureArchive();
...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "chappie-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("chappie-plugin-", expect.any(Function));
