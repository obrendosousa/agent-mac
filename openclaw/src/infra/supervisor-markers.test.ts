...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.chappie.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ CHAPPIE_WINDOWS_TASK_NAME: "Chappie Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.chappie.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
