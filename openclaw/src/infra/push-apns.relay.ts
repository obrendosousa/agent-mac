...
const GATEWAY_DEVICE_ID_HEADER = "x-chappie-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-chappie-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-chappie-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.CHAPPIE_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "CHAPPIE_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or CHAPPIE_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.CHAPPIE_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require CHAPPIE_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.CHAPPIE_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
