...
vi.mock("./chappie-root.js", () => ({
  resolveChappiePackageRoot: vi.fn(async () => null),
  resolveChappiePackageRootSync: vi.fn(() => null),
}));
...
let chappieRoot: typeof import("./chappie-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    chappieRoot = await import("./chappie-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/chappie");
    const wrapperArgv1 = abs("fixtures/bin/chappie");
  ...
  it("uses resolveChappiePackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/chappie");
    (
      chappieRoot.resolveChappiePackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/chappie"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "chappie" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "chappie.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-chappie");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/chappie-bundle");
    (
      chappieRoot.resolveChappiePackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/chappie-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
