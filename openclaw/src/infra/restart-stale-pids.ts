 ...
 * returning control to the caller (typically `triggerChappieRestart` →
...
/**
 * Parse chappie gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("chappie")) {
 ...
 * Returns only PIDs that belong to chappie gateway processes (not the current process).
 */
