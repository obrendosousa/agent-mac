...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-update-"));
  });
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "chappie.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "chappie.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "chappie", version: "1.0.0" };
  ...
  async function seedGlobalPackageRoot(pkgRoot: string, version = "1.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "chappie", version }),
      "utf-8",
    );
  }
  ...
  async function createGlobalPackageFixture(rootDir: string) {
    ...
    const pkgRoot = path.join(nodeModules, "chappie");
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g chappie@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g chappie@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "chappie.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "chappie", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "chappie", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g chappie@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g chappie@beta --no-fund --no-audit --loglevel=error",
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g chappie@latest --no-fund --no-audit --loglevel=error",
      gitRootMode: "missing",
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g chappie@latest --no-fund --no-audit --loglevel=error");
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "chappie");
    const staleDir = path.join(nodeModules, ".chappie-stale");
  ...
  it("uses CHAPPIE_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/chappie-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
    ...
    await withEnvAsync(
      { CHAPPIE_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/chappie-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g chappie@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g chappie@latest")).toBe(true);
    });
  });

  it("rejects git roots that are not a chappie checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-chappie-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when chappie.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "chappie.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("chappie doctor entry");
  });
