...
import { triggerChappieRestart } from "./restart.js";
 ...
 * - CHAPPIE_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.CHAPPIE_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerChappieRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
