...
import { resolveChappiePackageRoot, resolveChappiePackageRootSync } from "./chappie-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveChappiePackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "chappie" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/chappie -> .../dist/index.js)
