...
describe("installPackageDir", () => {
  ...
  it("keeps the existing install in place when staged validation fails", async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-install-package-dir-"));
    ...
    await expect(
      listMatchingDirs(installBaseDir, ".chappie-install-stage-"),
    ).resolves.toHaveLength(0);
    await expect(
      listMatchingDirs(installBaseDir, ".chappie-install-backups"),
    ).resolves.toHaveLength(0);
  });
  ...
  it("restores the original install if publish rename fails", async () => {
    ...
    const backupRoot = path.join(installBaseDir, ".chappie-install-backups");
  ...
  it("warns and leaves the backup in place when the install base changes before backup cleanup", async () => {
    ...
    const backupRoot = path.join(preservedInstallRoot, ".chappie-install-backups");
  ...
  it("installs peer dependencies for isolated plugin package installs", async () => {
    ...
    expect(vi.mocked(runCommandWithTimeout)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: expect.stringContaining(".chappie-install-stage-"),
      }),
    );
  });
});
