...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "chappie", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "chappie", version: "2.3.4" });
  ...
  it("ignores non-chappie package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers CHAPPIE_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CHAPPIE_VERSION: "9.9.9",
        CHAPPIE_SERVICE_VERSION: "2.2.2",
