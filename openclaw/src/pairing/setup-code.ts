...
function resolveGatewayTokenFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return env.CHAPPIE_GATEWAY_TOKEN?.trim() || env.CHAPPIEDBOT_GATEWAY_TOKEN?.trim() || undefined;
}

function resolveGatewayPasswordFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    env.CHAPPIE_GATEWAY_PASSWORD?.trim() || env.CHAPPIEDBOT_GATEWAY_PASSWORD?.trim() || undefined
  );
}
...
async function resolveGatewayTokenSecretRef(
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.CHAPPIE_GATEWAY_PASSWORD?.trim() || env.CHAPPIEDBOT_GATEWAY_PASSWORD?.trim(),
    );
