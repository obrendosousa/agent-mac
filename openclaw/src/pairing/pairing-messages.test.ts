...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect CHAPPIE_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:chappie|chappie) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
      );
