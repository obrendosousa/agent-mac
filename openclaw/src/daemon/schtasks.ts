...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.CHAPPIE_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.CHAPPIE_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.CHAPPIE_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.CHAPPIE_GATEWAY_PORT) ??
