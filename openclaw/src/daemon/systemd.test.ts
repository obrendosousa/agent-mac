...
const TEST_MANAGED_HOME = "/tmp/chappie-test-home";
const GATEWAY_SERVICE = "chappie-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/chappie gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/chappie", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/chappie-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "chappie-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "chappie-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/chappie-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled chappie-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled chappie-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when CHAPPIE_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/chappie-gateway.service",
    },
    {
      name: "uses profile-specific service name when CHAPPIE_PROFILE is set to a custom value",
      env: { HOME: "/home/test", CHAPPIE_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/chappie-gateway-jbphoenix.service",
    },
    {
      name: "prefers CHAPPIE_SYSTEMD_UNIT over CHAPPIE_PROFILE",
      env: {
        HOME: "/home/test",
        CHAPPIE_PROFILE: "jbphoenix",
        CHAPPIE_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles CHAPPIE_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        CHAPPIE_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from CHAPPIE_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        CHAPPIE_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/chappie gateway start --name "My Bot"')).toEqual([
      "/usr/bin/chappie",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('chappie --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["chappie", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('chappie --path "C:\\\\Program Files\\\\Chappie"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["chappie", "--path", "C:\\\\Program Files\\\\Chappie"]);

    expect(
      splitArgsPreservingQuotes('chappie --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["chappie", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/chappie gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads CHAPPIE_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/chappie gateway run", "EnvironmentFile=%h/.chappie/.env"],
      { [`${TEST_SERVICE_HOME}/.chappie/.env`]: "CHAPPIE_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CHAPPIE_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/chappie gateway run",
        "EnvironmentFile=%h/.chappie/.env",
        'Environment="CHAPPIE_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.chappie/.env`]: "CHAPPIE_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CHAPPIE_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.CHAPPIE_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.chappie/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/chappie-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/chappie gateway run",
          'EnvironmentFile=%h/.chappie/first.env "%h/.chappie/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.chappie/first.env") {
        return "CHAPPIE_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.chappie/second env.env") {
        return 'CHAPPIE_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      CHAPPIE_GATEWAY_TOKEN: "first-token",
      CHAPPIE_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "CHAPPIE_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "CHAPPIE_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "CHAPPIE_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CHAPPIE_GATEWAY_TOKEN: "override-token",
      CHAPPIE_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/chappie-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/chappie gateway run",
          "EnvironmentFile=%h/.chappie/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.chappie/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'CHAPPIE_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "CHAPPIE_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CHAPPIE_GATEWAY_TOKEN: "quoted token",
      CHAPPIE_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      CHAPPIE_GATEWAY_TOKEN: "file",
      CHAPPIE_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "chappie-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ CHAPPIE_PROFILE: "work" });
  });
