...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CHAPPIE_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CHAPPIE_PROFILE: profile,
    CHAPPIE_GATEWAY_PORT: String(port),
    CHAPPIE_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CHAPPIE_SYSTEMD_UNIT: systemdUnit,
    CHAPPIE_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    CHAPPIE_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CHAPPIE_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.CHAPPIE_GATEWAY_TOKEN?.trim() || env.CHAPPIEDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CHAPPIE_GATEWAY_TOKEN: gatewayToken,
    CHAPPIE_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CHAPPIE_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CHAPPIE_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CHAPPIE_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CHAPPIE_LOG_PREFIX: "node",
    CHAPPIE_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CHAPPIE_SERVICE_KIND: NODE_SERVICE_KIND,
    CHAPPIE_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    CHAPPIE_STATE_DIR: sharedEnv.stateDir,
    CHAPPIE_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.CHAPPIE_STATE_DIR;
  const configPath = env.CHAPPIE_CONFIG_PATH;
