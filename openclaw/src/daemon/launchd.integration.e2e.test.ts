...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `chappie-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      CHAPPIE_LAUNCHD_LABEL: `ai.chappie.launchd-int-${testId}`,
      CHAPPIE_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
