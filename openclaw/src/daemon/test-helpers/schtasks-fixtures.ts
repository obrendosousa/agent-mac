...
export async function writeGatewayScript(
  ...
  port = Number(env.CHAPPIE_GATEWAY_PORT || "18789"),
) {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "CHAPPIE_GATEWAY_PORT=${port}"`,
      `"C:\\Program Files\\nodejs\\node.exe" "C:\\Users\\chappie-team\\AppData\\Roaming\\npm\\node_modules\\chappie\\dist\\index.js" gateway --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
