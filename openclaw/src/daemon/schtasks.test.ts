...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when CHAPPIE_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".chappie", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when CHAPPIE_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", CHAPPIE_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".chappie-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers CHAPPIE_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        CHAPPIE_PROFILE: "rescue",
        CHAPPIE_STATE_DIR: "C:\\State\\chappie",
      },
      expected: path.join("C:\\State\\chappie", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", CHAPPIE_PROFILE: "default" },
      expected: path.join("/home/test", ".chappie", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Chappie Gateway",
          "cd /d C:\\Projects\\chappie",
          "set NODE_ENV=production",
          "set CHAPPIE_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\chappie",
          environment: {
            NODE_ENV: "production",
            CHAPPIE_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\chappie\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\chappie\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Chappie Share\\node.exe" "\\\\fileserver\\Chappie Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Chappie Share\\node.exe",
            "\\\\fileserver\\Chappie Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from CHAPPIE_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ CHAPPIE_STATE_DIR: path.join(tmpDir, "custom-state") }),
