...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "chappie" | "chappiedbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["chappie", "chappiedbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["chappie_service_marker"];
  const kindKeys = ["chappie_service_kind"];
...
function isChappieGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.chappie.");
}

function isChappieGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("chappie-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isChappieGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("chappie gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("chappiedbot") || lower.includes("moltbot");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "chappiedbot" : "moltbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "chappie" && isChappieGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "chappie" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "chappie" && isChappieGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isChappieGatewayTaskName(name)) {
        continue;
      }
