...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds CHAPPIE_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `chappie gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service CHAPPIE_GATEWAY_TOKEN does not match gateway.auth.token in chappie.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.CHAPPIE_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.CHAPPIE_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `chappie gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
