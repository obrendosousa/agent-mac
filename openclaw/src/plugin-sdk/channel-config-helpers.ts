...
export function createScopedAccountConfigAccessors<ResolvedAccount>(params: {
  resolveAccount: (params: { cfg: ChappieConfig; accountId?: string | null }) => ResolvedAccount;
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: ChappieConfig; accountId?: string | null }) =>
...
export function createScopedChannelConfigBase<
  ResolvedAccount,
  Config extends ChappieConfig = ChappieConfig,
>(params: {
  sectionKey: string;
