...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain(
      'ARG CHAPPIE_NODE_BOOKWORM_IMAGE="node:24-bookworm@sha256:3a09aa6354567619221ef6c45a5051b671f953f0a1924d1f819ffb236e520e6b"',
    );
    expect(dockerfile).toContain(
      'ARG CHAPPIE_NODE_BOOKWORM_SLIM_IMAGE="node:24-bookworm-slim@sha256:e8e2e91b1378f83c5b2dd15f0247f34110e2fe895f6ca7719dbb780f929368eb"',
    );
    expect(dockerfile).toContain("FROM ${CHAPPIE_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${CHAPPIE_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${CHAPPIE_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${CHAPPIE_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG CHAPPIE_INSTALL_BROWSER");
  ...
  it("Docker GPG fingerprint awk uses correct quoting for CHAPPIE_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
