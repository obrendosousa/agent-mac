...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: ChappieConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: ChappieConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/chappie-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<ChappieConfig["bindings"]>[number];
