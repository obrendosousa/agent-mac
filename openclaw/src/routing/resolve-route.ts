...
function listAgents(cfg: ChappieConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ChappieConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ChappieConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ChappieConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ChappieConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ChappieConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ChappieConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ChappieConfig,
  {
    bindingsRef: ChappieConfig["bindings"];
    agentsRef: ChappieConfig["agents"];
    sessionRef: ChappieConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ChappieConfig): Map<string, ResolvedAgentRoute> {
