...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".chappie", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".chappie", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "chappie-home");
    const roots = withEnv(
      {
        CHAPPIE_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        CHAPPIE_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            CHAPPIE_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".chappie", "extensions"),
    });
  });
});
