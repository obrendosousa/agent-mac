...
function createSinglePluginWithEmptySlotsConfig(): ChappieConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): ChappieConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "chappie-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-chappie-extensions/my-plugin",
      },
      extensionsDir,
    });
