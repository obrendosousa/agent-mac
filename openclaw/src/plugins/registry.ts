...
import type {
  ChappiePluginApi,
  ChappiePluginChannelRegistration,
  ChappiePluginCliRegistrar,
  ChappiePluginCommandDefinition,
  ChappiePluginHttpRouteAuth,
  ChappiePluginHttpRouteMatch,
  ChappiePluginHttpRouteHandler,
  ChappiePluginHttpRouteParams,
  ChappiePluginHookOptions,
  ProviderPlugin,
  ChappiePluginService,
  ChappiePluginToolContext,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: ChappiePluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: ChappiePluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: ChappiePluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: ChappiePluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: ChappiePluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: ChappiePluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: ChappiePluginHookOptions | undefined,
    config: ChappiePluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: ChappiePluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: ChappiePluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as ChappiePluginChannelRegistration).plugin === "object"
        ? (registration as ChappiePluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: ChappiePluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: ChappiePluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: ChappiePluginCommandDefinition) => {
