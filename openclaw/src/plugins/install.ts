...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing chappie.extensions; update the plugin package to include chappie.extensions (for example ["./dist/index.js"]). See https://docs.chappie.ai/help/troubleshooting#plugin-install-fails-with-missing-chappie-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_CHAPPIE_EXTENSIONS: "missing_chappie_extensions",
  EMPTY_CHAPPIE_EXTENSIONS: "empty_chappie_extensions",
...
function ensureChappieExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_CHAPPIE_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json chappie.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_CHAPPIE_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureChappieExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from chappie.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-chappie"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "chappie security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "chappie security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "chappie-npm-pack-",
    spec,
    timeoutMs,
