...
import type { ChappiePluginService, ChappiePluginServiceContext } from "./types.js";
...
function createRegistry(services: ChappiePluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: ChappiePluginServiceContext[] = [];

    const serviceA: ChappiePluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: ChappiePluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: ChappiePluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
