...
import { resolveChappiePackageRootSync } from "../infra/chappie-root.js";
...
import { discoverChappiePlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveChappiePackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`chappie/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ChappiePluginDefinition;
  register?: ChappiePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ChappiePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ChappiePluginDefinition;
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverChappiePlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverChappiePlugins>["candidates"][number];
  right: ReturnType<typeof discoverChappiePlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadChappiePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverChappiePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "chappie/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: ChappiePluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as ChappiePluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
