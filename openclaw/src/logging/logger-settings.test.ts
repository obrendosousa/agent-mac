...
let originalChappieLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CHAPPIE_TEST_FILE_LOG;
  originalChappieLogLevel = process.env.CHAPPIE_LOG_LEVEL;
  delete process.env.CHAPPIE_TEST_FILE_LOG;
  delete process.env.CHAPPIE_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CHAPPIE_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalChappieLogLevel === undefined) {
    delete process.env.CHAPPIE_LOG_LEVEL;
  } else {
    process.env.CHAPPIE_LOG_LEVEL = originalChappieLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
