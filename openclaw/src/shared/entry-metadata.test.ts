...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://chappie.ai " },
        frontmatter: { emoji: "🙂", homepage: "https://example.com" },
      }),
    ).toEqual({
      emoji: "🦀",
      homepage: "https://chappie.ai",
    });
  });
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.chappie.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
      homepage: "https://docs.chappie.ai",
    });
    expect(
      resolveEmojiAndHomepage({
        metadata: { homepage: "   " },
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://chappie.ai/install " },
      }),
    ).toEqual({
      homepage: "https://chappie.ai/install",
    });
  });
  ...
  it("does not fall back once frontmatter homepage aliases are present but blank", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          homepage: " ",
          website: "https://docs.chappie.ai",
          url: "https://chappie.ai/install",
        },
      }),
    ).toEqual({});
  });
