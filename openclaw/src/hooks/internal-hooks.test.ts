  ...
  describe("triggerInternalHook", () => {
    ...
    it("stores handlers in the global singleton registry", async () => {
      const globalHooks = globalThis as typeof globalThis & {
        __chappie_internal_hook_handlers__?: Map<string, Array<(event: unknown) => unknown>>;
      };
      const handler = vi.fn();
      registerInternalHook("command:new", handler);
      ...
      expect(handler).toHaveBeenCalledWith(event);
      expect(globalHooks.__chappie_internal_hook_handlers__?.has("command:new")).toBe(true);

      const injectedHandler = vi.fn();
      globalHooks.__chappie_internal_hook_handlers__?.set("command:new", [injectedHandler]);
      await triggerInternalHook(event);
