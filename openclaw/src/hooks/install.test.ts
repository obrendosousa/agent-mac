...
const fixtureRoot = path.join(os.tmpdir(), `chappie-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@chappie/test-hooks",
      version: "0.0.1",
      chappie: { hooks: params.hooks },
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"chappie":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "chappie.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "chappie.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@chappie/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@chappie/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@chappie/test-hooks@0.0.2-beta.1",
      name: "@chappie/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@chappie/test-hooks@beta"');
    }
  });
});
