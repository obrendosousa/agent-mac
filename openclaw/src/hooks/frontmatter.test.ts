...
`;
    ...
    expect(parsed.chappie.emoji).toBe("💾");
    expect(parsed.chappie.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.chappie.emoji).toBe("📝");
    expect(parsed.chappie.events).toEqual(["command"]);
    expect(parsed.chappie.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.chappie.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"chappie": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"chappie": {"events": ["test"]}}');
  });
...
describe("resolveChappieMetadata", () => {
  it("extracts chappie metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveChappieMetadata(frontmatter);
  ...
  it("returns undefined when chappie key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        chappie: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Chappie" },
            { id: "npm", kind: "npm", package: "@chappie/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@chappie/hook");
  });
...
`;
    ...
    const chappie = resolveChappieMetadata(frontmatter);
    expect(chappie).toBeDefined();
    expect(chappie?.emoji).toBe("💾");
    expect(chappie?.events).toEqual(["command:new", "command:reset"]);
    expect(chappie?.requires?.config).toEqual(["workspace.dir"]);
    expect(chappie?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const chappie = resolveChappieMetadata(frontmatter);
    expect(chappie?.emoji).toBe("disk");
    expect(chappie?.events).toEqual(["command:new"]);
  });
});
