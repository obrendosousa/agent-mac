...
async function ensureChappieHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing chappie.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json chappie.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureChappieHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `chappie.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `chappie.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await installFromValidatedNpmSpecArchive({
    tempDirPrefix: "chappie-hook-pack-",
    spec,
    timeoutMs,
