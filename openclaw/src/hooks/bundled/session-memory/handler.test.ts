...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => ChappieConfig;
  action?: "new" | "reset";
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): ChappieConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
