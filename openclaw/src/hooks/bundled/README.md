...
This directory contains hooks that ship with Chappie. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.chappie/chappie.json`:
...
2. Restart gateway: `pkill -9 -f 'chappie.*gateway' && pnpm chappie gateway`
3. Enable the hook: `chappie hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.chappie.ai/automation/hooks
