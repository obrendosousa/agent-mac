...
homepage: https://docs.chappie.ai/automation/hooks#command-logger
metadata:
  {
    "chappie":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.chappie/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.chappie/logs/commands.log ~/.chappie/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/chappie`:
   ```
   /home/username/.chappie/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
