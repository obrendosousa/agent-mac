...
function buildConfigForChappieTarget(entry: SecretRegistryEntry, envId: string): ChappieConfig {
  const config = {} as ChappieConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every chappie.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "chappie.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `CHAPPIE_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForChappieTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `CHAPPIE_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
