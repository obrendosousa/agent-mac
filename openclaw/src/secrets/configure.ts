...
function getSecretProviders(config: ChappieConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: ChappieConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: ChappieConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "chappie.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
...
function resolveConfigureAgentId(config: ChappieConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: ChappieConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredChappieConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "chappie.json",
