...
const CHAPPIE_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of CHAPPIE_COMPILED_SECRET_TARGETS) {
...
const CHAPPIE_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: CHAPPIE_COMPILED_SECRET_TARGETS,
    entriesById: CHAPPIE_TARGETS_BY_ID,
  });
