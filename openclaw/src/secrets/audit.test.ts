...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-secrets-audit-"));
  const stateDir = path.join(rootDir, ".chappie");
...
describe("secrets audit", () => {
  ...
  it("scans active agent-dir override models.json even when outside state dir", async () => {
    ...
    const report = await runSecretsAudit({
      env: {
        ...fixture.env,
        CHAPPIE_AGENT_DIR: externalAgentDir,
      },
    });
    expect(
      hasFinding(
  ...
  it("does not flag non-sensitive routing headers in chappie config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
