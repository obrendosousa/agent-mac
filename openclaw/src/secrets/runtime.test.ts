...
import { loadConfig, type ChappieConfig, writeConfigFile } from "../config/config.js";
...
function asConfig(value: unknown): ChappieConfig {
  return value as ChappieConfig;
}

const OPENAI_ENV_KEY_REF = { source: "env", provider: "default", id: "OPENAI_API_KEY" } as const;

function createOpenAiFileModelsConfig(): NonNullable<ChappieConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("resolves env refs for config and auth profiles", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/chappie-agent-main"],
      loadAuthStore: () =>
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: ChappieConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    ...
    const store = ensureAuthProfileStore("/tmp/chappie-agent-main");
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("chappie-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("chappie-secrets-runtime-refresh-fail-", async (home) => {
      const configDir = path.join(home, ".chappie");
      const secretFile = path.join(configDir, "secrets.json");
      const agentDir = path.join(configDir, "agents", "main", "agent");
      const authStorePath = path.join(agentDir, "auth-profiles.json");
  ...
  it("keeps last-known-good web runtime snapshot when reload introduces unresolved active web refs", async () => {
    await withTempHome("chappie-secrets-runtime-web-reload-lkg-", async (home) => {
      const prepared = await prepareSecretsRuntimeSnapshot({
        config: asConfig({
          tools: {
            web: {
      ...
      const persistedConfig = JSON.parse(
        await fs.readFile(path.join(home, ".chappie", "chappie.json"), "utf8"),
      ) as ChappieConfig;
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("chappie-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".chappie", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".chappie", "agents", "ops", "agent");
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-secrets-runtime-"));
    ...
    const prevStateDir = process.env.CHAPPIE_STATE_DIR;
    ...
    try {
      ...
      process.env.CHAPPIE_STATE_DIR = stateDir;
      ...
      if (prevStateDir === undefined) {
        delete process.env.CHAPPIE_STATE_DIR;
      } else {
        process.env.CHAPPIE_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
