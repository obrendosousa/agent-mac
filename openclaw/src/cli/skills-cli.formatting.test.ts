...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-bundled-skills-test-"));
    process.env.CHAPPIE_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
