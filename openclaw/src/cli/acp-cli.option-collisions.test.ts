...
describe("acp cli option collisions", () => {
  ...
  it.each([
    ...
    await withTempSecretFiles("chappie-acp-cli-", files, async ({ tokenFile, passwordFile }) => {
  ...
  it("trims token file path before reading", async () => {
    await withTempSecretFiles("chappie-acp-cli-", { token: "tok_file\n" }, async (files) => {
  ...
  it("reports missing token-file read errors", async () => {
    await parseAcp(["--token-file", "/tmp/chappie-acp-missing-token.txt"]);
