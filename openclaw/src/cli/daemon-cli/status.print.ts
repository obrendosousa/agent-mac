...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("chappie doctor")}" (or "${formatCliCommand("chappie doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("chappie gateway install --force")}\` from the same --profile / CHAPPIE_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("chappie gateway restart")} and re-check with ${formatCliCommand("chappie gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.CHAPPIE_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("chappie gateway install")}`),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = service.command?.environment ?? process.env;
      const unit = resolveGatewaySystemdServiceName(env.CHAPPIE_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("chappie status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.chappie.ai/troubleshooting`);
}
