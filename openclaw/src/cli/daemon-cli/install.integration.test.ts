...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("chappie-daemon-install-int-");
    configPath = path.join(tempHome, "chappie.json");
    process.env.HOME = tempHome;
    process.env.CHAPPIE_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CHAPPIE_GATEWAY_TOKEN).toBeUndefined();
  });
});
