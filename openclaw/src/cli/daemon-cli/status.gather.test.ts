...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    CHAPPIE_STATE_DIR: "/tmp/chappie-daemon",
    CHAPPIE_CONFIG_PATH: "/tmp/chappie-daemon/chappie.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.CHAPPIE_STATE_DIR ?? "/tmp/chappie-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.CHAPPIE_CONFIG_PATH ?? `${stateDir}/chappie.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/chappie-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CHAPPIE_STATE_DIR = "/tmp/chappie-cli";
    process.env.CHAPPIE_CONFIG_PATH = "/tmp/chappie-cli/chappie.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.CHAPPIE_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.CHAPPIE_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.CHAPPIE_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "chappie-gateway" }],
        hints: [],
      },
      healthy: false,
