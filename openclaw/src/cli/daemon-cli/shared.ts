...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("chappie gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.CHAPPIE_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.CHAPPIE_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("chappie gateway install", env),
    startCommand: formatCliCommand("chappie gateway", env),
