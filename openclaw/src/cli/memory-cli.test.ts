...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("chappie memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('chappie memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('chappie memory search --query "deployment" --max-results 20');
