...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      CHAPPIE_GATEWAY_PORT: String(params.port),
      ...(params.token ? { CHAPPIE_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CHAPPIE_STATE_DIR = "/tmp/chappie-cli-state";
    process.env.CHAPPIE_CONFIG_PATH = "/tmp/chappie-cli-state/chappie.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        CHAPPIE_PROFILE: "dev",
        CHAPPIE_STATE_DIR: "/tmp/chappie-daemon-state",
        CHAPPIE_CONFIG_PATH: "/tmp/chappie-daemon-state/chappie.json",
        CHAPPIE_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.chappie.gateway.plist",
    });
