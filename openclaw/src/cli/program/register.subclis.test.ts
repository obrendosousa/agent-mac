...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.CHAPPIE_DISABLE_LAZY_SUBCOMMANDS;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.CHAPPIE_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.CHAPPIE_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "chappie", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "chappie"]);

    const names = program.commands.map((cmd) => cmd.name());
    expect(names).toContain("acp");
    expect(names).toContain("gateway");
    expect(names).toContain("chappiebot");
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "chappie", "nodes", "list"], "chappie");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "chappie", "acp", "--help"], "chappie");
