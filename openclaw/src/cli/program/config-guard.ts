...
export async function ensureConfigReady(params: {
  ...
  if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
    ...
    if (!params.suppressDoctorStdout) {
      ...
      const originalSuppressNotes = process.env.CHAPPIE_SUPPRESS_NOTES;
      process.stdout.write = (() => true) as unknown as typeof process.stdout.write;
      process.env.CHAPPIE_SUPPRESS_NOTES = "1";
      try {
        await runDoctorConfigFlow();
      } finally {
        process.stdout.write = originalStdoutWrite;
        if (originalSuppressNotes === undefined) {
          delete process.env.CHAPPIE_SUPPRESS_NOTES;
        } else {
          process.env.CHAPPIE_SUPPRESS_NOTES = originalSuppressNotes;
        }
      }
    }
  }
  ...
  params.runtime.error(
    `${muted("Run:")} ${commandText(formatCliCommand("chappie doctor --fix"))}`,
  );
  if (!allowInvalid) {
