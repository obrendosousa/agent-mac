...
${formatHelpExamples([
  ['chappie agent --to +15555550123 --message "status update"', "Start a new session."],
  ['chappie agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'chappie agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'chappie agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['chappie agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'chappie agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.chappie.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.chappie.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['chappie agents set-identity --agent main --name "Chappie" --emoji "🦞"', "Set name + emoji."],
  ["chappie agents set-identity --agent main --avatar avatars/chappie.png", "Set avatar path."],
  [
    "chappie agents set-identity --workspace ~/.chappie/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "chappie agents set-identity --identity-file ~/.chappie/workspace/IDENTITY.md --agent main",
