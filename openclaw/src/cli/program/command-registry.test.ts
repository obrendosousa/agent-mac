...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "chappie", "doctor"]);
  ...
  it("does not narrow to the primary command when help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "chappie", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "chappie", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "chappie", "status"], async () => {
      await program.parseAsync(["node", "chappie", "status"]);
    });
