    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "chappie node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["chappie node status", "Check node host service status."],
          ["chappie node install", "Install the node host service."],
          ["chappie node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.chappie.ai/cli/node")}\n`,
    );
