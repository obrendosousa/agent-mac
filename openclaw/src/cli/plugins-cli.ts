...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: ChappieConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: ChappieConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.chappie.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: ChappieConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.chappie.ai/plugin");
      lines.push("");
