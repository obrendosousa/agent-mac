    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chappie nodes status", "List known nodes with live status."],
          ["chappie nodes pairing pending", "Show pending node pairing requests."],
          ['chappie nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["chappie nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.chappie.ai/cli/nodes")}\n`,
    );
