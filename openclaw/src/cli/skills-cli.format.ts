...
function appendChappieHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("chappie skills list")}\` to see all skills.`
      : "No skills found.";
    return appendChappieHubHint(message, opts.json);
  }
  ...
  return appendChappieHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendChappieHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("chappie skills list")}\` to see available skills.`,
      opts.json,
    );
  }
