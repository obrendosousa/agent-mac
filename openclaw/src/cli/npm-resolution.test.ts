...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@chappie/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@chappie/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@chappie/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@chappie/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @chappie/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@chappie/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@chappie/plugin-alpha@1.2.3",
        installPath: "/tmp/chappie/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@chappie/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @chappie/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@chappie/plugin-alpha@latest",
      true,
      "/tmp/chappie/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@chappie/plugin-alpha@latest",
      installPath: "/tmp/chappie/extensions/alpha",
      version: "1.2.3",
