...
const SANDBOX_EXAMPLES = {
  main: [
    ["chappie sandbox list", "List all sandbox containers."],
    ["chappie sandbox list --browser", "List only browser containers."],
    ["chappie sandbox recreate --all", "Recreate all containers."],
    ["chappie sandbox recreate --session main", "Recreate a specific session."],
    ["chappie sandbox recreate --agent mybot", "Recreate agent containers."],
    ["chappie sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["chappie sandbox list", "List all sandbox containers."],
    ["chappie sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["chappie sandbox recreate --all", "Recreate all containers."],
    ["chappie sandbox recreate --session main", "Recreate a specific session."],
    ["chappie sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["chappie sandbox recreate --browser --all", "Recreate only browser containers."],
    ["chappie sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["chappie sandbox explain", "Show effective sandbox config."],
    ["chappie sandbox explain --session agent:main:main", "Explain a specific session."],
    ["chappie sandbox explain --agent work", "Explain an agent sandbox."],
    ["chappie sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.chappie.ai/cli/sandbox")}\n`,
    )
    .action(() => {
