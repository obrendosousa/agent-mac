...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Chappie and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["chappie update", "Update a source checkout (git)"],
        ["chappie update --channel beta", "Switch to beta channel (git + npm)"],
        ["chappie update --channel dev", "Switch to dev channel (git + npm)"],
        ["chappie update --tag beta", "One-off update to a dist-tag or version"],
        ["chappie update --dry-run", "Preview actions without changing anything"],
        ["chappie update --no-restart", "Update without restarting the service"],
        ["chappie update --json", "Output result as JSON"],
        ["chappie update --yes", "Non-interactive (accept downgrade prompts)"],
        ["chappie update wizard", "Interactive update wizard"],
        ["chappie --update", "Shorthand for chappie update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run chappie update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.chappie.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chappie update status", "Show channel + version status."],
          ["chappie update status --json", "JSON output."],
          ["chappie update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.chappie.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
