...
import type { ChappieConfig, ConfigFileSnapshot } from "../config/types.chappie.js";
...
vi.mock("../infra/chappie-root.js", () => ({
  resolveChappiePackageRoot: vi.fn(),
}));
...
const { resolveChappiePackageRoot } = await import("../infra/chappie-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/chappie-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveChappiePackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("chappie-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveChappiePackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("chappie");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/chappie-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "chappie-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Chappie update status");
  });
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ChappieConfig,
        });
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "chappie@latest", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "chappie@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("chappie-update");
    const localAppData = createCaseDir("chappie-localappdata");
  ...
  it("uses CHAPPIE_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("chappie-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/chappie-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("chappie-updated-root");
  ...
  it("updateCommand preserves invocation-relative service env overrides during refresh", async () => {
    ...
    await withEnvAsync(
      {
        CHAPPIE_STATE_DIR: "./state",
        CHAPPIE_CONFIG_PATH: "./config/chappie.json",
      },
      async () => {
        await updateCommand({});
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          CHAPPIE_STATE_DIR: path.resolve("./state"),
          CHAPPIE_CONFIG_PATH: path.resolve("./config/chappie.json"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand reuses the captured invocation cwd when process.cwd later fails", async () => {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: root,
        env: expect.objectContaining({
          CHAPPIE_STATE_DIR: path.resolve(originalCwd, "./state"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ CHAPPIE_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.CHAPPIE_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("chappie-update-wizard");
    await withEnvAsync({ CHAPPIE_GIT_DIR: tempDir }, async () => {
      setTty(true);
