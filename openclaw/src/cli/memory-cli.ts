    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chappie memory status", "Show index and provider status."],
          ["chappie memory status --deep", "Probe embedding provider readiness."],
          ["chappie memory index --force", "Force a full reindex."],
          ['chappie memory search "meeting notes"', "Quick search using positional query."],
          [
            'chappie memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["chappie memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.chappie.ai/cli/memory")}\n`,
    );
