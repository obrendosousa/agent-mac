...
describe("resolveCliChannelOptions", () => {
  afterEach(() => {
    delete process.env.CHAPPIE_EAGER_CHANNEL_OPTIONS;
    vi.resetModules();
    vi.clearAllMocks();
  });
  ...
  it("respects eager mode and includes loaded plugin ids", async () => {
    process.env.CHAPPIE_EAGER_CHANNEL_OPTIONS = "1";
  ...
  it("keeps dynamic catalog resolution when external catalog env is set", async () => {
    process.env.CHAPPIE_PLUGIN_CATALOG_PATHS = "/tmp/plugins-catalog.json";
    ...
    expect(listCatalogMock).toHaveBeenCalledOnce();
    delete process.env.CHAPPIE_PLUGIN_CATALOG_PATHS;
  });
});
