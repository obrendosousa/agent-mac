...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): ChappieConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as ChappieConfig;
  }
  return {
    channels: {
      testchannel: {
...
describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as ChappieConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as ChappieConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as ChappieConfig)).toBe("default");
    });
  });
});
