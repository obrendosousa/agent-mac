...
function telegramCfg(): ChappieConfig {
  return { channels: { telegram: { botToken: "tok" } } } as ChappieConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: ChappieConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as ChappieConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as ChappieConfig;
...
function createSignalAccountOverrideCfg(): ChappieConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as ChappieConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as ChappieConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as ChappieConfig,
        accountId: undefined,
        params: {
