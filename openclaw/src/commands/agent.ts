...
function prependInternalEventContext(
  ...
  if (body.includes("Chappie runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("chappie agents list")}" to see configured agents.`,
      );
    }
  }
