...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".chappie", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: ChappieConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["CHAPPIEDBOT_GATEWAY_TOKEN", await getenv("CHAPPIEDBOT_GATEWAY_TOKEN")],
    ["CHAPPIEDBOT_GATEWAY_PASSWORD", await getenv("CHAPPIEDBOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`CHAPPIE_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["CHAPPIE_GATEWAY_TOKEN", await getenv("CHAPPIE_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["CHAPPIE_GATEWAY_PASSWORD", await getenv("CHAPPIE_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "CHAPPIE_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("CHAPPIEDBOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use CHAPPIE_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> CHAPPIE_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.CHAPPIE_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- CHAPPIE_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
