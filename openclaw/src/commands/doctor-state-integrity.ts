...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".chappie"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for CHAPPIE_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // CHAPPIE_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: ChappieConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.CHAPPIE_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".chappie");
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.chappie).",
        `  Set locally: CHAPPIE_STATE_DIR=~/.chappie ${formatCliCommand("chappie doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`chappie sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`chappie sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`chappie sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.chappie out of git; it contains credentials and session history.",
