...
const ensureChappieModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveChappieAgentDir = vi.fn().mockReturnValue("/tmp/chappie-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureChappieModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveChappieAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureChappieModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureChappieModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureChappieModelsJson).toHaveBeenCalled();
    expect(ensureChappieModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
