...
function createTelegramCfg(botToken: string, enabled?: boolean): ChappieConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelOnboardingAdapter>[1]) {
  return patchChannelOnboardingAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: ChappieConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as ChappieConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as ChappieConfig, prompter, {
      quickstartDefaults: true,
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: ChappieConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: ChappieConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
