...
describe("noteStartupOptimizationHints", () => {
  ...
  it("warns when compile cache is under /tmp and no-respawn is not set", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/tmp/chappie-compile-cache",
      },
      { platform: "linux", arch: "arm64", totalMemBytes: 4 * 1024 ** 3, noteFn },
    );
    ...
    expect(message).toContain("CHAPPIE_NO_RESPAWN is not set to 1");
    expect(message).toContain("export NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache");
    expect(message).toContain("export CHAPPIE_NO_RESPAWN=1");
  });
