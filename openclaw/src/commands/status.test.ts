...
beforeAll(() => {
  envSnapshot = captureEnv(["CHAPPIE_PROFILE"]);
  process.env.CHAPPIE_PROFILE = "isolated";
});
...
vi.mock("../infra/chappie-root.js", () => ({
  resolveChappiePackageRoot: vi.fn().mockResolvedValue("/tmp/chappie"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/chappie/pnpm-lock.yaml",
      markerPath: "/tmp/chappie/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.chappie.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.chappie.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("chappie status --all") ||
          line.includes("chappie --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("CHAPPIE_GATEWAY_TOKEN", "abcd1234", async () => {
