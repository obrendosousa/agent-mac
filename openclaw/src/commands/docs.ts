...
const SEARCH_TOOL = "https://docs.chappie.ai/mcp.SearchChappie";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.chappie.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('chappie docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.chappie.ai/");
      runtime.log(`Search: ${formatCliCommand('chappie docs "your query"')}`);
    }
    return;
  }
