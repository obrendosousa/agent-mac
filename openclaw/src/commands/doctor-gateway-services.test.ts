...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.CHAPPIE_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.CHAPPIE_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      CHAPPIE_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service CHAPPIE_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: ChappieConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.CHAPPIE_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses CHAPPIE_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ CHAPPIE_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/chappie/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/chappie@2026.3.12/node_modules/chappie/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    fsMocks.realpath.mockImplementation(async (value: string) => {
      if (value.includes("/global/5/node_modules/chappie/")) {
        return value.replace(
          "/global/5/node_modules/chappie/",
          "/global/5/node_modules/.pnpm/chappie@2026.3.12/node_modules/chappie/",
        );
      }
      return value;
    });
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/opt/chappie/../chappie/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/chappie/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    await withEnvAsync(
      {
        ...
        CHAPPIEDBOT_GATEWAY_TOKEN: undefined,
      },
      async () => {
        setupGatewayTokenRepairScenario();
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            CHAPPIE_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Chappie gateway next.",
    );
  });
});
