...
type EnvSnapshot = {
  HOME?: string;
  CHAPPIE_HOME?: string;
  CHAPPIE_STATE_DIR?: string;
  CHAPPIE_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CHAPPIE_HOME: process.env.CHAPPIE_HOME,
    CHAPPIE_STATE_DIR: process.env.CHAPPIE_STATE_DIR,
    CHAPPIE_OAUTH_DIR: process.env.CHAPPIE_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ChappieConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ChappieConfig) {
...
async function runStateIntegrityText(cfg: ChappieConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.CHAPPIE_HOME = tempHome;
    process.env.CHAPPIE_STATE_DIR = path.join(tempHome, ".chappie");
    delete process.env.CHAPPIE_OAUTH_DIR;
    fs.mkdirSync(process.env.CHAPPIE_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when CHAPPIE_OAUTH_DIR is explicitly configured", async () => {
    process.env.CHAPPIE_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ChappieConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints chappie-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ChappieConfig = {};
    ...
    expect(text).toMatch(/chappie sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/chappie sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /chappie sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
