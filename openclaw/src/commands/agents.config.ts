...
function resolveAgentName(cfg: ChappieConfig, agentId: string) {
...
function resolveAgentModel(cfg: ChappieConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ChappieConfig): AgentSummary[] {
