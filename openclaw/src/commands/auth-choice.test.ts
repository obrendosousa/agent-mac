...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("chappie-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/chappie-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireChappieAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
