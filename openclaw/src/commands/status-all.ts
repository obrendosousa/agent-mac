  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const readServiceSummary = async (service: GatewayService) => {
      try {
        ...
        return {
          ...
          managedByChappie: summary.managedByChappie,
    ...
    const overviewRows = [
      ...
      ...(probeAuthResolution.warning
        ...
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("chappie security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.managedByChappie ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.managedByChappie ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
