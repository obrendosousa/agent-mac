...
const secretRefKeyMock = vi.hoisted(() => vi.fn(() => "env:default:CHAPPIE_GATEWAY_TOKEN"));
...
describe("resolveGatewayInstallToken", () => {
  ...
  it("validates SecretRef token but does not persist resolved plaintext", async () => {
    const tokenRef = { source: "env", provider: "default", id: "CHAPPIE_GATEWAY_TOKEN" };
    resolveSecretInputRefMock.mockReturnValue({ ref: tokenRef });
    resolveSecretRefValuesMock.mockResolvedValue(
      new Map([["env:default:CHAPPIE_GATEWAY_TOKEN", "resolved-token"]]),
    );
    ...
    const result = await resolveGatewayInstallToken({
      ...
      env: { CHAPPIE_GATEWAY_TOKEN: "resolved-token" } as NodeJS.ProcessEnv,
    });
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("chappie config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("chappie config set gateway.auth.mode password");
  ...
  it("drops generated plaintext when config changes to SecretRef before persist", async () => {
    ...
    resolveSecretInputRefMock.mockReturnValueOnce({ ref: undefined }).mockReturnValueOnce({
      ref: { source: "env", provider: "default", id: "CHAPPIE_GATEWAY_TOKEN" },
    });
