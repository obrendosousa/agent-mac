...
function resolveSandboxDockerImage(cfg: ChappieConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: ChappieConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: ChappieConfig, image: string): ChappieConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: ChappieConfig, image: string): ChappieConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: chappie config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
...
export function noteSandboxScopeWarnings(cfg: ChappieConfig) {
