...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.CHAPPIE_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("chappie gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
