...
import { readConfigFileSnapshot, writeConfigFile, type ChappieConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("chappie config set gateway.auth.mode token")} or ${formatCliCommand("chappie config set gateway.auth.mode password")}.`,
  ].join(" ");
}
