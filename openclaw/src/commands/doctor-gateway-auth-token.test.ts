...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to CHAPPIE_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        CHAPPIE_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to CHAPPIE_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
  ...
  it("requires token in inferred mode when password env exists only in shell", async () => {
    await withEnvAsync(
      { [envVar("CHAPPIE", "GATEWAY", "PASSWORD")]: "password-from-env" },
      async () => {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            CHAPPIE_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as ChappieConfig,
      {} as NodeJS.ProcessEnv,
