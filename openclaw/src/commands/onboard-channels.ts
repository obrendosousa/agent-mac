...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("chappie pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`chappie pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!getChannelPlugin(channel)) {
      ...
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("chappie plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("chappie channels add")}\``,
        },
      ],
