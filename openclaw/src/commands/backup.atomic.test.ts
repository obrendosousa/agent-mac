...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("chappie-backup-atomic-test-");
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".chappie");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-backup-race-"));
  ...
  it("falls back to exclusive copy when hard-link publication is unsupported", async () => {
    const stateDir = path.join(tempHome.home, ".chappie");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-backup-copy-fallback-"));
    const linkSpy = vi.spyOn(fs, "link");
    try {
      await fs.writeFile(path.join(stateDir, "chappie.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
