...
import { writeConfigFile, type ChappieConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service CHAPPIE_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (chappiedbot/moltbot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      const removed: string[] = [];
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Chappie gateway next.");
      }
    }
  }
