...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.CHAPPIE_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-oauth-"));
    process.env.CHAPPIE_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.CHAPPIE_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-wa-auth-"));
    try {
