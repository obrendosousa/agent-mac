...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ChappieConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ChappieConfig) => { changed: boolean; next: ChappieConfig };
  defaultModel: string;
  overrideConfig: ChappieConfig;
  alreadyDefaultConfig: ChappieConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ChappieConfig) => config,
      applyDefaultConfig: (config: ChappieConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ChappieConfig = { agents: { defaults: {} } };
