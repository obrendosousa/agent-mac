...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CHAPPIE_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CHAPPIE_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.CHAPPIE_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("chappie config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain(
      "chappie config set gateway.auth.mode password",
    );
  });
});
