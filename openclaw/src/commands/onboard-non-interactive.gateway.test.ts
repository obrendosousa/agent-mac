...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("chappie-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "chappie");
  ...
  it("uses CHAPPIE_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.CHAPPIE_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveOnboarding(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "CHAPPIE_GATEWAY_TOKEN",
          },
          runtime,
        );
        ...
        runNonInteractiveOnboarding(
          {
            nonInteractive: true,
            mode: "local",
            workspace: path.join(stateDir, "chappie"),
            authChoice: "skip",
            skipSkills: true,
            skipHealth: false,
            installDaemon: false,
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `chappie gateway status --deep` for more detail.");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.CHAPPIE_STATE_DIR = stateDir;
      process.env.CHAPPIE_CONFIG_PATH = path.join(stateDir, "chappie.json");
