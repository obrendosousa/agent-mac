...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByChappie ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByChappie ? "installed · " : "";
  ...
  runtime.log(theme.heading("Chappie status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`chappie devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("chappie devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("chappie devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("chappie security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("chappie security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.chappie.ai/faq");
  runtime.log("Troubleshooting: https://docs.chappie.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("chappie status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("chappie logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("chappie status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("chappie gateway probe")}`);
  }
}
