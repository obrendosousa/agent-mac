...
import type { ChappieConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ChappieConfig): string[] {
...
export async function noteSecurityWarnings(cfg: ChappieConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("chappie security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.chappie/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("chappie approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("chappie configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("chappie config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("chappie doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("chappie config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('chappie config set session.dmScope "per-channel-peer"') +
