 ...
 * Runs as part of `chappie doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("chappie memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("chappie config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("chappie memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("chappie configure --section model")}`,
        `- To disable: ${formatCliCommand("chappie config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("chappie memory status --deep")}`,
      ].join("\n"),
      "Memory search",
