...
export const resolveChappiePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadChappiePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveChappiePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CHAPPIE_STATE_DIR;
  originalUpdateInProgress = process.env.CHAPPIE_UPDATE_IN_PROGRESS;
  process.env.CHAPPIE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CHAPPIE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CHAPPIE_UPDATE_IN_PROGRESS;
  } else {
    process.env.CHAPPIE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
