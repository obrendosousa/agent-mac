...
function rawKeyValue(config: ChappieConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: ChappieConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: ChappieConfig, provider: SearchProvider): ChappieConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: ChappieConfig, result: ChappieConfig): ChappieConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with chappie configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Chappie will store a reference instead of the API key.",
