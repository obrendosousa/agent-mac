...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.chappie", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.chappie",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.chappie", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/chappie-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/chappie/.chappie",
    });

    expect(result).toEqual({
      path: "/mnt/slow/chappie/.chappie",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".chappie"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.chappie`;
