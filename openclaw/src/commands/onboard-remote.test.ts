...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["CHAPPIE_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// hostname remote URLs when CHAPPIE_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.CHAPPIE_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://chappie-gateway.ai:18789")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18789")).toContain("Use wss://");
        return "ws://chappie-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://chappie-gateway.ai:18789");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.CHAPPIE_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18789";
      }
      if (params.message === "Environment variable name") {
        return "CHAPPIE_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
