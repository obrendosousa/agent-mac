...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/chappie/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\chappie\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/chappie/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { CHAPPIE_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ CHAPPIE_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.CHAPPIE_PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:chappie|chappie)( --profile isolated)? gateway install/,
    );
  });
});
