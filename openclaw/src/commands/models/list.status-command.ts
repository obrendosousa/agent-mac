...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveChappieAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("chappie models auth setup-token")}\` or \`${formatCliCommand("chappie configure")}\`.`
          : `Run \`${formatCliCommand("chappie configure")}\` or set an API key env var.`;
