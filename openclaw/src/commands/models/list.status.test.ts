...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveChappieAgentDir: vi.fn().mockReturnValue("/tmp/chappie-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/chappie-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/chappie-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/chappie-dev/chappie.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveChappieAgentDir: mocks.resolveChappieAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/chappie-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveChappieAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/chappie-dev/chappie.json");
    expect(payload.auth.storePath).toBe("/tmp/chappie-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/chappie-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/chappie-agent-custom");
