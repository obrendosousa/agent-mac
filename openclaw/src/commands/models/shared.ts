...
export async function loadValidConfigOrThrow(): Promise<ChappieConfig> {
...
export async function updateConfig(
  mutator: (cfg: ChappieConfig) => ChappieConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ChappieConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ChappieConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("chappie agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
