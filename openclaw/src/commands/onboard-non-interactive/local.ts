...
export async function runNonInteractiveOnboardingLocal(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `chappie gateway run` or `chappie agent --local`.",
              ]
            : [`Run \`${formatCliCommand("chappie gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("chappie gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("chappie configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.chappie.ai/tools/web`,
    );
  }
}
