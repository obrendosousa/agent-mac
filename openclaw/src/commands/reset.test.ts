...
describe("resetCommand", () => {
  ...
  beforeEach(() => {
    ...
    resolveCleanupPlanFromDisk.mockReturnValue({
      stateDir: "/tmp/.chappie",
      configPath: "/tmp/.chappie/chappie.json",
      oauthDir: "/tmp/.chappie/credentials",
      configInsideState: true,
      oauthInsideState: true,
      workspaceDirs: ["/tmp/.chappie/workspace"],
    });
    removePath.mockResolvedValue({ ok: true });
    listAgentSessionDirs.mockResolvedValue(["/tmp/.chappie/agents/main/sessions"]);
  ...
  it("recommends creating a backup before state-destructive reset scopes", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("chappie backup create"));
  });
  ...
  it("does not recommend backup for config-only reset", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("chappie backup create"));
  });
});
