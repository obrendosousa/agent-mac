...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CHAPPIE_GATEWAY_TOKEN;
    prevPassword = process.env.CHAPPIE_GATEWAY_PASSWORD;
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as ChappieConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CHAPPIE_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as ChappieConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as ChappieConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as ChappieConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("chappie approvals get --gateway");
  });
