...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under CHAPPIE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("chappie-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-oauth-sync-"));
    ...
    process.env.CHAPPIE_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-oauth-nosync-"));
    process.env.CHAPPIE_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CHAPPIE_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-oauth-external-"));
    process.env.CHAPPIE_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CHAPPIE_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CHAPPIE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("chappie-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
