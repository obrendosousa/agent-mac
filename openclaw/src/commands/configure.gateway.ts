...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "CHAPPIE_GATEWAY_TOKEN",
          placeholder: "CHAPPIE_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like CHAPPIE_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Chappie will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Chappie trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.chappie.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
