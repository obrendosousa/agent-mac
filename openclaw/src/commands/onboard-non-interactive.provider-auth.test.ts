...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "chappie.json");
    ...
    await withEnvAsync(
      {
        ...
        CHAPPIE_GATEWAY_PASSWORD: undefined,
        CUSTOM_API_KEY: undefined,
        CHAPPIE_DISABLE_CONFIG_CACHE: "1",
      },
      async () => {
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("chappie-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("chappie-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("chappie-onboard-zai-", async (env) => {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("chappie-onboard-zai-cn-", async (env) => {
      detectZaiEndpoint.mockResolvedValueOnce({
        endpoint: "coding-cn",
  ...
  it("supports Z.AI Coding Plan global endpoint with GLM-5 when available", async () => {
    await withOnboardEnv("chappie-onboard-zai-coding-global-", async (env) => {
      detectZaiEndpoint.mockResolvedValueOnce({
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-xai-", async (env) => {
  ...
  it("infers Mistral auth choice from --mistral-api-key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("chappie-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("chappie-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "chappie-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
    ...
    {
      name: "openai",
      prefix: "chappie-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
    ...
    {
      name: "openrouter",
      prefix: "chappie-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "chappie-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
    ...
    {
      name: "volcengine",
      prefix: "chappie-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
    ...
    {
      name: "byteplus",
      prefix: "chappie-onboard-ref-flag-byteplus-",
  ...
  it("stores the detected env alias as keyRef for both OpenCode runtime providers", async () => {
    await withOnboardEnv("chappie-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("configures vLLM via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("chappie-onboard-vllm-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "vllm",
  ...
  it("configures SGLang via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("chappie-onboard-sglang-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "sglang",
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "chappie-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "chappie-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers Model Studio auth choice from --modelstudio-api-key and sets default model", async () => {
    await withOnboardEnv("chappie-onboard-modelstudio-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("chappie-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveOnboardingWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key"; // pragma: allowlist secret
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref", // pragma: allowlist secret
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv("chappie-onboard-custom-provider-ref-flag-", async ({ runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("chappie-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "chappie-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            customApiKey: "custom-test-key", // pragma: allowlist secret
