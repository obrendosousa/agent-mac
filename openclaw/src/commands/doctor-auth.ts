...
function pruneAuthProfiles(
  cfg: ChappieConfig,
  profileIds: Set<string>,
): { next: ChappieConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("chappie models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("chappie models auth setup-token")} or ${formatCliCommand(
      "chappie configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "chappie models auth login --provider openai-codex",
    )} or ${formatCliCommand("chappie configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("chappie configure")}\` or \`${formatCliCommand("chappie onboard")}\`.`;
}
