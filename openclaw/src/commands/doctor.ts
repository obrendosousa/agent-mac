...
import { resolveChappiePackageRoot } from "../infra/chappie-root.js";
...
function resolveMode(cfg: ChappieConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Chappie doctor");

  const root = await resolveChappiePackageRoot({
  ...
  let cfg: ChappieConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("chappie configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("chappie config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("chappie setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("chappie config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("chappie config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("chappie doctor --fix")}" to apply changes.`);
  }
