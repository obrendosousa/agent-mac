...
import { ChappieSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: ChappieConfig): {
  config: ChappieConfig;
  removed: string[];
} {
  const parsed = ChappieSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: ChappieConfig): void {
