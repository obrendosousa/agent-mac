...
import type { GatewayBindMode, GatewayTailscaleMode, ChappieConfig } from "../config/config.js";
...
export async function configureGatewayForOnboarding(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.CHAPPIE_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.CHAPPIE_GATEWAY_TOKEN) ??
          "",
      });
