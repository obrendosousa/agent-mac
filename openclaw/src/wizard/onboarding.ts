...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Chappie is a hobby project and still in beta. Expect sharp edges.",
      "By default, Chappie is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Chappie is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Chappie.",
      ...
      "",
      "Must read: https://docs.chappie.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Chappie onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: ChappieConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.chappie.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("chappie doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("chappie configure")}.`;
  ...
  let localGatewayToken = process.env.CHAPPIE_GATEWAY_TOKEN ?? process.env.CHAPPIEDBOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.CHAPPIE_GATEWAY_PASSWORD ?? process.env.CHAPPIEDBOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: ChappieConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
