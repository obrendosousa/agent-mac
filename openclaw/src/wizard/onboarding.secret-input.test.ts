...
function makeConfig(): ChappieConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveOnboardingSecretInputString", () => {
  it("resolves env-template SecretInput strings", async () => {
    const resolved = await resolveOnboardingSecretInputString({
      config: makeConfig(),
      value: "${CHAPPIE_GATEWAY_PASSWORD}",
      path: "gateway.auth.password",
      env: {
        CHAPPIE_GATEWAY_PASSWORD: "gateway-secret", // pragma: allowlist secret
      },
    });
  ...
  it("throws with path context when env-template SecretRef cannot resolve", async () => {
    await expect(
      ...
      'gateway.auth.password: failed to resolve SecretRef "env:default:CHAPPIE_GATEWAY_PASSWORD"',
    );
  });
});
