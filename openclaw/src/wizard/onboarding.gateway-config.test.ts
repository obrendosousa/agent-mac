...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers CHAPPIE_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.CHAPPIE_GATEWAY_TOKEN;
    process.env.CHAPPIE_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.CHAPPIE_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "CHAPPIE_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
  ...
  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.CHAPPIE_GATEWAY_TOKEN;
    process.env.CHAPPIE_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "CHAPPIE_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.CHAPPIE_GATEWAY_TOKEN = previous;
      }
    }
  });
