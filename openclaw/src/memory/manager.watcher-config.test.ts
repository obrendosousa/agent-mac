...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): ChappieConfig {
    const defaults: NonNullable<NonNullable<ChappieConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: ChappieConfig) {
