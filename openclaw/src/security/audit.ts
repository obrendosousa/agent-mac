...
import type { ConfigFileSnapshot, ChappieConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: ChappieConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Chappie state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Chappie state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.CHAPPIE_GATEWAY_TOKEN) || hasNonEmptyString(env.CHAPPIEDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.CHAPPIE_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.CHAPPIEDBOT_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("chappie security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.CHAPPIE_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.CHAPPIEDBOT_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: ChappieConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: ChappieConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: ChappieConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("chappie status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("chappie security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set CHAPPIE_GATEWAY_TOKEN/CHAPPIE_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("chappie security audit --deep")}".`,
    });
  }
