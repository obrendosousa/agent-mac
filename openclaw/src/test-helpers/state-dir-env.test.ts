...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    chappie: process.env.CHAPPIE_STATE_DIR,
    legacy: process.env.CHAPPIEDBOT_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.CHAPPIE_STATE_DIR).toBe(snapshot.chappie);
  expect(process.env.CHAPPIEDBOT_STATE_DIR).toBe(snapshot.legacy);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips CHAPPIE_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/chappie-state-dir-test");
    expect(process.env.CHAPPIE_STATE_DIR).toBe("/tmp/chappie-state-dir-test");
    expect(process.env.CHAPPIEDBOT_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("chappie-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.CHAPPIE_STATE_DIR).toBe(stateDir);
      expect(process.env.CHAPPIEDBOT_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("chappie-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.CHAPPIE_STATE_DIR = "/tmp/original-chappie";
    process.env.CHAPPIEDBOT_STATE_DIR = "/tmp/original-legacy";
    const prev = snapshotCurrentStateDirVars();
