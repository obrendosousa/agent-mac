...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set CHAPPIE_GATEWAY_TOKEN/CHAPPIE_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.CHAPPIE_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.CHAPPIE_GATEWAY_PASSWORD);
