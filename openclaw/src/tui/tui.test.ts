...
describe("resolveInitialTuiAgentId", () => {
  const cfg: ChappieConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/chappie" },
        { id: "ops", workspace: "/tmp/chappie/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/chappie/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run chappie devices list");
    expect(state.pairingHint).toContain("chappie devices list");
  });
