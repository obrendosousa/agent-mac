...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `chappie-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.CHAPPIEDBOT_GATEWAY_URL;
  });
  ...
  it("falls back to CHAPPIE_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers CHAPPIE_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "CHAPPIE_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-tui-file-secret-"));
