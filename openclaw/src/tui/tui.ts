...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run chappie devices list",
      pairingHint:
        "Pairing required. Run `chappie devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `chappie tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
