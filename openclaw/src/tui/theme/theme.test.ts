...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      CHAPPIE_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when CHAPPIE_THEME=light", async () => {
    const mod = await importThemeWithEnv({ CHAPPIE_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when CHAPPIE_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ CHAPPIE_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats CHAPPIE_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ CHAPPIE_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("CHAPPIE_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      CHAPPIE_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ CHAPPIE_THEME: "light" });
    const darkMod = await importThemeWithEnv({ CHAPPIE_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.CHAPPIE_THEME = "light";
