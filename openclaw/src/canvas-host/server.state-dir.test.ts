...
describe("canvas host state dir defaults", () => {
  it("uses CHAPPIE_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("chappie-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Chappie Canvas");
      } finally {
        await handler.close();
      }
    });
