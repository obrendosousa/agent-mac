 ...
 * A common mistake — caused by the similarity with the `chappie.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (chappie.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.CHAPPIE_AUTH_STORE_READONLY === "1";
