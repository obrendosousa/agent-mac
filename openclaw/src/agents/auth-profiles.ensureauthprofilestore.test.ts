...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-auth-merge-"));
    const previousAgentDir = process.env.CHAPPIE_AGENT_DIR;
    ...
    try {
      ...
      process.env.CHAPPIE_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        delete process.env.CHAPPIE_AGENT_DIR;
      } else {
        process.env.CHAPPIE_AGENT_DIR = previousAgentDir;
      }
  ...
  it("normalizes auth-profiles credential aliases with canonical-field precedence", () => {
    ...
    for (const testCase of cases) {
      const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-auth-alias-"));
      try {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-auth-legacy-alias-"));
    try {
      fs.writeFileSync(
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-auth-invalid-"));
