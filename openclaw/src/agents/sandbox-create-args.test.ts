...
import { CHAPPIE_CLI_ENV_VALUE } from "../infra/chappie-exec-env.js";
...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "chappie-sbx-test",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      labels: { "chappie.sandboxBrowser": "1" },
    });
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "chappie.sandbox=1",
        "--label",
        "chappie.sessionKey=main",
        "--label",
        "chappie.createdAtMs=1700000000000",
        "--label",
  ...
  it("preserves the Chappie exec marker when strict env sanitization is enabled", () => {
  ...
  it("blocks bind sources outside runtime allowlist roots", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "chappie-sbx-outside-roots",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "chappie-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("chappie-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "chappie-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "chappie-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
