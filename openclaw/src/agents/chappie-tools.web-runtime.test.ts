...
function findTool(name: string, config: ChappieConfig) {
  const allTools = createChappieTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: ChappieConfig; env?: NodeJS.ProcessEnv }) {
...
describe("chappie tools runtime web metadata wiring", () => {
