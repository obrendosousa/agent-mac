...
import { markChappieExecEnv } from "../../infra/chappie-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "chappie.sandbox=1");
  args.push("--label", `chappie.sessionKey=${params.scopeKey}`);
  args.push("--label", `chappie.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `chappie.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markChappieExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "chappie.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`chappie sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`chappie sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("chappie sandbox recreate --all");
}
