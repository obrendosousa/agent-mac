...
} from "./chappie-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.CHAPPIE_TEST_FAST;
...
describe("chappie-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.CHAPPIE_TEST_FAST = fastModeEnv.previous;
  });
