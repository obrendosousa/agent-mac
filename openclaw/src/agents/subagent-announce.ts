...
const FAST_TEST_MODE = process.env.CHAPPIE_TEST_FAST === "1";
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to Chappie sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`chappie ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for Chappie subagents (`runtime: "subagent"`).',
