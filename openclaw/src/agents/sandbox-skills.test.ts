...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "chappie-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.CHAPPIE_BUNDLED_SKILLS_DIR = bundledDir;
