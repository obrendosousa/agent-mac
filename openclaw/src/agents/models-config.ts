...
import { planChappieModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: ChappieConfig): {
...
export async function ensureChappieModelsJson(
  config?: ChappieConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveChappieAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planChappieModelsJson({
      cfg,
