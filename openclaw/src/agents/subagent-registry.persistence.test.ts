...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CHAPPIE_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-subagent-"));
  ...
  it("uses isolated temp state when CHAPPIE_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "chappie-test-state"));
  });
});
