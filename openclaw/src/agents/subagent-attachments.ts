...
function resolveAttachmentLimits(config: ChappieConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".chappie", "attachments");
  const relDir = path.posix.join(".chappie", "attachments", attachmentId);
