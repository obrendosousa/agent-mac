...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CHAPPIE_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CHAPPIE_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-bundled-override-"));
    process.env.CHAPPIE_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.CHAPPIE_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "chappie" }));
