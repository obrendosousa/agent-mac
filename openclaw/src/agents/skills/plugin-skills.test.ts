    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "chappie.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "chappie.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "chappie.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("chappie-");
  const acpxRoot = await tempDirs.make("chappie-acpx-plugin-");
  const helperRoot = await tempDirs.make("chappie-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("chappie-");
  const pluginRoot = await tempDirs.make("chappie-plugin-");
  const outsideDir = await tempDirs.make("chappie-outside-");
