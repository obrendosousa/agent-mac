...
function resolveSkillsLimits(config?: ChappieConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "chappie-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "chappie-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveChappieMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: ChappieConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`chappie skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
