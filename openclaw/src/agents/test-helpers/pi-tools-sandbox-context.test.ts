...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("chappie-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("chappie-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("chappie-sbx-");
