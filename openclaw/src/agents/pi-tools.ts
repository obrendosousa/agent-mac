...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending Chappie's
...
function resolveExecConfig(params: { cfg?: ChappieConfig; agentId?: string }) {
...
export function createChappieCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createChappieReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
