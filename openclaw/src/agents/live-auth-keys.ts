...
const GOOGLE_LIVE_SINGLE_KEY = "CHAPPIE_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CHAPPIE_LIVE_ANTHROPIC_KEY",
    listVar: "CHAPPIE_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CHAPPIE_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CHAPPIE_LIVE_${base}_KEY`;
