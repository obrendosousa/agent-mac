...
import { resolveChappieDocsPath } from "../../docs-path.js";
...
import { createChappieCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "chappie.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "chappie.sessions_yield";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveChappieAgentDir();
    ...
    const docsPath = await resolveChappieDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/chappie/chappie/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("chappie:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/chappie/chappie/issues/8643
