...
vi.mock("../../docs-path.js", () => ({
  resolveChappieDocsPath: async () => undefined,
}));
...
describe("runEmbeddedAttempt sessions_spawn workspace inheritance", () => {
  ...
  it("passes the real workspace to sessions_spawn when workspaceAccess is ro", async () => {
    const realWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-real-workspace-"));
    const sandboxWorkspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "chappie-sandbox-workspace-"),
    );
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-agent-dir-"));
...
describe("runEmbeddedAttempt cache-ttl tracking after compaction", () => {
  ...
  async function runAttemptWithCacheTtl(compactionCount: number) {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-cache-ttl-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-cache-ttl-agent-"));
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  async function runAttemptWithContextEngine(contextEngine: {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-ctx-engine-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-ctx-engine-agent-"));
    const sessionFile = path.join(workspaceDir, "session.jsonl");
    tempPaths.push(workspaceDir, agentDir);
    await fs.writeFile(sessionFile, "", "utf8");
