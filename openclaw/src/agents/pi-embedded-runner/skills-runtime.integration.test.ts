...
const originalBundledDir = process.env.CHAPPIE_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("chappie-bundled-");
  const workspaceDir = await createTempDir("chappie-workspace-");
...
afterEach(async () => {
  process.env.CHAPPIE_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.CHAPPIE_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
