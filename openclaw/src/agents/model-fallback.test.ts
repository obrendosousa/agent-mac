...
function makeFallbacksOnlyCfg(): ChappieConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): ChappieConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-auth-"));
...
// https://github.com/chappie/chappie/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal Chappie compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
