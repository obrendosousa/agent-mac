...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CHAPPIE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CHAPPIE_HOME: "/srv/chappie-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/chappie-home"), ".chappie", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".chappie", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("chappie-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-workspace-hardlink-"));
    try {
