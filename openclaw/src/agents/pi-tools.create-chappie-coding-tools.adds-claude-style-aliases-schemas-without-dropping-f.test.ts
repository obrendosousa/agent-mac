...
describe("createChappieCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-alias-"));
    try {
      const tools = createChappieCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-structured-write-"));
    try {
      const tools = createChappieCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-structured-edit-"));
    try {
