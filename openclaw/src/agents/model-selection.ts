...
export function isCliProvider(provider: string, cfg?: ChappieConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: ChappieConfig; agentId?: string }): string[] {
  if (params.agentId) {
