...
test("exec sets CHAPPIE_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-chappie-shell", {
    command: "node -e \"process.stdout.write(process.env.CHAPPIE_SHELL || '')\"",
    pty: true,
  });
