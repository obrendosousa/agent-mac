...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const chappieTmpDir = path.resolve(resolvePreferredChappieTmpDir());
  if (!isPathInside(chappieTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: chappieTmpDir });
  return resolved;
}
