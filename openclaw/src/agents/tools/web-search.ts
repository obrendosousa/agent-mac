...
type WebSearchConfig = NonNullable<ChappieConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ChappieConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("chappie configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
