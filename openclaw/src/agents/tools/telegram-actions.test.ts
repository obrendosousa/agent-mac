...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ChappieConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ChappieConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ChappieConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ChappieConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ChappieConfig, accountId = "media") {
    await handleTelegramAction(
