...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    chappie: process.env.CHAPPIE_GATEWAY_TOKEN,
    chappiedbot: process.env.CHAPPIEDBOT_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.chappie === undefined) {
      delete process.env.CHAPPIE_GATEWAY_TOKEN;
    } else {
      process.env.CHAPPIE_GATEWAY_TOKEN = envSnapshot.chappie;
    }
    if (envSnapshot.chappiedbot === undefined) {
      delete process.env.CHAPPIEDBOT_GATEWAY_TOKEN;
    } else {
      process.env.CHAPPIEDBOT_GATEWAY_TOKEN = envSnapshot.chappiedbot;
    }
  });
  ...
  it("uses CHAPPIE_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CHAPPIE_GATEWAY_TOKEN = "local-env-token";
    process.env.CHAPPIEDBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
