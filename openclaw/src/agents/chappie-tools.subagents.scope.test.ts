...
} from "./chappie-tools.subagents.test-harness.js";
...
describe("chappie-tools: subagents scope isolation", () => {
  let storePath = "";
  ...
  beforeEach(() => {
    ...
    storePath = path.join(
      os.tmpdir(),
      `chappie-subagents-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
