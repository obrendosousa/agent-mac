...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CHAPPIE_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".chappie", `workspace-${profile}`);
  }
  return path.join(home, ".chappie", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".chappie";
