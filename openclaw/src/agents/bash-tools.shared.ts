...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via CHAPPIE_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `CHAPPIE_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${CHAPPIE_PREPEND_PATH}:$PATH"; unset CHAPPIE_PREPEND_PATH; '
    : "";
