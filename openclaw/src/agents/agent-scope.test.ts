...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: ChappieConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/chappie" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ChappieConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ChappieConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ChappieConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: ChappieConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: ChappieConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ChappieConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: ChappieConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/chappie");
  });

  it("uses CHAPPIE_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "chappie-home");
    vi.stubEnv("CHAPPIE_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as ChappieConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".chappie", "workspace"));
  });

  it("uses CHAPPIE_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "chappie-home");
    vi.stubEnv("CHAPPIE_HOME", home);
    // Clear state dir so it falls back to CHAPPIE_HOME
    vi.stubEnv("CHAPPIE_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as ChappieConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".chappie", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/chappie-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/chappie-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-agent-scope-"));
