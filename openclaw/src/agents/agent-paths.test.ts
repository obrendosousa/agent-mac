...
describe("resolveChappieAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          CHAPPIE_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveChappieAgentDir();
  ...
  it("honors CHAPPIE_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          CHAPPIE_STATE_DIR: undefined,
          CHAPPIE_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveChappieAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when CHAPPIE_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers CHAPPIE_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          CHAPPIE_STATE_DIR: undefined,
          CHAPPIE_AGENT_DIR: primaryOverride,
