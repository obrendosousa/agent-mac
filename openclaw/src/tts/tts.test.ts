...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): ChappieConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.CHAPPIE_TTS_PREFS;
      process.env.CHAPPIE_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      const originalFetch = globalThis.fetch;
      const fetchMock = vi.fn(async () => ({
        ok: true,
      ...
      try {
        ...
        globalThis.fetch = originalFetch;
        process.env.CHAPPIE_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: ChappieConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
