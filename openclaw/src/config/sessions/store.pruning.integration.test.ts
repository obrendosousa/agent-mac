...
// Keep integration tests deterministic: never read a real chappie.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.CHAPPIE_SESSION_CACHE_TTL_MS;
    process.env.CHAPPIE_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.CHAPPIE_SESSION_CACHE_TTL_MS;
    } else {
      process.env.CHAPPIE_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");
