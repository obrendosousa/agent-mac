...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/chappie/agents/main/sessions";
  ...
  it("falls back to derived path when sessionFile is outside known agent sessions dirs", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/chappie/agents/work/not-sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts symlink-alias session paths that resolve under the sessions dir", () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-symlink-session-"));
  ...
  it("falls back when sessionFile is a symlink that escapes sessions dir", () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-symlink-escape-"));
...
describe("session store lock (Promise chain mutex)", () => {
  ...
  beforeAll(async () => {
    lockFixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "chappie-lock-test-"));
  });
