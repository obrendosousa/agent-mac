...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): ChappieConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: ChappieConfig, env: NodeJS.ProcessEnv) =>
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".chappie/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".chappie/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });
