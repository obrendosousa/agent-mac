...
function isWhatsAppConfigured(cfg: ChappieConfig): boolean {
...
function isGenericChannelConfigured(cfg: ChappieConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: ChappieConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ChappieConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: ChappieConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: ChappieConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ChappieConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: ChappieConfig, pluginId: string): ChappieConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
