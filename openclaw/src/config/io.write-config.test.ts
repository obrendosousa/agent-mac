...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".chappie", "chappie.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".chappie", "logs", "config-audit.jsonl");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        const stateDir = path.join(home, ".chappie");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: ChappieConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "chappie config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'chappie config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          CHAPPIE_WATCH_MODE: "1",
          CHAPPIE_WATCH_SESSION: "watch-session-1",
          CHAPPIE_WATCH_COMMAND: "gateway --force",
