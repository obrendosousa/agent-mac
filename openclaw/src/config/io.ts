...
import type { ChappieConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareChappieVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  chappie config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  chappie config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: ChappieConfig,
  pathSegments: string[],
): { changed: boolean; next: ChappieConfig } {
...
function coerceConfig(value: unknown): ChappieConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ChappieConfig;
}
...
function stampConfigVersion(cfg: ChappieConfig): ChappieConfig {
...
function warnIfConfigFromFuture(cfg: ChappieConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareChappieVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Chappie (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ChappieConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ChappieConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ChappieConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ChappieConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ChappieConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CHAPPIE_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CHAPPIE_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.CHAPPIE_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CHAPPIE_WATCH_SESSION === "string" &&
        deps.env.CHAPPIE_WATCH_SESSION.trim().length > 0
          ? deps.env.CHAPPIE_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CHAPPIE_WATCH_COMMAND === "string" &&
        deps.env.CHAPPIE_WATCH_COMMAND.trim().length > 0
          ? deps.env.CHAPPIE_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CHAPPIE_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ChappieConfig | null = null;
let runtimeConfigSourceSnapshot: ChappieConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CHAPPIE_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CHAPPIE_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): ChappieConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): ChappieConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: ChappieConfig;
  candidate: ChappieConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: ChappieConfig): ChappieConfig {
...
export function loadConfig(): ChappieConfig {
...
export async function readBestEffortConfig(): Promise<ChappieConfig> {
...
export async function writeConfigFile(
  ...
  // edits to chappie.json remain visible (no stale snapshot).
}
