...
function collectConfigEnvVarsByTarget(cfg?: ChappieConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: ChappieConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: ChappieConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: ChappieConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CHAPPIE_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
