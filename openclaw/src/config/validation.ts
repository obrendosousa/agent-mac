...
import type { ChappieConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ChappieConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: ChappieConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ChappieConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ChappieSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ChappieConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ChappieConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as ChappieConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as ChappieConfig,
  };
}
