...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("chappie-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "chappie-sandbox-browser-global" },
      agentBrowser: { network: "chappie-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("chappie-sandbox-browser-agent");
  });
