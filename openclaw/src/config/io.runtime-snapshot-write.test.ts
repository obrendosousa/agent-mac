...
function createSourceConfig(): ChappieConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): ChappieConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("chappie-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("chappie-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: ChappieConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("chappie-config-runtime-write-", async (home) => {
      const configPath = path.join(home, ".chappie", "chappie.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("chappie-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".chappie", "chappie.json");
      const sourceConfig: ChappieConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: ChappieConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("chappie-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".chappie", "chappie.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: ChappieConfig = {
