...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CHAPPIE_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-chappie",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-chappie#general");
  });
  ...
  it("derives session transcripts dir from CHAPPIE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CHAPPIE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CHAPPIE_STATE_DIR differs from stored paths", () => {
