...
const ETC_CHAPPIE_DIR = path.join(ROOT_DIR, "etc", "chappie");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "chappie.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcChappiePath(...parts: string[]) {
  return path.join(ETC_CHAPPIE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcChappiePath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "chappie.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-includes-hardlink-"));
      try {
    ...
    it("rejects oversized include files", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-includes-big-"));
      try {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "chappie.json")),
