...
import type { ConfigFileSnapshot } from "./types.chappie.js";
import { ChappieSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(ChappieSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.chappie/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/chappie/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/chappie/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/chappie/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/chappie/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ChappieSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ChappieConfig";
    const hints = mainSchemaHints;
