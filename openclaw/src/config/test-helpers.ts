...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "chappie-config-" });
}

export async function writeChappieConfig(home: string, config: unknown): Promise<string> {
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeChappieConfig(home, config);
