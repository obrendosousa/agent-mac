...
import { buildWebSearchProviderConfig, withTempHome, writeChappieConfig } from "./test-helpers.js";
import { ChappieSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = ChappieSchema.safeParse({
      $schema: "https://chappie.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://chappie.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = ChappieSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = ChappieSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeChappieConfig(home, {
        gateway: { bind: "${CHAPPIE_BIND}" },
      });

      const prev = process.env.CHAPPIE_BIND;
      process.env.CHAPPIE_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.CHAPPIE_BIND;
        } else {
          process.env.CHAPPIE_BIND = prev;
        }
      }
    });
  });
