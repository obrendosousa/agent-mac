...
const TEMP_MEDIA_PREFIX = "chappie-media-";
...
function createGroqAudioConfig(): ChappieConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ChappieConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ChappieConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ChappieConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredChappieTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        CHAPPIE_AGENT_DIR: isolatedAgentDir,
