...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("chappie-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("chappie-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("chappie-lid-a-", async (first) => {
      await withTempDir("chappie-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.chappie when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "chappie-config-dir-"));
    try {
  ...
  it("expands CHAPPIE_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/chappie-home",
      CHAPPIE_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/chappie-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers CHAPPIE_HOME over HOME", () => {
    vi.stubEnv("CHAPPIE_HOME", "/srv/chappie-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/chappie-home"));
...
describe("shortenHomePath", () => {
  it("uses $CHAPPIE_HOME prefix when CHAPPIE_HOME is set", () => {
    vi.stubEnv("CHAPPIE_HOME", "/srv/chappie-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/chappie-home")}/.chappie/chappie.json`)).toBe(
      "$CHAPPIE_HOME/.chappie/chappie.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $CHAPPIE_HOME replacement when CHAPPIE_HOME is set", () => {
    vi.stubEnv("CHAPPIE_HOME", "/srv/chappie-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/chappie-home")}/.chappie/chappie.json`),
    ).toBe("config: $CHAPPIE_HOME/.chappie/chappie.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/chappie", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "chappie"),
    );
  });
  ...
  it("prefers CHAPPIE_HOME for tilde expansion", () => {
    vi.stubEnv("CHAPPIE_HOME", "/srv/chappie-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/chappie")).toBe(path.resolve("/srv/chappie-home", "chappie"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/chappie", env)).toBe(path.resolve("/srv/chappie-home", "chappie"));
  });
