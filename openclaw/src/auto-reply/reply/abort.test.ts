...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop chappie!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@chappie_bot", { botUsername: "chappie_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@chappie_bot", { botUsername: "chappie_bot" })).toBe(true);
