...
const SLACK_REPLY_BUTTON_ACTION_ID = "chappie:reply_button";
const SLACK_REPLY_SELECT_ACTION_ID = "chappie:reply_select";
...
function buildButtonsBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `chappie_reply_buttons_${index}`,
...
function buildSelectBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `chappie_reply_select_${index}`,
    elements: [
      {
        type: "static_select",
