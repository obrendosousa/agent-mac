...
function createDiscordParams(commandBody: string, cfg: ChappieConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: ChappieConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

function createTelegramTopicParams(commandBody: string, cfg: ChappieConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
...
function createTelegramDmParams(commandBody: string, cfg: ChappieConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
    OriginatingChannel: "telegram",
...
async function runDiscordAcpCommand(commandBody: string, cfg: ChappieConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: ChappieConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: ChappieConfig = baseCfg) {
  return handleAcpCommand(createTelegramTopicParams(commandBody, cfg), true);
}

async function runTelegramDmAcpCommand(commandBody: string, cfg: ChappieConfig = baseCfg) {
