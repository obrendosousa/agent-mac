...
function withAcpConfig(acp: ChappieConfig["acp"]): ChappieConfig {
  return { acp } as ChappieConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm chappie plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm chappie plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toContain("chappie plugins install ");
  ...
  it("falls back to npm install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("chappie plugins install acpx");
  });
