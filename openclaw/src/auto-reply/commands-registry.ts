...
export function isCommandEnabled(cfg: ChappieConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ChappieConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ChappieConfig),
...
export function getCommandDetection(_cfg?: ChappieConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ChappieConfig) {
