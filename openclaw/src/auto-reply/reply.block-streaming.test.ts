...
function createReplyConfig(home: string, streamMode?: "block"): ChappieConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("chappie-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("CHAPPIE_TEST_FAST", "1");
  ...
  it("still parses media directives for direct block payloads", async () => {
    await withTempHome(async (home) => {
      ...
      expect(onBlockReply.mock.calls[0][0]).toMatchObject({
        text: "Result",
        mediaUrls: [path.join(home, "chappie", "image.png")],
      });
    });
  });
});
