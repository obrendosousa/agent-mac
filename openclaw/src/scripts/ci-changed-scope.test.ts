...
describe("detectChangedScope", () => {
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(detectChangedScope(["apps/shared/ChappieKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: true,
      runWindows: false,
  ...
  it("does not force macOS for generated protocol model-only changes", () => {
    expect(detectChangedScope(["apps/macos/Sources/ChappieProtocol/GatewayModels.swift"])).toEqual(
      {
        runNode: false,
        runMacos: false,
        runAndroid: false,
  ...
  it("treats base and head as literal git args", () => {
    const markerPath = path.join(
      os.tmpdir(),
      `chappie-ci-changed-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`,
    );
