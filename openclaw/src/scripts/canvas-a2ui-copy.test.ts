...
describe("canvas a2ui copy", () => {
  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-a2ui-"));
    try {
      await run(dir);
    } finally {
  ...
  it("skips missing assets when CHAPPIE_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.CHAPPIE_A2UI_SKIP_MISSING;
      process.env.CHAPPIE_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.CHAPPIE_A2UI_SKIP_MISSING;
        } else {
          process.env.CHAPPIE_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
