...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".chappie");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CHAPPIE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CHAPPIE_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CHAPPIE_STATE_DIR.
