...
vi.mock("./paths.js", () => {
  return {
    DEFAULT_UPLOAD_DIR: "/tmp/chappie/uploads",
...
describe("setInputFilesViaPlaywright", () => {
  ...
  beforeEach(() => {
    ...
    resolveStrictExistingPathsWithinRoot.mockResolvedValue({
      ok: true,
      paths: ["/private/tmp/chappie/uploads/ok.txt"],
    });
  });
  ...
  it("revalidates upload paths and uses resolved canonical paths for inputRef", async () => {
    ...
    await setInputFilesViaPlaywright({
      ...
      targetId: "T1",
      inputRef: "e7",
      paths: ["/tmp/chappie/uploads/ok.txt"],
    });

    expect(resolveStrictExistingPathsWithinRoot).toHaveBeenCalledWith({
      rootDir: "/tmp/chappie/uploads",
      requestedPaths: ["/tmp/chappie/uploads/ok.txt"],
      scopeLabel: "uploads directory (/tmp/chappie/uploads)",
    });
    expect(refLocator).toHaveBeenCalledWith(page, "e7");
    expect(setInputFiles).toHaveBeenCalledWith(["/private/tmp/chappie/uploads/ok.txt"]);
  });
  ...
  it("throws and skips setInputFiles when use-time validation fails", async () => {
    ...
    await expect(
      setInputFilesViaPlaywright({
        ...
        paths: ["/tmp/chappie/uploads/missing.txt"],
      }),
