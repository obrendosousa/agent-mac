...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".chappie-profile-decorated");
}
 ...
 */
export function decorateChappieProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CHAPPIE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CHAPPIE_BROWSER_COLOR).toUpperCase();
