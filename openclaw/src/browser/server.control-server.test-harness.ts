...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CHAPPIE_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "chappie",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/chappie" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    chappie: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchChappieChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "chappie";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.CHAPPIE_GATEWAY_PORT;
  process.env.CHAPPIE_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CHAPPIE_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CHAPPIE_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CHAPPIE_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CHAPPIE_GATEWAY_PASSWORD;
  } else {
    process.env.CHAPPIE_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
