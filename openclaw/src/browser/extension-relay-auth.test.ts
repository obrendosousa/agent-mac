...
import {
  probeAuthenticatedChappieRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedChappieRelay({
    baseUrl,
    relayAuthHeader: "x-chappie-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated chappie relay probe responses", async () => {
