...
describe("browser server-context existing-session profile", () => {
  it("routes tab operations through the Chrome MCP backend", async () => {
    ...
    vi.mocked(chromeMcp.listChromeMcpTabs)
      ...
      .mockResolvedValueOnce([
        { targetId: "8", title: "", url: "https://chappie.ai", type: "page" },
      ])
      .mockResolvedValueOnce([
        { targetId: "8", title: "", url: "https://chappie.ai", type: "page" },
      ])
    ...
    const opened = await live.openTab("https://chappie.ai");
    ...
    expect(chromeMcp.openChromeMcpTab).toHaveBeenCalledWith("chrome-live", "https://chappie.ai");
