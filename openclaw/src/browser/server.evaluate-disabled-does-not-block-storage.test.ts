  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          chappie: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CHAPPIE_GATEWAY_PORT;
    process.env.CHAPPIE_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CHAPPIE_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CHAPPIE_GATEWAY_PASSWORD;
