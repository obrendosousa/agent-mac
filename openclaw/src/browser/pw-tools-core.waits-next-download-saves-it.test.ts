...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredChappieTmpDir: vi.fn(() => "/tmp/chappie"),
}));
vi.mock("../infra/tmp-chappie-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredChappieTmpDir.mockReturnValue("/tmp/chappie");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".chappie-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredChappieTmpDir.mockReturnValue("/tmp/chappie-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "chappie-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "chappie-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredChappieTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "chappie-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "chappie-preferred", "downloads")}${path.sep}`),
    );
  });
