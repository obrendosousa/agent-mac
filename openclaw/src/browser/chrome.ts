...
export function resolveChappieUserDataDir(profileName = DEFAULT_CHAPPIE_BROWSER_PROFILE_NAME) {
...
export async function launchChappieChrome(
  ...
  const userDataDir = resolveChappieUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CHAPPIE_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateChappieProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 chappie browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`chappie browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`chappie browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 chappie browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopChappieChrome(
