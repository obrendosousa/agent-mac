...
const RELAY_AUTH_HEADER = "x-chappie-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Chappie-Extension-Relay",
            revision: "0",
            userAgent: "Chappie-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Chappie/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedChappieRelay({
