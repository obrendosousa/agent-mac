...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "CHAPPIE_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.CHAPPIE_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.CHAPPIE_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-chappie-relay-token");
    expect(headers["x-chappie-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${sharedUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-chappie-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(sharedUrl)["x-chappie-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${sharedUrl}/json/version`, {
      headers: { "x-chappie-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-chappie-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "Chappie/extension-relay" }));
        return;
      }
  ...
  it(
    ...
    async () => {
      process.env.CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.CHAPPIE_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
  ...
  it("does not swallow EADDRINUSE when occupied port is not an chappie relay", async () => {
    const port = await getFreePort();
