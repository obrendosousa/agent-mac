...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("chappie");
    expect(profile?.driver).toBe("chappie");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const chappie = resolveProfile(resolved, "chappie");
    expect(chappie?.driver).toBe("chappie");
    expect(chappie?.cdpPort).toBe(18800);
    expect(chappie?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from CHAPPIE_GATEWAY_PORT when unset", () => {
    withEnv({ CHAPPIE_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const chappie = resolveProfile(resolved, "chappie");
      expect(chappie?.cdpPort).toBe(19012);
      expect(chappie?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ CHAPPIE_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const chappie = resolveProfile(resolved, "chappie");
      expect(chappie?.cdpPort).toBe(19022);
      expect(chappie?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(chappie?.cdpPort).toBe(19000);
    expect(chappie?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "chappie");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "chappie")!;
  ...
  describe("default profile preference", () => {
    it("defaults to chappie profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("chappie");
    });

    it("keeps chappie default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("chappie");
    ...
    it("keeps chappie default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("chappie");
    ...
    it("keeps chappie default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
