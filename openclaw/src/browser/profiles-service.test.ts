...
vi.mock("./chrome.js", () => ({
  resolveChappieUserDataDir: vi.fn(() => "/tmp/chappie-test/chappie/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "chappie-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveChappieUserDataDir).mockReturnValue(userDataDir);
