...
describe("acp prompt cwd prefix", () => {
  ...
  async function runPromptAndCaptureRequest(
    ...
    sessionStore.createSession({
      ...
      cwd: options.cwd ?? path.join(os.homedir(), "chappie-test"),
    });
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "chappie-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]chappie-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\chappie-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\chappie-test]"),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        systemProvenanceReceipt: expect.stringContaining("bridge=chappie-acp"),
      }),
      { expectFinal: true },
    );
    expect(requestSpy).toHaveBeenCalledWith(
