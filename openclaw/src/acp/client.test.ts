...
const createTempDir = () => tempDirs.make("chappie-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets CHAPPIE_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "chappie",
    });

    expect(env.CHAPPIE_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("chappie");
  });

  it("overrides pre-existing CHAPPIE_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      CHAPPIE_SHELL: "wrong",
    });
    expect(env.CHAPPIE_SHELL).toBe("acp-client");
  ...
  it("preserves CHAPPIE_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        CHAPPIE_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["CHAPPIE_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Chappie bridge", () => {
    ...
    expect(env.CHAPPIE_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.CHAPPIE_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "chappie",
        serverArgs: ["acp"],
        defaultServerCommand: "chappie",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("CHAPPIE_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "chappie", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "chappie", "dist", "entry.js");
    const shimPath = path.join(dir, "chappie.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\chappie\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/chappie-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/chappie-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/chappie-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
