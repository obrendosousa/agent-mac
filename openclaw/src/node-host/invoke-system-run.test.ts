...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-exec-approvals-"));
    const previousChappieHome = process.env.CHAPPIE_HOME;
    process.env.CHAPPIE_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousChappieHome === undefined) {
        delete process.env.CHAPPIE_HOME;
      } else {
        process.env.CHAPPIE_HOME = previousChappieHome;
      }
  ...
  async function withFakeRuntimeOnPath<T>(params: {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `chappie-${params.runtime}-path-`));
    const binDir = path.join(tmp, "bin");
    fs.mkdirSync(binDir, { recursive: true });
    const runtimePath =
  ...
  for (const testCase of approvedEnvShellWrapperCases) {
    it.runIf(process.platform !== "win32")(testCase.name, async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approved-wrapper-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "chappie-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "chappie-prepare-run-path-pin-",
        run: async ({ expected }) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "chappie-allowlist-path-pin-",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-cwd-link-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-cwd-parent-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-cwd-real-"));
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-cwd-drift-"));
    const fallback = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-cwd-drift-alt-"));
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("denies approval-based execution when a script operand changes after approval", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-script-drift-"));
  ...
  it("keeps approved shell script execution working when the script is unchanged", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-script-stable-"));
    const fixture = createMutableScriptOperandFixture(tmp);
    fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
    if (process.platform !== "win32") {
      fs.chmodSync(fixture.scriptPath, 0o755);
  ...
  for (const runtime of ["bun", "deno", "tsx", "jiti"] as const) {
      ...
      await withFakeRuntimeOnPath({
        ...
        run: async () => {
          const tmp = fs.mkdtempSync(
            path.join(os.tmpdir(), `chappie-approval-${runtime}-script-drift-`),
          );
    ...
    it(`keeps approved ${runtime} script execution working when the script is unchanged`, async () => {
      await withFakeRuntimeOnPath({
        ...
        run: async () => {
          const tmp = fs.mkdtempSync(
            path.join(os.tmpdir(), `chappie-approval-${runtime}-script-stable-`),
          );
          const fixture = createRuntimeScriptOperandFixture({ tmp, runtime });
          fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
          try {
  ...
  it("denies approval-based execution when tsx is missing a required mutable script binding", async () => {
    await withFakeRuntimeOnPath({
      ...
      run: async () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "chappie-approval-tsx-missing-binding-"),
        );
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `chappie-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["chappie status; id", "chappie status; cat /etc/passwd"];
