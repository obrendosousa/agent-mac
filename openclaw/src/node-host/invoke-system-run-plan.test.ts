...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `chappie-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "chappie-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "chappie-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "chappie-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chappie-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chappie-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chappie-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "chappie-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-approval-hardening-"));
      const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "chappie-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chappie-shell-option-value-"));
    try {
