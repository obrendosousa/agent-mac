...
type DispatchCronDeliveryParams = {
  cfg: ChappieConfig;
  cfgWithAgentDefaults: ChappieConfig;
  deps: CliDeps;
  job: CronJob;
  agentId: string;
...
function pruneCompletedDirectCronDeliveries(now: number) {
  const ttlMs = process.env.CHAPPIE_TEST_FAST === "1" ? 60_000 : 24 * 60 * 60 * 1000;
...
function resolveDirectCronRetryDelaysMs(): readonly number[] {
  return process.env.NODE_ENV === "test" && process.env.CHAPPIE_TEST_FAST === "1"
    ? [8, 16, 32]
    : [5_000, 10_000, 20_000];
}
    ...
    try {
      ...
      const runDelivery = async () =>
        await deliverOutboundPayloads({
          ...
          // See: https://github.com/chappie/chappie/issues/40545
          skipQueue: true,
        });
