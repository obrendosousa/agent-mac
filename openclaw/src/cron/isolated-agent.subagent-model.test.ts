...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "chappie-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".chappie", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<ChappieConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<ChappieConfig>,
