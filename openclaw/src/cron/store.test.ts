...
const { makeStorePath } = createCronStoreHarness({ prefix: "chappie-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses CHAPPIE_HOME for tilde expansion", () => {
    vi.stubEnv("CHAPPIE_HOME", "/srv/chappie-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/chappie-home", "cron", "jobs.json"));
  });
});
