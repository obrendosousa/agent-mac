...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    await withRunLogDir("chappie-cron-log-", async (dir) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("chappie-cron-log-perms-", async (dir) => {
        const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("chappie-cron-log-dir-perms-", async (dir) => {
  ...
  it("reads newest entries and filters by jobId", async () => {
    await withRunLogDir("chappie-cron-log-read-", async (dir) => {
  ...
  it("ignores invalid and non-finished lines while preserving delivery fields", async () => {
    await withRunLogDir("chappie-cron-log-filter-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("reads telemetry fields", async () => {
    await withRunLogDir("chappie-cron-log-telemetry-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("cleans up pending-write bookkeeping after appends complete", async () => {
    await withRunLogDir("chappie-cron-log-pending-", async (dir) => {
  ...
  it("read drains pending fire-and-forget writes", async () => {
    await withRunLogDir("chappie-cron-log-drain-", async (dir) => {
