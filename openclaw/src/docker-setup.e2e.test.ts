...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "chappie-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    CHAPPIE_GATEWAY_TOKEN: "test-token",
    CHAPPIE_CONFIG_DIR: join(sandbox.rootDir, "config"),
    CHAPPIE_WORKSPACE_DIR: join(sandbox.rootDir, "chappie"),
  };
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    CHAPPIE_GATEWAY_TOKEN: undefined,
    CHAPPIE_CONFIG_DIR: configDir,
    CHAPPIE_WORKSPACE_DIR: workspaceDir,
  });
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      CHAPPIE_EXTRA_MOUNTS: undefined,
      CHAPPIE_HOME_VOLUME: "chappie-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAPPIE_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("CHAPPIE_EXTRA_MOUNTS=");
    expect(envFile).toContain("CHAPPIE_HOME_VOLUME=chappie-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("chappie-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("chappie-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg CHAPPIE_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm chappie-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm chappie-cli config set gateway.mode local");
    expect(log).toContain("run --rm chappie-cli config set gateway.bind lan");
  });
  ...
  it("writes CHAPPIE_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAPPIE_TZ=Asia/Shanghai");
  });
  ...
  it("reuses existing config token when CHAPPIE_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAPPIE_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when CHAPPIE_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "CHAPPIE_GATEWAY_TOKEN=dotenv-token-123\nCHAPPIE_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAPPIE_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "CHAPPIE_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAPPIE_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("CHAPPIE_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats CHAPPIE_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAPPIE_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg CHAPPIE_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg CHAPPIE_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker chappie-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        CHAPPIE_SANDBOX: "1",
        CHAPPIE_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("chappie-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps chappie-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate chappie-gateway"));
  ...
  it("rejects injected multiline CHAPPIE_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAPPIE_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid CHAPPIE_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid CHAPPIE_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAPPIE_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAPPIE_HOME_VOLUME must match");
  });

  it("rejects CHAPPIE_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAPPIE_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:chappie-gateway"');
    expect(compose).toContain("depends_on:\n      - chappie-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/CHAPPIE_GATEWAY_TOKEN: \$\{CHAPPIE_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{CHAPPIE_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
