...
const CLI_LIVE = isTruthyEnvValue(process.env.CHAPPIE_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.CHAPPIE_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.CHAPPIE_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("CHAPPIE_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.CHAPPIE_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `CHAPPIE_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.CHAPPIE_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `CHAPPIE_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "CHAPPIE_LIVE_CLI_BACKEND_ARGS",
        process.env.CHAPPIE_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`CHAPPIE_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "CHAPPIE_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.CHAPPIE_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.CHAPPIE_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.CHAPPIE_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "CHAPPIE_LIVE_CLI_BACKEND_IMAGE_MODE requires CHAPPIE_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-live-cli-"));
    const disableMcpConfig = process.env.CHAPPIE_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        delete process.env.CHAPPIE_CONFIG_PATH;
      } else {
