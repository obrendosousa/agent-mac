...
describe("gateway pre-auth hardening", () => {
  it("closes idle unauthenticated sockets after the handshake timeout", async () => {
    const previous = process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS;
    process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS = "200";
    cleanupEnv.push(() => {
      if (previous === undefined) {
        delete process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS = previous;
      }
    });
