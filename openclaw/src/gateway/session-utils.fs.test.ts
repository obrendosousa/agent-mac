...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("chappie-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __chappie?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__chappie?.kind).toBe("compaction");
    expect(marker.__chappie?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("chappie-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses CHAPPIE_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/chappie-home"), ".chappie", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile for standard and custom store roots", () => {
    const cases = [
      {
        storePath: "/tmp/chappie/agents/main/sessions/sessions.json",
        sessionFile: "/tmp/chappie/agents/ops/sessions/sess-safe.jsonl",
      },
      {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/chappie/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/chappie/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("chappie-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });

  beforeAll(() => {
    vi.stubEnv("CHAPPIE_HOME", tmpDir);
  });
