...
import { createChappieTools } from "../agents/chappie-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-chappie-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-chappie-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-chappie-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-chappie-thread-id")?.trim() || undefined;
  ...
  const allTools = createChappieTools({
