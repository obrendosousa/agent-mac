...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gw-"));
    try {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gw-"));
