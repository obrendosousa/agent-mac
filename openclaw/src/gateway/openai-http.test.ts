    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "chappie", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-chappie-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-chappie-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "chappie", messages: [{ role: "user", content: "hi" }] },
          {
            "x-chappie-agent-id": "beta",
            "x-chappie-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Chappie.");
      }
