...
import type { ChappieConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: ChappieConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: ChappieConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ChappieConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ChappieConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ChappieConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: ChappieConfig) => {
