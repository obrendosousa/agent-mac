...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CHAPPIE_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            CHAPPIE_VERSION: " ",
            CHAPPIE_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `chappie-test-device-${randomUUID()}.json`),
        nonce,
      });
