...
import { type ChappieConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: ChappieConfig): string[] {
...
export function listAgentsForGateway(cfg: ChappieConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: ChappieConfig): string {
...
function resolveSessionStoreAgentId(cfg: ChappieConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: ChappieConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: ChappieConfig): GatewaySessionsDefaults {
