...
let lastCreateChappieToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/chappie-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateChappieToolsContext?.agentTo,
          agentThreadId: lastCreateChappieToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createChappieTools: (ctx: Record<string, unknown>) => {
      lastCreateChappieToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateChappieToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateChappieToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-chappie-message-to": "channel:24514",
        "x-chappie-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
