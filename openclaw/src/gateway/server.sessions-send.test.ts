...
const { createChappieTools } = await import("../agents/chappie-tools.js");
...
type SessionSendTool = ReturnType<typeof createChappieTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createChappieTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CHAPPIE_GATEWAY_PORT", "CHAPPIE_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.CHAPPIE_GATEWAY_PORT = String(gatewayPort);
  process.env.CHAPPIE_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("CHAPPIE_CONFIG_PATH missing in gateway test environment");
      }
