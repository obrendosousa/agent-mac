...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "chappie");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = nextGatewayId("nonce-a");
      const nonceB = nextGatewayId("nonce-b");
      const toolProbePath = path.join(workspaceDir, `.chappie-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".chappie");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-wizard-home-"));
      process.env.HOME = tempHome;
