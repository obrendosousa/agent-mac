...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "CHAPPIE_STATE_DIR",
  "CHAPPIE_CONFIG_PATH",
  "CHAPPIE_SKIP_BROWSER_CONTROL_SERVER",
  "CHAPPIE_SKIP_GMAIL_WATCHER",
  "CHAPPIE_SKIP_CANVAS_HOST",
  "CHAPPIE_BUNDLED_PLUGINS_DIR",
  "CHAPPIE_SKIP_CHANNELS",
  "CHAPPIE_SKIP_PROVIDERS",
  "CHAPPIE_SKIP_CRON",
  "CHAPPIE_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CHAPPIE_STATE_DIR = path.join(tempHome, ".chappie");
  delete process.env.CHAPPIE_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.CHAPPIE_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.CHAPPIE_SKIP_GMAIL_WATCHER = "1";
  process.env.CHAPPIE_SKIP_CANVAS_HOST = "1";
  process.env.CHAPPIE_SKIP_CHANNELS = "1";
  process.env.CHAPPIE_SKIP_PROVIDERS = "1";
  process.env.CHAPPIE_SKIP_CRON = "1";
  process.env.CHAPPIE_TEST_MINIMAL_GATEWAY = "1";
  process.env.CHAPPIE_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "chappie-test-no-bundled-extensions")
    : "chappie-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".chappie-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".chappie-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__chappieTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__chappieTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["CHAPPIE_GATEWAY_TOKEN"]);
  const prev = process.env.CHAPPIE_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CHAPPIE_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.CHAPPIE_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
