...
const connectNodeClient = async (params: {
  ...
  const token = process.env.CHAPPIE_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("CHAPPIE_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".chappie", "restart-sentinel.json");
...
describe("gateway node command allowlist", () => {
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    ...
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `chappie-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
  ...
  test("filters system.run for confusable iOS metadata at connect time", async () => {
    ...
    for (const testCase of cases) {
      const deviceIdentityPath = path.join(
        os.tmpdir(),
        `chappie-confusable-node-${testCase.label}-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
      );
      const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
