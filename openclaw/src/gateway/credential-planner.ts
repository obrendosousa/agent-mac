...
type GatewaySecretDefaults = NonNullable<ChappieConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${CHAPPIE_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["CHAPPIE_GATEWAY_TOKEN", "CHAPPIEDBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["CHAPPIE_GATEWAY_PASSWORD", "CHAPPIEDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
