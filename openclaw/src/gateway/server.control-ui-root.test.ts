...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-gateway-ui-hardlink-"));
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "chappie");
    const controlUiRoot = path.join(packageRoot, "dist", "control-ui");
    await fs.mkdir(controlUiRoot, { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "chappie" }),
    );
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Chappie package control-ui", async () => {
