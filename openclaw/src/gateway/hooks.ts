...
export function resolveHooksConfig(cfg: ChappieConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ChappieConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-chappie-token"] === "string"
      ? req.headers["x-chappie-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-chappie-idempotency-key"]) ||
