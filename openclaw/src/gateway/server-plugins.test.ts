...
const loadChappiePlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadChappiePlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadChappiePlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadChappiePlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadChappiePlugins.mock.calls.at(-1)?.[0];
