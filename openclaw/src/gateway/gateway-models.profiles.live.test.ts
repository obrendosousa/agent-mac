...
import { resolveChappieAgentDir } from "../agents/agent-paths.js";
...
import { ensureChappieModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, ChappieConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.CHAPPIE_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.CHAPPIE_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CHAPPIE_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.CHAPPIE_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.CHAPPIE_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.CHAPPIE_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.CHAPPIE_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ChappieConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.CHAPPIE_CONFIG_PATH,
    token: process.env.CHAPPIE_GATEWAY_TOKEN,
    skipChannels: process.env.CHAPPIE_SKIP_CHANNELS,
    skipGmail: process.env.CHAPPIE_SKIP_GMAIL_WATCHER,
    skipCron: process.env.CHAPPIE_SKIP_CRON,
    skipCanvas: process.env.CHAPPIE_SKIP_CANVAS_HOST,
    agentDir: process.env.CHAPPIE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CHAPPIE_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.CHAPPIE_SKIP_CHANNELS = "1";
  process.env.CHAPPIE_SKIP_GMAIL_WATCHER = "1";
  process.env.CHAPPIE_SKIP_CRON = "1";
  process.env.CHAPPIE_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.CHAPPIE_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveChappieAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-live-state-"));
  process.env.CHAPPIE_STATE_DIR = tempStateDir;
  ...
  process.env.CHAPPIE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.chappie-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveChappieAgentDir();
  const sanitizedCfg: ChappieConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-live-"));
  const tempConfigPath = path.join(tempDir, "chappie.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CHAPPIE_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "Chappie live tool probe (local, safe): " +
    ...
    process.env.CHAPPIE_CONFIG_PATH = previous.configPath;
    process.env.CHAPPIE_GATEWAY_TOKEN = previous.token;
    process.env.CHAPPIE_SKIP_CHANNELS = previous.skipChannels;
    process.env.CHAPPIE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CHAPPIE_SKIP_CRON = previous.skipCron;
    process.env.CHAPPIE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CHAPPIE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CHAPPIE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.CHAPPIE_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via CHAPPIE_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.chappie-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
