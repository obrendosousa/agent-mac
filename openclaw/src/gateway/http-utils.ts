...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-chappie-agent-id")?.trim() ||
    getHeader(req, "x-chappie-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(model: string | undefined): string | undefined {
  ...
  const m =
    raw.match(/^chappie[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-chappie-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-chappie-message-channel")) ??
