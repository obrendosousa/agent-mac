...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CHAPPIE_SKIP_CHANNELS;
    prevSkipGmail = process.env.CHAPPIE_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.CHAPPIE_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.CHAPPIE_SKIP_CHANNELS = "0";
    delete process.env.CHAPPIE_SKIP_GMAIL_WATCHER;
    delete process.env.CHAPPIE_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CHAPPIE_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CHAPPIE_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CHAPPIE_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.CHAPPIE_SKIP_PROVIDERS;
    } else {
      process.env.CHAPPIE_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.CHAPPIE_CONFIG_PATH;
    if (!configPath) {
      throw new Error("CHAPPIE_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.CHAPPIE_STATE_DIR;
    if (!stateDir) {
      throw new Error("CHAPPIE_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_CHAPPIE_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_CHAPPIE_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_CHAPPIE_AUTH_REF" is missing or empty.',
      );
    } finally {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.CHAPPIE_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.CHAPPIE_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
      envSnapshot.restore();
      ws.close();
      await server.close();
