...
function cfg(input: Partial<ChappieConfig>): ChappieConfig {
  return input as ChappieConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: ChappieConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  CHAPPIE_GATEWAY_TOKEN: "env-token",
  CHAPPIE_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  const cases: ConnectionAuthCase[] = [
    ...
    {
      name: "includeLegacyEnv controls CHAPPIEDBOT fallback",
      cfg: cfg({
        gateway: {
          mode: "local",
          auth: {},
      ...
      env: {
        ...
        CHAPPIEDBOT_GATEWAY_PASSWORD: "legacy-password", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      options: {
  ...
  it("resolves config-first token SecretRef even when CHAPPIE env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when CHAPPIE env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
