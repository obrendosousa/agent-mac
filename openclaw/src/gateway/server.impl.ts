...
import { ensureChappieCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.CHAPPIE_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.CHAPPIE_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "CHAPPIE_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "CHAPPIE_RAW_STREAM_PATH",
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("chappie doctor")}" to repair, then retry.`,
    );
  }
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `chappie config set gateway.auth.mode token` and `chappie config set gateway.auth.token <token>`.",
      );
    }
  }
  cfgAtStart = (
