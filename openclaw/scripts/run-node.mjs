...
const shouldBuild = (deps) => {
  if (deps.env.CHAPPIE_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.CHAPPIE_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[chappie] ${message}\n`);
};

const runChappie = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["chappie.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
