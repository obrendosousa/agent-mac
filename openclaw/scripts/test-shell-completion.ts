 ...
 * `chappie update`. Use it to verify the completion installation flow
...
\`chappie update\`, \`chappie doctor\`, and \`chappie onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Chappie Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`chappie completion --install\` later to enable.`));
    return;
  }
