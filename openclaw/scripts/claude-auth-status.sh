...
# Checks both Claude Code and Chappie auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CHAPPIE_AUTH="$HOME/.chappie/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    chappie models status --json 2>/dev/null || true
}
...
check_chappie_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$CHAPPIE_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    chappie_status=$(check_chappie_auth 2>/dev/null || true)

    claude_expires=0
    chappie_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        chappie_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        chappie_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CHAPPIE_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$chappie_status" \
        --arg be "$chappie_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            chappie: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$chappie_status" == EXPIRED* ]] || [[ "$chappie_status" == MISSING* ]]; then
        echo "CHAPPIE_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$chappie_status" == EXPIRING* ]]; then
        echo "CHAPPIE_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Chappie Auth (~/.chappie/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$CHAPPIE_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$CHAPPIE_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'chappie doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active chappie >/dev/null 2>&1; then
    echo -e "Chappie service: ${GREEN}running${NC}"
else
    echo -e "Chappie service: ${RED}NOT running${NC}"
fi
