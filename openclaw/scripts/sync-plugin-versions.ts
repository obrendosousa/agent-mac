...
function stripWorkspaceChappieDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.chappie !== "workspace:*") {
    return false;
  }
  delete devDeps.chappie;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceChappieDevDependency(pkg);
