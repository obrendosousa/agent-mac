...
import type { ApiContributor, Entry, MapConfig, User } from "./update-chappietributors.types.js";

const REPO = "chappie/chappie";
const PER_LINE = 10;

const mapPath = resolve("scripts/chappietributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing chappietributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README chappietributors: ${entries.length} entries`);
