...
import { discoverChappiePlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']chappie\/plugin-sdk["']/;
...
function main() {
  ...
  if (offenders.length > 0) {
    console.error("Bundled plugin source files must not import monolithic chappie/plugin-sdk.");
    for (const file of offenders.toSorted()) {
      const relative = path.relative(process.cwd(), file) || file;
      console.error(`- ${relative}`);
    }
    console.error(
      "Use chappie/plugin-sdk/<channel> for channel plugins, /core for startup surfaces, or /compat for broader internals.",
    );
    process.exit(1);
  }
