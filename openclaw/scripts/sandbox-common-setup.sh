...
BASE_IMAGE="${BASE_IMAGE:-chappie-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-chappie-sandbox-common:bookworm-slim}"
...
CHAPPIE_DOCKER_BUILD_USE_BUILDX="${CHAPPIE_DOCKER_BUILD_USE_BUILDX:-0}"
CHAPPIE_DOCKER_BUILD_CACHE_FROM="${CHAPPIE_DOCKER_BUILD_CACHE_FROM:-}"
CHAPPIE_DOCKER_BUILD_CACHE_TO="${CHAPPIE_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${CHAPPIE_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${CHAPPIE_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${CHAPPIE_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${CHAPPIE_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${CHAPPIE_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=chappie.sandbox=1)
NOTE
