...
const EXPECTED_REPOSITORY_URL = "https://github.com/chappie/chappie";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "chappie") {
    errors.push(`package.json name must be "chappie"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.chappie !== "chappie.mjs") {
    errors.push(
      `package.json bin.chappie must be "chappie.mjs"; found "${pkg.bin?.chappie ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`chappie-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `chappie-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
