...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "chappie.json");
  ...
  const sessionId =
    process.env.CHAPPIE_ZAI_FALLBACK_SESSION_ID ??
    process.env.CHAPPIEDBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const baseEnv: NodeJS.ProcessEnv = {
    ...
    CHAPPIEDBOT_CONFIG_PATH: configPath,
    CHAPPIEDBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["chappie", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["chappie", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
