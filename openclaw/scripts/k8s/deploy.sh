...
#
# Environment:
#   CHAPPIE_NAMESPACE   Kubernetes namespace (default: chappie)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${CHAPPIE_NAMESPACE:-chappie}"
...
Usage: ./scripts/k8s/deploy.sh [OPTION]
  ...
  (no args)        Deploy Chappie (creates secret from env if needed)
...
_apply_secret() {
  ...
  if kubectl get secret chappie-secrets -n "$NS" &>/dev/null; then
    EXISTING_SECRET=true
    EXISTING_TOKEN="$(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.CHAPPIE_GATEWAY_TOKEN}' | base64 -d)"
    ANTHROPIC_VALUE="$(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.ANTHROPIC_API_KEY}' 2>/dev/null | base64 -d)"
    OPENAI_VALUE="$(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.OPENAI_API_KEY}' 2>/dev/null | base64 -d)"
    GEMINI_VALUE="$(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.GEMINI_API_KEY}' 2>/dev/null | base64 -d)"
    OPENROUTER_VALUE="$(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.OPENROUTER_API_KEY}' 2>/dev/null | base64 -d)"
  fi
  ...
  printf '%s' "$TOKEN" > "$TMP_DIR/CHAPPIE_GATEWAY_TOKEN"
  ...
  chmod 600 \
    "$TMP_DIR/CHAPPIE_GATEWAY_TOKEN" \
    "$TMP_DIR/ANTHROPIC_API_KEY" \
    "$TMP_DIR/OPENAI_API_KEY" \
    "$TMP_DIR/GEMINI_API_KEY" \
    "$TMP_DIR/OPENROUTER_API_KEY"

  kubectl create secret generic chappie-secrets \
    -n "$NS" \
    --from-file=CHAPPIE_GATEWAY_TOKEN="$TMP_DIR/CHAPPIE_GATEWAY_TOKEN" \
  ...
  kubectl apply --server-side --field-manager=chappie -f "$SECRET_MANIFEST" >/dev/null
  # Clean up any annotation left by older client-side apply runs.
  kubectl annotate secret chappie-secrets -n "$NS" kubectl.kubernetes.io/last-applied-configuration- >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
  trap - EXIT
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret chappie-secrets -n $NS -o jsonpath='{.data.CHAPPIE_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
kubectl rollout restart deployment/chappie -n "$NS" 2>/dev/null || true
echo ""
echo "Waiting for rollout..."
kubectl rollout status deployment/chappie -n "$NS" --timeout=300s
echo ""
echo "Done. Access the gateway:"
echo "  kubectl port-forward svc/chappie 18789:18789 -n $NS"
echo "  open http://localhost:18789"
echo ""
if $SHOW_TOKEN; then
  echo "Gateway token (paste into Control UI):"
  echo "  $(kubectl get secret chappie-secrets -n "$NS" -o jsonpath='{.data.CHAPPIE_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
