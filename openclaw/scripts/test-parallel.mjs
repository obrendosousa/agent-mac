...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/chappie-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// Keep it opt-out via CHAPPIE_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor <= 24 : true;
const useVmForks =
  process.env.CHAPPIE_TEST_VM_FORKS === "1" ||
  (process.env.CHAPPIE_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks && !lowMemLocalHost);
const disableIsolation = process.env.CHAPPIE_TEST_NO_ISOLATE === "1";
const includeGatewaySuite = process.env.CHAPPIE_TEST_INCLUDE_GATEWAY === "1";
const includeExtensionsSuite = process.env.CHAPPIE_TEST_INCLUDE_EXTENSIONS === "1";
const rawTestProfile = process.env.CHAPPIE_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const shardOverride = Number.parseInt(process.env.CHAPPIE_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.CHAPPIE_TEST_SHARD_INDEX ?? "", 10);
...
if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] CHAPPIE_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires CHAPPIE_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
if (shardIndexOverride !== null && shardIndexOverride > shardCount) {
  console.error(
    `[test-parallel] CHAPPIE_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} exceeds CHAPPIE_TEST_SHARDS=${String(shardCount)}.`,
  );
  process.exit(2);
}
const windowsCiArgs = isWindowsCi ? ["--dangerouslyIgnoreUnhandledErrors"] : [];
const silentArgs =
  process.env.CHAPPIE_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
...
const overrideWorkers = Number.parseInt(process.env.CHAPPIE_TEST_WORKERS ?? "", 10);
const resolvedOverride =
  Number.isFinite(overrideWorkers) && overrideWorkers > 0 ? overrideWorkers : null;
const parallelGatewayEnabled =
  process.env.CHAPPIE_TEST_PARALLEL_GATEWAY === "1" || (!isCI && highMemLocalHost);
// Keep gateway serial by default except when explicitly requested or on high-memory local hosts.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.CHAPPIE_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
  !parallelGatewayEnabled;
const parallelRuns = keepGatewaySerial ? runs.filter((entry) => entry.name !== "gateway") : runs;
const serialRuns = keepGatewaySerial ? runs.filter((entry) => entry.name === "gateway") : [];
const baseLocalWorkers = Math.max(4, Math.min(16, hostCpuCount));
const loadAwareDisabledRaw = process.env.CHAPPIE_TEST_LOAD_AWARE?.trim().toLowerCase();
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.CHAPPIE_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
