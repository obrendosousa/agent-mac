...
# Build and bundle Chappie into a minimal .app we can open.
# Outputs to dist/Chappie.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Chappie.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Chappie"
BUNDLE_ID="${BUNDLE_ID:-ai.chappie.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/chappie/chappie/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Chappie/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ChappieBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ChappieGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Chappie"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Chappie"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Chappie"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Chappie" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Chappie/Resources/Chappie.icns" "$APP_ROOT/Contents/Resources/Chappie.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Chappie/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ChappieKit resources"
CHAPPIEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ChappieKit_ChappieKit.bundle"
if [ -d "$CHAPPIEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ChappieKit_ChappieKit.bundle"
  cp -R "$CHAPPIEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ChappieKit_ChappieKit.bundle"
else
  echo "WARN: ChappieKit resource bundle not found at $CHAPPIEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Chappie"
killall -q Chappie 2>/dev/null || true
