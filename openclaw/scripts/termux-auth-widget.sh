...
# Chappie Auth Widget for Termux
...
SERVER="${CHAPPIE_SERVER:-${CHAPPIEDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Chappie auth..."

STATUS=$(ssh "$SERVER" '$HOME/chappie/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/chappie/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CHAPPIE_EXPIRING|CHAPPIEDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/chappie/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CHAPPIE_EXPIRED|CHAPPIEDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Chappie won't work until re-auth"
                ;;
        esac
        ;;
