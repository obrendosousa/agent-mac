...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/chappie/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Chappie Re-Auth" -c "After getting key, run: ssh $SERVER '~/chappie/scripts/mobile-reauth.sh'" --id chappie-auth
        ;;
    *)
