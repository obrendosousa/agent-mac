...
# Chappie Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Chappie."
...
GUM_VERSION="${CHAPPIE_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Chappie Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://chappie.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$CHAPPIE_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.chappie.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_chappie_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.chappie-* "$npm_root"/chappie 2>/dev/null || true
}

extract_chappie_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_chappie_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/chappie" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/chappie"|"/usr/local/bin/chappie")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/chappie/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale chappie symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing chappie binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Chappie package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_chappie_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Chappie npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*chappie" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_chappie_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_chappie_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Chappie npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an chappie binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "CHAPPIE_TAGLINE_INDEX" "CHAPPIEDBOT_TAGLINE_INDEX"
map_legacy_env "CHAPPIE_NO_ONBOARD" "CHAPPIEDBOT_NO_ONBOARD"
map_legacy_env "CHAPPIE_NO_PROMPT" "CHAPPIEDBOT_NO_PROMPT"
map_legacy_env "CHAPPIE_DRY_RUN" "CHAPPIEDBOT_DRY_RUN"
map_legacy_env "CHAPPIE_INSTALL_METHOD" "CHAPPIEDBOT_INSTALL_METHOD"
map_legacy_env "CHAPPIE_VERSION" "CHAPPIEDBOT_VERSION"
map_legacy_env "CHAPPIE_BETA" "CHAPPIEDBOT_BETA"
map_legacy_env "CHAPPIE_GIT_DIR" "CHAPPIEDBOT_GIT_DIR"
map_legacy_env "CHAPPIE_GIT_UPDATE" "CHAPPIEDBOT_GIT_UPDATE"
map_legacy_env "CHAPPIE_NPM_LOGLEVEL" "CHAPPIEDBOT_NPM_LOGLEVEL"
map_legacy_env "CHAPPIE_VERBOSE" "CHAPPIEDBOT_VERBOSE"
map_legacy_env "CHAPPIE_PROFILE" "CHAPPIEDBOT_PROFILE"
map_legacy_env "CHAPPIE_INSTALL_SH_NO_RUN" "CHAPPIEDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${CHAPPIE_TAGLINE_INDEX:-}" ]]; then
        if [[ "${CHAPPIE_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((CHAPPIE_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${CHAPPIE_NO_ONBOARD:-0}
NO_PROMPT=${CHAPPIE_NO_PROMPT:-0}
DRY_RUN=${CHAPPIE_DRY_RUN:-0}
INSTALL_METHOD=${CHAPPIE_INSTALL_METHOD:-}
CHAPPIE_VERSION=${CHAPPIE_VERSION:-latest}
USE_BETA=${CHAPPIE_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/chappie"
GIT_DIR=${CHAPPIE_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${CHAPPIE_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${CHAPPIE_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${CHAPPIE_VERBOSE:-0}"
VERIFY_INSTALL="${CHAPPIE_VERIFY_INSTALL:-0}"
CHAPPIE_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Chappie installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/chappie)
...
Environment variables:
  CHAPPIE_INSTALL_METHOD=git|npm
  CHAPPIE_VERSION=latest|next|<semver>
  CHAPPIE_BETA=0|1
  CHAPPIE_GIT_DIR=...
  CHAPPIE_GIT_UPDATE=0|1
  CHAPPIE_NO_PROMPT=1
  CHAPPIE_VERIFY_INSTALL=1
  CHAPPIE_DRY_RUN=1
  CHAPPIE_NO_ONBOARD=1
  CHAPPIE_VERBOSE=1
  CHAPPIE_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Chappie checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Chappie source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_chappie_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"chappie"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://chappie.ai/install.sh | bash"
}
...
ensure_chappie_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/chappie" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/chappie" ]]; then
        ln -sf "$npm_root/chappie/dist/entry.js" "${npm_bin}/chappie"
        ui_info "Created chappie bin link at ${npm_bin}/chappie"
    fi
    return 0
}

# Check for existing Chappie installation
check_existing_chappie() {
    if [[ -n "$(type -P chappie 2>/dev/null || true)" ]]; then
        ui_info "Existing Chappie installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make chappie show as \"command not found\" in new terminals."
...
warn_chappie_not_found() {
    ui_warn "Installed, but chappie is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t chappie 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named chappie; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_chappie_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P chappie 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/chappie" ]]; then
        echo "${npm_bin}/chappie"
        return 0
    fi
...
install_chappie_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/chappie/chappie.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Chappie from git checkout: ${repo_dir}"
    else
        ui_info "Installing Chappie from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Chappie" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Chappie" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/chappie" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/chappie"
    ui_success "Chappie wrapper installed to \$HOME/.local/bin/chappie"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view chappie dist-tags.beta 2>/dev/null || true)"
...
install_chappie() {
    local package_name="chappie"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            CHAPPIE_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="chappie"
        else
            CHAPPIE_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${CHAPPIE_VERSION}" ]]; then
        CHAPPIE_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${CHAPPIE_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Chappie v${resolved_version}"
    else
        ui_info "Installing Chappie (${CHAPPIE_VERSION})"
    fi
    local install_spec=""
    if [[ "${CHAPPIE_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${CHAPPIE_VERSION}"
    fi

    if ! install_chappie_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_chappie_paths
        install_chappie_npm "${install_spec}"
    fi

    if [[ "${CHAPPIE_VERSION}" == "latest" && "${package_name}" == "chappie" ]]; then
        if ! resolve_chappie_bin &> /dev/null; then
            ui_warn "npm install chappie@latest failed; retrying chappie@next"
            cleanup_npm_chappie_paths
            install_chappie_npm "chappie@next"
        fi
    fi

    ensure_chappie_bin_link || true

    ui_success "Chappie installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${CHAPPIE_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_chappie_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (chappie not on PATH yet)"
        warn_chappie_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${CHAPPIE_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.chappie/workspace-${profile}"
    else
        echo "${HOME}/.chappie/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${CHAPPIE_CONFIG_PATH:-$HOME/.chappie/chappie.json}"
    if [[ -f "${config_path}" || -f "$HOME/.chappiedbot/chappiedbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run chappie onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but chappie not on PATH; skipping onboarding"
        warn_chappie_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run chappie onboard to retry"
        return
    }
}
...
if ! declare -F extract_chappie_semver >/dev/null 2>&1; then
...
resolve_chappie_version() {
    ...
    local claw="${CHAPPIE_BIN:-}"
    if [[ -z "$claw" ]] && command -v chappie &> /dev/null; then
        claw="$(command -v chappie)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_chappie_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/chappie/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/chappie/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: chappie not on PATH yet"
        warn_chappie_not_found
        return 1
    fi

    run_quiet_step "Checking Chappie version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: chappie gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_chappie_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Chappie checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set CHAPPIE_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_chappie; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Chappie"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g chappie &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g chappie 2>/dev/null || true
        ...
        install_chappie_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/chappie" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/chappie"
    ...
    CHAPPIE_BIN="$(resolve_chappie_bin || true)"
    ...
    installed_version=$(resolve_chappie_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Chappie installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Chappie installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/chappie"
        ui_kv "Update command" "chappie update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://chappie.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running chappie doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                CHAPPIE_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                CHAPPIE_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                CHAPPIE_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run chappie doctor and chappie plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run chappie onboard later"
        else
            local config_path="${CHAPPIE_CONFIG_PATH:-$HOME/.chappie/chappie.json}"
            if [[ -f "${config_path}" || -f "$HOME/.chappiedbot/chappiedbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (chappie not on PATH yet)"
                    warn_chappie_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run chappie onboard to finish setup"
            return 0
        fi
    fi

    if command -v chappie &> /dev/null; then
        local claw="${CHAPPIE_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_chappie_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (chappie daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if CHAPPIE_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: chappie daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${CHAPPIE_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
