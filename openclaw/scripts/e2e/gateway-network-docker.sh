...
IMAGE_NAME="chappie-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="chappie-net-e2e-$$"
GW_NAME="chappie-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "CHAPPIE_GATEWAY_TOKEN=$TOKEN" \
  -e "CHAPPIE_SKIP_CHANNELS=1" \
  -e "CHAPPIE_SKIP_GMAIL_WATCHER=1" \
  -e "CHAPPIE_SKIP_CRON=1" \
  -e "CHAPPIE_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
