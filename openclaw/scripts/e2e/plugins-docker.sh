...
IMAGE_NAME="chappie-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/chappie-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.chappie/extensions/demo-plugin"

  cat > "$HOME/.chappie/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.chappie/extensions/demo-plugin/chappie.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CHAPPIE_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/chappie-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@chappie/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/chappie.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$CHAPPIE_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$CHAPPIE_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/chappie-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@chappie/demo-plugin-dir",
  "version": "0.0.1",
  "chappie": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/chappie.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CHAPPIE_ENTRY" plugins install "$dir_plugin"
	  node "$CHAPPIE_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/chappie-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@chappie/demo-plugin-file",
  "version": "0.0.1",
  "chappie": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/chappie.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CHAPPIE_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$CHAPPIE_ENTRY" plugins list --json > /tmp/plugins4.json
