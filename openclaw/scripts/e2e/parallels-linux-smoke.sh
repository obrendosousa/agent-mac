...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/chappie-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/chappie-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/chappie-install.sh
  guest_exec /usr/bin/env CHAPPIE_NO_ONBOARD=1 bash /tmp/chappie-install.sh "${version_args[@]}" --no-onboard
  guest_exec chappie --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec chappie --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" chappie onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" chappie agent \
    --local \
    --agent main \
    --message ping \
    --json
