...
IMAGE_NAME="chappie-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/chappie-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/chappie-bin/systemctl

  cat > /tmp/chappie-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/chappie-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/chappie"
	  npm_root="/tmp/npm-prefix/lib/node_modules/chappie"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/chappie-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/chappie-gateway.service"
    if [ ! -f "$unit_path" ]; then
