...
IMAGE_NAME="chappie-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    CHAPPIE_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    CHAPPIE_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export CHAPPIE_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/chappie-bin:$PATH"
  mkdir -p /tmp/chappie-bin
  cat > /tmp/chappie-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$CHAPPIE_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_chappie_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/chappie-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/chappie-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$CHAPPIE_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/chappie-e2e-$1.XXXXXX"
  }

  set_isolated_chappie_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export CHAPPIE_HOME="$home_dir"
    export CHAPPIE_STATE_DIR="$home_dir/.chappie"
    export CHAPPIE_CONFIG_PATH="$CHAPPIE_STATE_DIR/chappie.json"
    mkdir -p "$CHAPPIE_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$CHAPPIE_STATE_DIR/workspace"
    config_path="$CHAPPIE_CONFIG_PATH"
    sessions_dir="$CHAPPIE_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_chappie_env "$home_dir"
	    ...
	    node "$CHAPPIE_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$CHAPPIE_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$CHAPPIE_ENTRY\" configure --section skills" send_skills_flow
