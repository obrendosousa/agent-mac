...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-chappie-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://chappie.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/chappie-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/chappie-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-chappie-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://chappie.ai/install.ps1
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: chappie@2026.3.13-beta.1
...
guest_run_chappie() {
  ...
  stdout_name="chappie-stdout-$RANDOM-$RANDOM.log"
  stderr_name="chappie-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\chappie.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
resolve_latest_version() {
  ...
  npm view chappie version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "chappie-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Chappie\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Chappie\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Chappie\\deps\" mkdir \"%LOCALAPPDATA%\\Chappie\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Chappie\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Chappie\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/chappie-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/chappie-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\chappie.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Chappie\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\chappie.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_chappie "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="chappie-onboard-$RANDOM-$RANDOM.ps1"
  log_name="chappie-onboard-$RANDOM-$RANDOM.log"
  done_name="chappie-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$chappie = Join-Path \$env:APPDATA 'npm\chappie.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$chappie, \$log)
...
verify_gateway() {
  guest_run_chappie "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_chappie "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_chappie "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_chappie "" "" gateway status --deep
}

verify_turn() {
  guest_run_chappie "" "" agent --agent main --message ping --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "chappie-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "chappie-main-upgrade.tgz" || return $?
