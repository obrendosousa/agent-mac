...
#
# Suggested cron: */30 * * * * /home/admin/chappie/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Chappie notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., chappie-alerts)
...
STATE_FILE="$HOME/.chappie/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Chappie if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use chappie
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Chappie to $NOTIFY_PHONE..."
            chappie send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Chappie is down. Run: ssh l36 '~/chappie/scripts/mobile-reauth.sh'" "urgent"
    exit 1
