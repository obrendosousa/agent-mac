...
type DriverMode = "token" | "webhook" | "chappie";
...
function resolveStateDir(): string {
  const override = process.env.CHAPPIE_STATE_DIR?.trim() || process.env.CHAPPIEDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.CHAPPIE_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".chappie");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver chappie] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Chappie spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|chappie> Driver transport mode (default: token)\n" +
    ...
    "  --chappie-bin <path>        Chappie CLI binary for driver=chappie (default: chappie)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  CHAPPIE_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  CHAPPIE_DISCORD_SMOKE_DRIVER\n" +
    "  CHAPPIE_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  CHAPPIE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  CHAPPIE_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  CHAPPIE_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  CHAPPIE_DISCORD_SMOKE_AGENT\n" +
    "  CHAPPIE_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  CHAPPIE_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  CHAPPIE_DISCORD_SMOKE_POLL_MS\n" +
    "  CHAPPIE_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  CHAPPIE_DISCORD_SMOKE_CHAPPIE_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.CHAPPIE_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.CHAPPIE_DISCORD_SMOKE_DRIVER ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "chappie"
        ? "chappie"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.CHAPPIE_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.CHAPPIE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.CHAPPIE_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.CHAPPIE_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.CHAPPIE_DISCORD_SMOKE_AGENT ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.CHAPPIE_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.CHAPPIE_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CHAPPIEDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.CHAPPIE_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.CHAPPIE_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.CHAPPIE_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const chappieBin =
    resolveArg("--chappie-bin") || process.env.CHAPPIE_DISCORD_SMOKE_CHAPPIE_BIN || "chappie";
...
async function chappieCliJson<T>(params: { chappieBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.chappieBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`chappie ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithChappie(params: {
  chappieBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await chappieCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    chappieBin: params.chappieBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "chappie") {
    return await readMessagesWithChappie({
      chappieBin: params.args.chappieBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `chappie-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await chappieCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("chappie message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "chappie"
            ? await readMessagesWithChappie({
                chappieBin: args.chappieBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Chappie.`,
        diagnostics: {
          bindingCandidates: [
            {
