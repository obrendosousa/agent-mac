...
- regenerates apps/ios/Chappie.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${CHAPPIE_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid CHAPPIE_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid CHAPPIE_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid CHAPPIE_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid CHAPPIE_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing CHAPPIE_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(CHAPPIE_URL_SLASH)$(CHAPPIE_URL_SLASH)#g'
)"
...
CHAPPIE_CODE_SIGN_STYLE = Automatic
CHAPPIE_DEVELOPMENT_TEAM = ${TEAM_ID}
CHAPPIE_IOS_SELECTED_TEAM = ${TEAM_ID}
CHAPPIE_APP_BUNDLE_ID = ai.chappie.client
CHAPPIE_SHARE_BUNDLE_ID = ai.chappie.client.share
CHAPPIE_ACTIVITY_WIDGET_BUNDLE_ID = ai.chappie.client.activitywidget
CHAPPIE_WATCH_APP_BUNDLE_ID = ai.chappie.client.watchkitapp
CHAPPIE_WATCH_EXTENSION_BUNDLE_ID = ai.chappie.client.watchkitapp.extension
CHAPPIE_APP_PROFILE =
CHAPPIE_SHARE_PROFILE =
CHAPPIE_PUSH_TRANSPORT = relay
CHAPPIE_PUSH_DISTRIBUTION = official
CHAPPIE_URL_SLASH = /
CHAPPIE_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
CHAPPIE_PUSH_APNS_ENVIRONMENT = production
EOF
