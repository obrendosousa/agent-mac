...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "chappielog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
chappielog - Chappie Logging Utility

USAGE:
    chappielog [OPTIONS]

DESCRIPTION:
    View Chappie logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Chappie logs flow through the macOS unified log (subsystem: ai.chappie).
...
QUICK START:
    chappielog -n 100             Show last 100 lines from all components
    chappielog -f                 Follow logs in real-time
    chappielog -e                 Show only errors
    chappielog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    chappielog                   Show last 50 lines from past 5 minutes (default)
    chappielog -f                Stream logs continuously
    chappielog -n 100            Show last 100 lines
    chappielog -e                Show only recent errors
    chappielog -l 30m -n 200     Show last 200 lines from past 30 minutes
    chappielog -c ServerManager  Show recent ServerManager logs
    chappielog -s "fail"         Search for "fail" in recent logs
    chappielog --server -e       Show recent server errors
    chappielog -f -d             Stream debug logs continuously
