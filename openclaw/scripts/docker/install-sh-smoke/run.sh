...
INSTALL_URL="${CHAPPIE_INSTALL_URL:-https://chappie.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CHAPPIE_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CHAPPIE_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="chappie"
PACKAGE_NAME="${CHAPPIE_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CHAPPIE_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CHAPPIE_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CHAPPIE_INSTALL_LATEST_OUT:-}"
fi
