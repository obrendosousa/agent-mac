#!/bin/bash
# Setup Chappie Auth Management System
...
echo "=== Chappie Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'chappie-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Chappie message: Send warning via Chappie itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/chappie-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/chappie-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/chappie-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now chappie-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status chappie-auth-monitor.timer"
echo ""
echo "Quick commands:"
