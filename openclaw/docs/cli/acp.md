...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Chappie Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`chappie acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Chappie gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
chappie acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
chappie acp --url wss://gateway-host:18789 --token-file ~/.chappie/gateway.token

# Attach to an existing session key
chappie acp --session agent:main:main

# Attach by label (must already exist)
chappie acp --session-label "support inbox"

# Reset the session key before the first prompt
chappie acp --session agent:main:main --reset-session
```
...
chappie acp client --server-args --url wss://gateway-host:18789 --token-file ~/.chappie/gateway.token

# Override the server command (default: chappie)
chappie acp client --server "node" --server-args chappie.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Chappie Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `chappie acp` over stdio.

Example config (persisted):

```bash
chappie config set gateway.remote.url wss://gateway-host:18789
chappie config set gateway.remote.token <token>
```
...
```bash
chappie acp --session agent:main:main
chappie acp --session agent:design:main
chappie acp --session agent:qa:bug-123
```
...
Chappie bot over ACP, use `acpx` with its built-in `chappie` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx chappie` at `chappie acp`.
3. Target the Chappie session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Chappie ACP session
acpx chappie exec "Summarize the active Chappie session state."

# Persistent named session for follow-up turns
acpx chappie sessions ensure --name codex-bridge
acpx chappie -s codex-bridge --cwd /path/to/repo \
  "Ask my Chappie work agent for recent context relevant to this repo."
```

If you want `acpx chappie` to target a specific Gateway and session key every
time, override the `chappie` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "chappie": {
      "command": "env CHAPPIE_HIDE_BANNER=1 CHAPPIE_SUPPRESS_NOTES=1 chappie acp --url ws://127.0.0.1:18789 --token-file ~/.chappie/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Chappie checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env CHAPPIE_HIDE_BANNER=1 CHAPPIE_SUPPRESS_NOTES=1 node chappie.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Chappie agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Chappie ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`CHAPPIE_GATEWAY_TOKEN`, `CHAPPIE_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`CHAPPIE_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `CHAPPIE_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `chappie acp client` sets `CHAPPIE_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `chappie`).
