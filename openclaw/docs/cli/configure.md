---
summary: "CLI reference for `chappie configure` (interactive configuration prompts)"
read_when:
...
Tip: `chappie config` without a subcommand opens the same wizard. Use
`chappie config get|set|unset` for non-interactive edits.
...
```bash
chappie configure
chappie configure --section model --section channels
```
