---
summary: "CLI reference for `chappie onboard` (interactive onboarding wizard)"
read_when:
...
```bash
chappie onboard
chappie onboard --flow quickstart
chappie onboard --flow manual
chappie onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CHAPPIE_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `chappie gateway run`.
...
- Fastest first chat: `chappie dashboard` (Control UI, no channel setup).
