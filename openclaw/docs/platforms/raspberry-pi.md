---
summary: "Chappie on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Chappie on a Raspberry Pi
  - Running Chappie on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Chappie on Raspberry Pi

## Goal

Run a persistent, always-on Chappie Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u chappie -f
```

## 9) Access the Chappie Dashboard
...
```bash
ssh user@gateway-host 'chappie dashboard --no-open'
```
...
(or `CHAPPIE_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache
mkdir -p /var/tmp/chappie-compile-cache
export CHAPPIE_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `CHAPPIE_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Chappie, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit chappie
```

```ini
[Service]
Environment=CHAPPIE_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Chappie state/cache on SSD-backed storage to avoid SD-card
...
Most Chappie features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled chappie

# Enable if not
sudo systemctl enable chappie

# Start on boot
sudo systemctl start chappie
```
...
```bash
# Check logs
journalctl -u chappie --no-pager -n 100

# Common fix: rebuild
cd ~/chappie  # if using hackable install
npm run build
sudo systemctl restart chappie
```
