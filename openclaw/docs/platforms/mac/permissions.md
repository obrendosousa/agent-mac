...
- Same path: run the app from a fixed location (for Chappie, `dist/Chappie.app`).
...
```bash
sudo tccutil reset Accessibility ai.chappie.mac
sudo tccutil reset ScreenCapture ai.chappie.mac
...
Workaround: move files into the Chappie workspace (`~/.chappie/workspace`) if you want to avoid per-folder grants.
