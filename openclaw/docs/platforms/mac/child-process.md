...
service via the external `chappie` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.chappie.gateway`
  (or `ai.chappie.<profile>` when using `--profile`/`CHAPPIE_PROFILE`; legacy `com.chappie.*` is supported).
