...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `ai.chappie`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.chappie" AND category CONTAINS "voicewake"' --level info --style compact
  ```
