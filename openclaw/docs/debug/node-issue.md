...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Chappie
...
Running Chappie via Node with `tsx` fails at startup with:

```
[chappie] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `chappie status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch chappie.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node chappie.mjs status` works on Node 25.
