...
- During heartbeat runs, Chappie treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `chappie sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Chappie skips the heartbeat run entirely (no model call).
...
```bash
chappie system event --text "Check for urgent follow-ups" --mode now
```
