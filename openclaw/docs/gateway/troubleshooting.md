...
- `chappie gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `chappie doctor` reports no blocking config/service issues.
- `chappie channels status --probe` shows connected/ready channels.
...
chappie config get agents.defaults.models
```
...
chappie status
chappie channels status --probe
chappie pairing list --channel <channel> [--account <id>]
chappie config get channels
chappie logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `chappie config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `chappie devices list` then `chappie devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `chappie configure`). If you are running Chappie via Podman using the dedicated `chappie` user, the config lives at `~chappie/.chappie/chappie.json`.
...
chappie cron status
chappie cron list
chappie cron runs --id <jobId> --limit 20
chappie system heartbeat last
chappie logs --follow
```
...
chappie approvals get --node <idOrNameOrIp>
chappie logs --follow
chappie status
```
...
chappie browser start --browser-profile chappie
...
```bash
chappie gateway status
chappie config get gateway.mode
chappie config get gateway.remote.url
chappie config get gateway.auth.mode
```
...
```bash
chappie config get gateway.bind
chappie config get gateway.auth.token
chappie gateway status
chappie logs --follow
```
