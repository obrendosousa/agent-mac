---
summary: "How Chappie sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Chappie can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`chappie-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Chappie emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- `"none"` (default): tools see a sandbox workspace under `~/.chappie/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Chappie mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Chappie blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `chappie-sandbox:bookworm-slim`
...
`chappie-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `CHAPPIE_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `CHAPPIE_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `CHAPPIE_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `CHAPPIE_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `CHAPPIE_DOCKER_SOCKET`. Full setup and env
...
- Use `chappie sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
