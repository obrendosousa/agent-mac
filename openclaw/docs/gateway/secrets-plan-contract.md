...
read_when:
  - Generating or reviewing `chappie secrets apply` plans
...
This page defines the strict contract enforced by `chappie secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`chappie secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `chappie.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `chappie secrets configure` or fix the target path to a supported shape above.
