...
Chappie supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `CHAPPIE_GATEWAY_TOKEN` (or `CHAPPIEDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Chappie runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Chappie validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Chappie QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/chappie"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `chappie.json` refs)
...
When reload-time activation fails after a healthy state, Chappie enters degraded secrets state.
...
- Strict command paths (for example `chappie memory` remote-memory paths and `chappie qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `chappie status`, `chappie status --all`, `chappie channels status`, `chappie channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `chappie secrets reload`.
...
```bash
chappie secrets audit --check
chappie secrets configure
chappie secrets audit --check
```
...
- plaintext values at rest (`chappie.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `chappie.json` refs)
...
- lets you select supported secret-bearing fields in `chappie.json` plus `auth-profiles.json` for one agent scope
...
- `chappie secrets configure --providers-only`
- `chappie secrets configure --skip-provider-setup`
- `chappie secrets configure --agent <id>`
...
```bash
chappie secrets apply --from /tmp/chappie-secrets-plan.json
chappie secrets apply --from /tmp/chappie-secrets-plan.json --dry-run
```
...
Chappie intentionally does not write rollback backups containing historical plaintext secret values.
