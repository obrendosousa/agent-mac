...
> Chappie is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Chappie security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `chappie security audit`
...
```bash
chappie security audit
chappie security audit --deep
chappie security audit --fix
chappie security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Chappie is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Chappie assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.chappie`, including `chappie.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- Chappie can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Chappie, use one VPS/host per user.
...
Chappie is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/chappie/chappie/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Chappie also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.chappie/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.chappie/credentials/<channel>-allowFrom.json` (default account)
  - `~/.chappie/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.chappie/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.chappie/secrets.json`
- **Legacy OAuth import**: `~/.chappie/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Chappie state                                 | filesystem perms on `~/.chappie`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.chappie/chappie.json`                                                   | yes      |
...
`chappie security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`chappie security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Chappie config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${CHAPPIE_GATEWAY_PASSWORD}
```
...
- Chappie gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Chappie responses.
...
Chappie stores session transcripts on disk under `~/.chappie/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.chappie` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Chappie cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

Chappie can refresh the skills list mid-session:
...
- If you install plugins from npm (`chappie plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.chappie/extensions/<pluginId>/` (or `$CHAPPIE_STATE_DIR/extensions/<pluginId>/`).
  - Chappie uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
chappie pairing list <channel>
chappie pairing approve <channel> <code>
```
...
By default, Chappie routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Chappie has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.chappie/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.chappie or your logs.”

## Unsafe external content bypass flags

Chappie includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.chappie/chappie.json`: `600` (user read/write only)
- `~/.chappie`: `700` (user only)

`chappie doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `CHAPPIE_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__chappie__/canvas/` and `/__chappie__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Chappie with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_chappie-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `CHAPPIE_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `chappie doctor --generate-gateway-token`.
...
paths, set `CHAPPIE_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CHAPPIE_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `CHAPPIE_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Chappie
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Chappie verifies the identity by resolving the
...
- Chappie will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.chappie/` (or `$CHAPPIE_STATE_DIR/`) may contain secrets or private data:

- `chappie.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `chappie status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@chappie", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.chappie`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.chappie/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `chappie` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Chappie clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

Chappie’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Chappie limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `chappie gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `CHAPPIE_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/chappie/chappie-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.chappie/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `chappie security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Chappie version
...
Found a vulnerability in Chappie? Please report responsibly:

1. Email: [security@chappie.ai](mailto:security@chappie.ai)
