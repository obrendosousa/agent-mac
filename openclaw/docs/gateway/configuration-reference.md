...
description: "Complete field-by-field reference for ~/.chappie/chappie.json"
summary: "Complete reference for every Chappie config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.chappie/chappie.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Chappie uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.chappie/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `chappie doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `chappie doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["chappie-dm"] },
      guilds: {
        "123456789012345678": {
...
- Chappie additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `chappie plugins install @chappie/mattermost`.
...
- `commands.callbackUrl` must resolve to the Chappie gateway endpoint and be reachable from the Mattermost server.
...
Chappie spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `chappie channels add` (or channel onboarding) while still on a single-account top-level channel config, Chappie moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `chappie doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@chappie", "chappie"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@chappie"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `chappie.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Chappie auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/chappie" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Chappie assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.chappie/sandboxes",
        docker: {
          image: "chappie-sandbox:bookworm-slim",
          containerPrefix: "chappie-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "chappie-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "chappie-sandbox-browser:bookworm-slim",
          network: "chappie-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.chappie/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `chappie.json`.
noVNC observer access uses VNC auth by default and Chappie emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `chappie-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from CHAPPIE_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `CHAPPIE_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `CHAPPIE_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `CHAPPIE_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.chappie/workspace",
        agentDir: "~/.chappie/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@chappie"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.chappie/workspace-home" },
      { id: "work", workspace: "~/.chappie/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Chappie resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.chappie/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.chappie/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.chappie/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.chappie/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Chappie starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.chappie/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Chappie treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:chappie`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.chappie/attachments/<uuid>/` with a `.manifest.json`.
...
Chappie uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.chappie/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `CHAPPIE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Chappie forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `chappie onboard --auth-choice opencode-zen` or `chappie onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `chappie onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `chappie onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `chappie onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `chappie onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `chappie onboard --auth-choice minimax-api`.
...
- Loaded from `~/.chappie/extensions`, `<workspace>/.chappie/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `chappie plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or CHAPPIE_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `CHAPPIE_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `CHAPPIE_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `CHAPPIE_APNS_RELAY_BASE_URL` / `CHAPPIE_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `CHAPPIE_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
CHAPPIE_CONFIG_PATH=~/.chappie/a.json \
CHAPPIE_STATE_DIR=~/.chappie-a \
chappie gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.chappie-dev` + port `19001`), `--profile <name>` (uses `~/.chappie-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.chappie/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-chappie-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `CHAPPIE_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.chappie/workspace/canvas",
    liveReload: true,
    // enabled: false, // or CHAPPIE_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__chappie__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__chappie__/a2ui/`
...
- Also serves A2UI at `/__chappie__/a2ui/`.
...
- Hostname defaults to `chappie`. Override with `CHAPPIE_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.chappie/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `chappie dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.chappie/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${CHAPPIE_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `chappie.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.chappie/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/chappie-vault-resolver",
...
- Legacy OAuth imports from `~/.chappie/credentials/oauth.json`.
...
- Default log file: `/tmp/chappie/chappie-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Chappie.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `CHAPPIE_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `chappie doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `chappie.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
