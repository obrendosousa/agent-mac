...
read_when:
  - Running Chappie behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Chappie
...
- You run Chappie behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Chappie checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Chappie extracts the user identity from the configured header
...
- Chappie can stay on loopback HTTP behind the proxy.
...
If Chappie itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://chappie.example.com
    to: http://chappie-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`chappie security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Chappie config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `chappie security audit` and review findings
