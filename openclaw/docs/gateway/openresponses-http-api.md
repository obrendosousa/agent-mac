...
Chappie’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`chappie agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "chappie:<agentId>"`, `model: "agent:<agentId>"`, or `x-chappie-agent-id`
- use `x-chappie-session-key` for explicit session routing
