...
Chappie supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `chappie gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.chappie/.env` so the daemon can read it:
...
API keys for daemon use: `chappie onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.chappie/.env`, systemd/launchd).
...
```bash
chappie models auth paste-token --provider anthropic
chappie models auth paste-token --provider openrouter
```
...
- Priority order:
  - `CHAPPIE_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Chappie retries with the next key only for rate-limit errors (for example
...
Run `chappie models status` to confirm which profile is expiring. If the profile
