...
read_when:
  - Setting up Chappie for the first time
...
Chappie reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.chappie/chappie.json`.

If the file is missing, Chappie uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `chappie onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    chappie onboard       # full setup wizard
    chappie configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.chappie/chappie.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Chappie only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`chappie doctor`, `chappie logs`, `chappie health`, `chappie status`)
- Run `chappie doctor` to see exact issues
- Run `chappie doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `chappie.json`.
    ...
    ```bash
    chappie config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `CHAPPIE_APNS_RELAY_BASE_URL` and `CHAPPIE_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `CHAPPIE_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.chappie/chappie.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `chappie config set` for single keys.
    </Warning>
    ...
    chappie gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.chappie/workspace\" } } }",
      "baseHash": "<hash>",
...
Chappie reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.chappie/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Chappie runs your login shell and imports only the missing keys:
...
Env var equivalent: `CHAPPIE_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${CHAPPIE_GATEWAY_TOKEN}" } },
