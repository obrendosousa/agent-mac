...
read_when:
  - You want to place an outbound voice call from Chappie
...
Voice calls for Chappie via a plugin. Supports outbound notifications and
...
- Use `chappie voicecall ...` or the `voice_call` tool
...
```bash
chappie plugins install ./extensions/voice-call
