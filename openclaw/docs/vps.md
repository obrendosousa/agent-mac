---
summary: "VPS hosting hub for Chappie (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `CHAPPIE_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `CHAPPIE_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/chappie-compile-cache`
