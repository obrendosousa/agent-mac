...
- When given a `sessionId`, Chappie resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Chappie runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Chappie runs the **agent‑to‑agent announce step** (target agent only):
...
- `attachments?` (optional array of inline files; subagent runtime only, ACP rejects). Each entry: `{ name, content, encoding?: "utf8" | "base64", mimeType? }`. Files are materialized into the child workspace at `.chappie/attachments/<uuid>/`. Returns a receipt with sha256 per file.
...
- After completion, Chappie runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, Chappie clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
