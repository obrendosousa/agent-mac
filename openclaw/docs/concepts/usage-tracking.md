...
- `/usage cost` in chats: local cost summary aggregated from Chappie session logs.
- CLI: `chappie status --usage` prints a full per-provider breakdown.
- CLI: `chappie channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
