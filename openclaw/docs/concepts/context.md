...
read_when:
  - You want to understand what “context” means in Chappie
...
“Context” is **everything Chappie sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Chappie-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Chappie builds the system prompt

The system prompt is **Chappie-owned** and rebuilt each run. It includes:
...
By default, Chappie injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). Chappie also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `150000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
...
By default, Chappie uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, Chappie delegates context
