---
summary: "Deploy Chappie Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Chappie on a Kubernetes cluster
  - You want to test Chappie in a Kubernetes environment
title: "Kubernetes"
---

# Chappie on Kubernetes

A minimal starting point for running Chappie on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.
...
kubectl port-forward svc/chappie 18789:18789 -n chappie
...
```bash
kubectl get secret chappie-secrets -n chappie -o jsonpath='{.data.CHAPPIE_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: chappie (configurable via CHAPPIE_NAMESPACE)
├── Deployment/chappie        # Single pod, init container + gateway
├── Service/chappie           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/chappie-config  # chappie.json + AGENTS.md
└── Secret/chappie-secrets    # Gateway token + API keys
```
...
Edit `chappie.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret chappie-secrets -n chappie \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/chappie -n chappie
```

### Custom namespace

```bash
CHAPPIE_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/chappie/chappie:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # chappie.json + AGENTS.md
