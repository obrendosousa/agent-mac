...
- `gateway`/`node` approvals are controlled by `~/.chappie/exec-approvals.json`.
...
- Chappie sets `CHAPPIE_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Chappie prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`chappie security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `chappie doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
