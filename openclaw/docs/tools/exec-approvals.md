...
- **gateway host** → `chappie` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Chappie also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.chappie/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.chappie/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `chappie approvals allowlist ...`).
- `chappie security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `chappie doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.chappie/exec-approvals.json` directly.

CLI: `chappie approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Chappie does not treat
...
topics, Chappie preserves the topic for the approval prompt and the post-approval follow-up.
