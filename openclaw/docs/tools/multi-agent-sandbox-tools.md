...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `chappie sandbox explain`.
...
Legacy `agent.*` configs are migrated by `chappie doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=chappie-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CHAPPIE_STATE_DIR:-$HOME/.chappie}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
