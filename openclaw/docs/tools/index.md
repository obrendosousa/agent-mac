---
summary: "Agent tool surface for Chappie (browser, canvas, nodes, message, cron) replacing legacy `chappie-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `chappie-*` skills
title: "Tools"
---

# Tools (Chappie)

Chappie exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `chappie-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `chappie.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Chappie logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:chappie`: all built-in Chappie tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`chappie node run`).
...
Chappie tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `chappie configure --section web`).
...
Control the dedicated Chappie-managed browser.
...
- Omit `profile` for the safe default: isolated Chappie-managed browser (`chappie`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `chappie`).
...
- Quick smoke: `chappie nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `chappie gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Chappie uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.chappie/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Chappie clamps `tools.sessions.visibility` to `tree`.
