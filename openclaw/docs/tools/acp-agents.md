...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Chappie run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Chappie in plain language to "run this in Codex" or "start Claude Code in a thread", Chappie should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Chappie-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Chappie native sub-agent runtime  |
...
- Chappie binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Chappie returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Chappie agent id.
...
- Chappie ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Chappie uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Chappie may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Chappie session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Chappie sandbox.
...
If no target resolves, Chappie returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Chappie returns a clear unsupported-control error.
...
When Chappie uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Chappie `agentId` path).
...
```bash
chappie plugins install acpx
chappie config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
chappie plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@chappie/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from Chappie workspace directory.
...
- Chappie startup remains non-blocking while the backend health check runs.
...
```bash
chappie config set plugins.entries.acpx.config.permissionMode approve-all
chappie config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Chappie currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
