...
Chappie ships two lightweight web tools:
...
- In auto-detect mode, Chappie resolves only the selected provider key. Non-selected provider SecretRefs stay inactive until selected.
- If the selected provider SecretRef is unresolved and no provider env fallback exists, startup/reload fails fast.

## Setting up web search

Use `chappie configure --section web` to set up your API key and choose a provider.

### Brave Search

1. Create a Brave Search API account at [brave.com/search/api](https://brave.com/search/api/)
2. In the dashboard, choose the **Search** plan and generate an API key.
3. Run `chappie configure --section web` to store the key in config, or set `BRAVE_API_KEY` in your environment.
...
3. Run `chappie configure --section web` to store the key in config, or set `PERPLEXITY_API_KEY` in your environment.
...
**Via config:** run `chappie configure --section web`. It stores the key under the provider-specific config path:
...
For a gateway install, put these in `~/.chappie/.env` (or your service environment). See [Env vars](/help/faq#how-does-chappie-load-environment-variables).
...
For a gateway install, put it in `~/.chappie/.env`.
