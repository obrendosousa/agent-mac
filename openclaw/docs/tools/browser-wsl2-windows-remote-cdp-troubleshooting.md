...
read_when:
  - Running Chappie Gateway in WSL2 while Chrome lives on Windows
...
- Chappie Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/chappie/chappie/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome-relay` profile plus the Chappie Chrome extension.
...
- Chappie points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Chappie is not the problem yet.
...
Fix that before touching Chappie config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Chappie at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Chappie to succeed
...
```bash
chappie browser open https://example.com --browser-profile remote
chappie browser tabs --browser-profile remote
```

For the extension relay:

```bash
chappie browser tabs --browser-profile chrome-relay
```

Good result:

- the tab opens in Windows Chrome
- `chappie browser tabs` returns the target
...
3. Chappie config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Chappie config or Control UI auth
