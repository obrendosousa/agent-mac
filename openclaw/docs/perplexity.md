...
Chappie supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Chappie also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `chappie configure --section web`. It stores the key in
`~/.chappie/chappie.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.chappie/.env` (or your service environment). See [Env vars](/help/faq#how-does-chappie-load-environment-variables).
