...
summary: "`chappie configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `chappie config` 会打开相同的向导。使用 `chappie config get|set|unset` 进行非交互式编辑。
...
```bash
chappie configure
chappie configure --section models --section channels
```
