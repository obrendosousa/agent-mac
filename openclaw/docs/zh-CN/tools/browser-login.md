...
当网站需要登录时，请在**主机**浏览器配置文件（chappie 浏览器）中**手动登录**。
...
Chappie 控制一个**专用的 Chrome 配置文件**（名为 `chappie`，橙色调 UI）。这与你的日常浏览器配置文件是分开的。
...
如果你有多个配置文件，传入 `--browser-profile <name>`（默认是 `chappie`）。

## X/Twitter：推荐流程

- **阅读/搜索/话题：** 使用 **bird** CLI Skills（无浏览器，稳定）。
  - 仓库：https://github.com/chappie-team/bird
- **发布更新：** 使用**主机**浏览器（手动登录）。
