...
你可以在 `~/.chappie/chappie.json` 中覆盖此设置：
...
所有日志配置都在 `~/.chappie/chappie.json` 的 `logging` 下。
...
- Chappie 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `chappie plugins enable diagnostics-otel` 启用插件。
...
- `chappie.tokens`（计数器，属性：`chappie.token`、`chappie.channel`、`chappie.provider`、`chappie.model`）
- `chappie.cost.usd`（计数器，属性：`chappie.channel`、`chappie.provider`、`chappie.model`）
- `chappie.run.duration_ms`（直方图，属性：`chappie.channel`、`chappie.provider`、`chappie.model`）
- `chappie.context.tokens`（直方图，属性：`chappie.context`、`chappie.channel`、`chappie.provider`、`chappie.model`）

消息流：

- `chappie.webhook.received`（计数器，属性：`chappie.channel`、`chappie.webhook`）
- `chappie.webhook.error`（计数器，属性：`chappie.channel`、`chappie.webhook`）
- `chappie.webhook.duration_ms`（直方图，属性：`chappie.channel`、`chappie.webhook`）
- `chappie.message.queued`（计数器，属性：`chappie.channel`、`chappie.source`）
- `chappie.message.processed`（计数器，属性：`chappie.channel`、`chappie.outcome`）
- `chappie.message.duration_ms`（直方图，属性：`chappie.channel`、`chappie.outcome`）

队列 + 会话：

- `chappie.queue.lane.enqueue`（计数器，属性：`chappie.lane`）
- `chappie.queue.lane.dequeue`（计数器，属性：`chappie.lane`）
- `chappie.queue.depth`（直方图，属性：`chappie.lane` 或 `chappie.channel=heartbeat`）
- `chappie.queue.wait_ms`（直方图，属性：`chappie.lane`）
- `chappie.session.state`（计数器，属性：`chappie.state`、`chappie.reason`）
- `chappie.session.stuck`（计数器，属性：`chappie.state`）
- `chappie.session.stuck_age_ms`（直方图，属性：`chappie.state`）
- `chappie.run.attempt`（计数器，属性：`chappie.attempt`）
...
- `chappie.webhook.error`
  - `chappie.channel`、`chappie.webhook`、`chappie.chatId`、`chappie.error`
- `chappie.message.processed`
  - `chappie.channel`、`chappie.outcome`、`chappie.chatId`、`chappie.messageId`、`chappie.sessionKey`、`chappie.sessionId`、`chappie.reason`
- `chappie.session.stuck`
  - `chappie.state`、`chappie.ageMs`、`chappie.queueDepth`、`chappie.sessionKey`、`chappie.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `chappie doctor`。
