...
read_when:
  - You need detailed behavior for chappie onboard
...
This page is the full reference for `chappie onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.chappie/chappie.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `chappie doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `chappie pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `chappie health`.
    - `chappie status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.chappie/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.chappie/agents/<agentId>/agent/auth-profiles.json`
...
`~/.chappie/credentials/oauth.json` (or `$CHAPPIE_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`chappie onboard`](/cli/onboard)
