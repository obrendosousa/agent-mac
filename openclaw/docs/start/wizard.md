...
The onboarding wizard is the **recommended** way to set up Chappie on macOS,
...
`chappie dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`chappie configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.chappie/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `chappie doctor` first.
</Note>
...
Use `chappie agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.chappie/workspace-<agentId>`.
...
- CLI command reference: [`chappie onboard`](/cli/onboard)
