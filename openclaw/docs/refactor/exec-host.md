...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.chappie/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.chappie/node.json` (node id + pairing token).
...
Path: `~/.chappie/exec-approvals.json`
...
- Unix socket at `~/.chappie/exec-approvals.sock` (0600).
