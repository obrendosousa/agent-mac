...
- Publish as `chappie/plugin-sdk` (or export from core under `chappie/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ChappiePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ChappieConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ChappieConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ChappieConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ChappieConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ChappieConfig): string;
  };
};
```
...
- Introduce `chappie/plugin-sdk`.
- Add `api.runtime` to `ChappiePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `chappieRuntime: ">=2026.2.0"`).
