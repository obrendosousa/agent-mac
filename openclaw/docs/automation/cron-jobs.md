...
- Jobs persist under `~/.chappie/cron/` so restarts don’t lose schedules.
...
- For upgrades, `chappie doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
chappie cron list
chappie cron run <job-id>
chappie cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.chappie/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `chappie cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Chappie applies a
...
If `delivery` is omitted for isolated jobs, Chappie defaults to `announce`.
...
- CLI equivalents: `chappie cron add --light-context ...` and `chappie cron edit --light-context`.
...
- Job store: `~/.chappie/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.chappie/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Chappie classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Chappie archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `chappie cron runs` and adjust retention before logs become large
...
chappie cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
chappie cron edit <jobId> --agent ops
chappie cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
chappie cron run <jobId>
chappie cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `chappie cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
chappie cron edit <jobId> \
...
```bash
chappie system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`chappie system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `CHAPPIE_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Chappie applies exponential retry backoff for recurring jobs after consecutive errors:
