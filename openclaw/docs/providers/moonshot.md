...
```bash
chappie onboard --auth-choice moonshot-api-key
```

Kimi Coding:

```bash
chappie onboard --auth-choice kimi-code-api-key
```
...
Chappie also maps runtime `/think` levels for Moonshot:

- `/think off` -> `thinking.type=disabled`
- any non-off thinking level -> `thinking.type=enabled`

When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Chappie normalizes incompatible `tool_choice` values to `auto` for compatibility.
