---
summary: "Use Venice AI privacy-focused models in Chappie"
read_when:
  - You want privacy-focused inference in Chappie
...
```bash
chappie onboard --auth-choice venice-api-key
```
...
```bash
chappie agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Chappie shows all available Venice models. Pick based on your needs:
...
```bash
chappie models set venice/kimi-k2-5
chappie models set venice/claude-opus-4-6
```

List all available models:

```bash
chappie models list | grep venice
```

## Configure via `chappie configure`

1. Run `chappie configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Chappie automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
chappie agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
chappie agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
chappie agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
chappie agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
chappie agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `chappie models list` to see currently available models. Some models may be temporarily offline.
