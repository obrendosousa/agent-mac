...
read_when:
  - You want to use Hugging Face Inference with Chappie
...
[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers) offer OpenAI-compatible chat completions through a single router API. You get access to many models (DeepSeek, Llama, and more) with one token. Chappie uses the **OpenAI-compatible endpoint** (chat completions only); for text-to-image, embeddings, or speech use the [HF inference clients](https://huggingface.co/docs/api-inference/quicktour) directly.
...
```bash
chappie onboard --auth-choice huggingface-api-key
```
...
is available to that process (for example, in `~/.chappie/.env` or via
`env.shellEnv`).

## Model discovery and onboarding dropdown

Chappie discovers models by calling the **Inference endpoint directly**:
