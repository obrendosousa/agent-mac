---
summary: "Use OpenCode Zen and Go catalogs with Chappie"
read_when:
...
OpenCode exposes two hosted catalogs in Chappie:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Chappie keeps the runtime provider ids
...
chappie onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
chappie onboard --auth-choice opencode-go
chappie onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
