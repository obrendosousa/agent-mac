---
summary: "Use OpenAI via API keys or Codex subscription in Chappie"
read_when:
  - You want to use OpenAI models in Chappie
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Chappie.
...
```bash
chappie onboard --auth-choice openai-api-key
# or non-interactive
chappie onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Chappie forwards both through the `openai/*` Responses path.
Chappie intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Chappie does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Chappie.
...
chappie onboard --auth-choice openai-codex

# Or run OAuth directly
chappie models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Chappie
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Chappie also supports:

- `openai-codex/gpt-5.3-codex-spark`

Chappie treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Chappie also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Chappie uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Chappie also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Chappie enables it by default for
...
Chappie, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Chappie exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Chappie applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Chappie now auto-enables OpenAI server-side
