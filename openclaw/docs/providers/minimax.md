---
summary: "Use MiniMax M2.5 in Chappie"
read_when:
  - You want MiniMax models in Chappie
...
```bash
chappie plugins enable minimax-portal-auth  # skip if already loaded.
chappie gateway restart  # restart if gateway is already running
chappie onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/chappie/chappie/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `chappie.json`:
...
- Use `chappie models list` and `chappie models set minimax/MiniMax-M2.5` to switch.
...
- Running `chappie configure` and selecting **MiniMax M2.5**, or
