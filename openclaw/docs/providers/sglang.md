---
summary: "Run Chappie with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Chappie against a local SGLang server
...
Chappie can connect to SGLang using the `openai-completions` API.

Chappie can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Chappie will query:
