...
This plan defines how Chappie should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Chappie core owns ACP control plane concerns
...
Chappie should not reimplement ACP transport internals in core.
Chappie should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Chappie, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Chappie state dir
...
- ACP event stream is normalized to Chappie reply chunks
...
For the first production backend (`extensions/acpx`), Chappie and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Chappie `sessionKey`)
...
- Chappie consumes ndjson events from `acpx --format json --json-strict`
