...
read_when:
  - Designing workspace memory (~/.chappie/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Chappie integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.chappie/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.chappie/workspace` as canonical human-readable memory.
...
```
~/.chappie/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Chappie**, but keep a separable core library.

### Why integrate into Chappie?

- Chappie already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `chappie memory recall "…" --k 25 --since 30d`
  - `chappie memory reflect --since 7d`
...
Pragmatic take for `~/.chappie/workspace`:
