...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/chappie`)
...
If the page fails to load, start the Gateway first: `chappie gateway`.
...
you revoke it with `chappie devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.chappie/chappie.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop chappie`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Chappie
...
Chappie **blocks** Control UI connections without device identity.
