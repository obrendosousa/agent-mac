...
```bash
chappie tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `chappie tui --deliver`
...
- Local shell commands receive `CHAPPIE_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `CHAPPIE_THEME=light` before launching `chappie tui`.
- To force the original dark palette instead, set `CHAPPIE_THEME=dark`.
...
- Check the Gateway logs: `chappie logs --follow`.
- Confirm the agent can run: `chappie status` and `chappie models status`.
...
- No agents in picker: check `chappie agents list` and your routing config.
