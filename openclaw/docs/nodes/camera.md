...
Chappie supports **camera capture** for agent workflows:
...
```bash
chappie nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
chappie nodes camera snap --node <id> --facing front
chappie nodes camera clip --node <id> --duration 3000
chappie nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`chappie.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `chappie` CLI to invoke camera commands on the macOS node.

Examples:

```bash
chappie nodes camera list --node <id>            # list camera ids
chappie nodes camera snap --node <id>            # prints MEDIA:<path>
chappie nodes camera snap --node <id> --max-width 1280
chappie nodes camera snap --node <id> --delay-ms 2000
chappie nodes camera snap --node <id> --device-id <id>
chappie nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
chappie nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
chappie nodes camera clip --node <id> --device-id <id>
chappie nodes camera clip --node <id> --no-audio
```

Notes:

- `chappie nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
chappie nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
