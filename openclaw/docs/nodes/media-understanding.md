...
Chappie can **summarize inbound media** (image/audio/video) before the reply pipeline runs. It auto‑detects when local tools or provider keys are available, and can be disabled or customized. If understanding is off, models still receive the original files/URLs as usual.
...
- If `<capability>.enabled: true` but no models are configured, Chappie tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, Chappie auto-detects in this order and **stops at the first
working option**:
...
When provider-based **audio** and **video** media understanding is enabled, Chappie
...
If the proxy value is malformed, Chappie logs a warning and falls back to direct
fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared
lists, Chappie can infer defaults:
...
## Provider support matrix (Chappie integrations)
...
- `parakeet-mlx` note: with `--output-dir`, Chappie reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
