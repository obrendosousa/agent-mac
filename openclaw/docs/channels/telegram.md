...
```
    ...
    Telegram does **not** use `chappie channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `chappie doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `chappie doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `chappie logs --follow`
...
- DM messages can carry `message_thread_id`; Chappie routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Chappie can stream partial replies in real time:
    ...
    - DM: Chappie keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Chappie keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Chappie falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Chappie skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Chappie automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Chappie retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Chappie pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.chappie/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Chappie enqueues system events like:
...
```bash
chappie message send --channel telegram --target 123456789 --message "hi"
chappie message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `chappie message poll` and support forum topics:

```bash
chappie message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
chappie message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Chappie preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `chappie channels status` warns when config expects unmentioned group messages.
    - `chappie channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `chappie logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Chappie now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `CHAPPIE_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `CHAPPIE_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `CHAPPIE_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `chappie doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `chappie doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, Chappie falls back to the first normalized account ID and `chappie doctor` warns.
