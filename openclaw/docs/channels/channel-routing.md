...
Chappie routes replies **back to the channel where a message came from**. The
...
Chappie infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Chappie still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Chappie would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.chappie/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.chappie`):

- `~/.chappie/agents/<agentId>/sessions/sessions.json`
