...
Tlon is a decentralized messenger built on Urbit. Chappie connects to your Urbit ship and can
...
```bash
chappie plugins install @chappie/tlon
```

Local checkout (when running from a git repo):

```bash
chappie plugins install ./extensions/tlon
```
...
By default, Chappie blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `chappie message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Chappie replies in-thread.
