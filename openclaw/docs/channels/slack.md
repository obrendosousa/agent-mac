...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Chappie HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `chappie pairing approve slack <code>`.
...
- The interactive callback values are Chappie-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Chappie falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Chappie is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Chappie",
    "description": "Slack connector for Chappie"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/chappie",
        "description": "Send a message to Chappie",
        "should_escape": false
      }
    ]
  },
...
Chappie supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Chappie falls back to normal delivery for remaining payloads.
