...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Chappie to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Chappie releases, so no separate plugin install
is required.
...
If you just installed Chappie, run the wizard:
...
2. Configuring app credentials in Chappie
...
1. You already ran `chappie channels add` for Feishu
2. The gateway is running (`chappie gateway status`)
...
```bash
chappie pairing approve feishu <CODE>
```
...
2. Run `chappie logs --follow` and look for `chat_id`
...
2. Run `chappie logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `chappie gateway status`  | Show gateway status           |
| `chappie gateway install` | Install/start gateway service |
| `chappie gateway stop`    | Stop gateway service          |
| `chappie gateway restart` | Restart gateway service       |
| `chappie logs --follow`   | Tail gateway logs             |
...
4. Check logs: `chappie logs --follow`
...
5. Ensure the gateway is running: `chappie gateway status`
6. Check logs: `chappie logs --follow`
