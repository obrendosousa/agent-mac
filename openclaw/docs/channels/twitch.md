...
read_when:
  - Setting up Twitch chat integration for Chappie
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Chappie connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
chappie plugins install @chappie/twitch
```

Local checkout (when running from a git repo):

```bash
chappie plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `CHAPPIE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "chappie", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CHAPPIE_TWITCH_ACCESS_TOKEN env var)
...
```bash
CHAPPIE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
