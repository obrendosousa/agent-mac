...
- Chappie installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Chappie"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Chappie and restart the gateway.
5. Send a first DM and approve pairing (`chappie pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Chappie"` then scan the QR in Signal.
...
4. Configure Chappie, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `chappie pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Chappie at it:
...
This skips auto-spawn and the startup wait inside Chappie. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `chappie pairing list signal`
  - `chappie pairing approve signal <CODE>`
...
- **Typing indicators**: Chappie sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Chappie forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `chappie doctor --fix`.
...
chappie pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/chappie/chappie-$(date +%Y-%m-%d).log" | tail -20
```
