---
summary: "Mattermost bot setup and Chappie config"
read_when:
...
```bash
chappie plugins install @chappie/mattermost
```

Local checkout (when running from a git repo):

```bash
chappie plugins install ./extensions/mattermost
```
...
4. Configure Chappie and start the gateway.
...
Native slash commands are opt-in. When enabled, Chappie registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Chappie derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Chappie.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Chappie.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Chappie, not `404`.
...
- Approve via:
  - `chappie pairing list mattermost`
  - `chappie pairing approve mattermost <CODE>`
...
Use these target formats with `chappie message send` or cron/webhooks:
...
Chappie resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Chappie sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, Chappie derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Chappie run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="chappie-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"chappie-mattermost-interactions",
...
`chappie message send` and cron/webhook deliveries.
