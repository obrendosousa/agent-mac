...
read_when:
  - Setting up Zalo Personal for Chappie
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Chappie.
...
- Install via CLI: `chappie plugins install @chappie/zalouser`
- Or from a source checkout: `chappie plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `chappie channels login --channel zalouser`
...
```bash
chappie directory self --channel zalouser
chappie directory peers list --channel zalouser --query "name"
chappie directory groups list --channel zalouser --query "work"
```
...
- `chappie pairing list zalouser`
- `chappie pairing approve zalouser <code>`
...
- On startup, Chappie resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Chappie stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Chappie state. Example:
...
- Chappie sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Chappie sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `chappie channels logout --channel zalouser && chappie channels login --channel zalouser`
...
- The channel now runs fully in Chappie without external CLI binaries.
