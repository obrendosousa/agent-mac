...
Matrix is an open, decentralized messaging protocol. Chappie connects as a Matrix **user**
...
```bash
chappie plugins install @chappie/matrix
```

Local checkout (when running from a git repo):

```bash
chappie plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `chappie plugins install @chappie/matrix`
   - From a local checkout: `chappie plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Chappie calls the same
     login endpoint, stores the access token in `~/.chappie/credentials/matrix/credentials.json`,
...
- On first connection, Chappie requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Chappie logs a warning.
...
`~/.chappie/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `chappie pairing list matrix`
  - `chappie pairing approve matrix <CODE>`
...
- On startup, Chappie resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
