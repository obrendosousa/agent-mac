...
```bash
chappie plugins install @chappie/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
chappie plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Chappie" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `chappie pairing list nextcloud-talk`
  - `chappie pairing approve nextcloud-talk <CODE>`
