---
summary: "How Chappie builds prompt context and reports token usage + costs"
read_when:
...
Chappie tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Chappie assembles its own system prompt on every run. It includes:
...
For images, Chappie downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Chappie session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `chappie status --usage` and `chappie channels list` show
...
`cacheWrite`. If pricing is missing, Chappie shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Chappie can
...
Anthropic's 1M context window is currently beta-gated. Chappie can inject the
...
Chappie skips the `context-1m-*` beta header because Anthropic currently
