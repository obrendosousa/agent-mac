...
Chappie integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Chappie owns lifecycle when `channels.signal.autoStart=true`.
...
- Chappie spawns `imsg rpc` as a child process (legacy iMessage integration).
