...
- With Anthropic API-key auth profiles, Chappie seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Chappie injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Chappie exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.chappie/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$CHAPPIE_STATE_DIR/logs/cache-trace.jsonl`
...
- `CHAPPIE_CACHE_TRACE=1` enables cache tracing.
- `CHAPPIE_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `CHAPPIE_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `CHAPPIE_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `CHAPPIE_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
