...
This is the full reference for the `chappie onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.chappie/credentials/oauth.json`; auth profiles live in `~/.chappie/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.chappie/credentials/oauth.json` (or `$CHAPPIE_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.chappie/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `chappie pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `chappie configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `chappie health`.
    - Tip: `chappie status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export CHAPPIE_GATEWAY_TOKEN="your-token"
chappie onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env CHAPPIE_GATEWAY_TOKEN
```
...
- Stores it under `~/.chappie/tools/signal-cli/<version>/`.
...
Typical fields in `~/.chappie/chappie.json`:
...
`chappie agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.chappie/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.chappie/agents/<agentId>/sessions/`.
