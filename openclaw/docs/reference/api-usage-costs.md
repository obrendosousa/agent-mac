...
Chappie features that can generate provider usage or paid API calls.
...
- `chappie status --usage` and `chappie channels list` show provider **usage windows**
...
Chappie can pick up credentials from:
...
- `chappie status --usage`
- `chappie models status --json`
...
`chappie models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
