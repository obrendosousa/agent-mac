...
This document explains how Chappie manages sessions end-to-end:
...
Chappie is designed around a single **Gateway process** that owns session state.
...
Chappie persists sessions in two layers:
...
- Transcripts: `~/.chappie/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Chappie resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Chappie reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.chappie/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Chappie intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Chappie consumes the events, but Pi decides when to compact).
...
Chappie also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Chappie bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Chappie leaves it alone.
...
- `chappie status` (CLI)
- `chappie sessions` / `sessions --json`
...
Chappie supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Chappie strips/suppresses this in the delivery layer.

As of `2026.1.10`, Chappie also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Chappie uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Chappie’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `chappie status`.
