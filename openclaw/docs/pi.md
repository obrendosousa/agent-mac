...
summary: "Architecture of Chappie's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Chappie
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Chappie integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Chappie uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Chappie directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Chappie's local TUI mode)                                             |
...
├── pi-tools.ts                    # createChappieCodingTools()
...
├── chappie-tools.ts              # Chappie-specific tools
...
Image injection is prompt-local: Chappie loads image refs from the current prompt and
...
2. **Custom Replacements**: Chappie replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Chappie Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Chappie's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Chappie CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Chappie wraps this with `guardSessionManager()` for tool result safety.
...
Chappie maintains an auth profile store with multiple API keys per provider:
...
Chappie loads custom pi extensions for specialized behavior:
...
Chappie also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Chappie Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Chappie tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.chappie/agents/<agentId>/sessions/` (or `$CHAPPIE_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `CHAPPIE_LIVE_TEST=1`)
