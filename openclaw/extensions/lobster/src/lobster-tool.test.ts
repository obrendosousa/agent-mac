...
import type { ChappiePluginApi, ChappiePluginToolContext } from "chappie/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<ChappiePluginApi> = {}): ChappiePluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<ChappiePluginToolContext> = {}): ChappiePluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ChappiePluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
