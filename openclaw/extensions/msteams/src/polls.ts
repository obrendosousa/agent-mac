...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["chappiePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["chappie", "pollId"]) ??
    readNestedString(value, ["chappie", "poll", "id"]) ??
    readNestedString(value, ["data", "chappiePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "chappie", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          chappiePollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "chappie poll vote",
          displayText: "Vote recorded",
          value: { chappiePollId: pollId, pollId },
        },
      },
    ],
  };
