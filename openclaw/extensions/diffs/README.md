...
Read-only diff viewer plugin for **Chappie** agents.
...
Set plugin-wide defaults in `~/.chappie/chappie.json`:
...
Before:
Chappie supports plugins.

After:
Chappie supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/chappie-diffs`).
