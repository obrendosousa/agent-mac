...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-chappie-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-chappie-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.chappieDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.chappieDiffsError = "true";
