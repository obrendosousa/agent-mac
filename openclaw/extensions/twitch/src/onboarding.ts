...
import {
  ...
  type WizardPrompter,
} from "chappie/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: CHAPPIE_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: ChappieConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var CHAPPIE_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.CHAPPIE_TWITCH_ACCESS_TOKEN?.trim();
