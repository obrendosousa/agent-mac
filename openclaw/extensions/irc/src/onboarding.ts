  ...
  configure: async ({
    ...
    if (useEnv) {
      ...
      const username = String(
        await prompter.text({
          ...
          initialValue: resolved.config.username || nick || "chappie",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const realname = String(
        await prompter.text({
          ...
          initialValue: resolved.config.realname || "Chappie",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
    ...
    await prompter.note(
      [
        ...
        "Command: chappie channels status --probe",
        `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
      ].join("\n"),
      "IRC next steps",
    );
