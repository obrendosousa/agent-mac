...
function resolveChappieRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.CHAPPIE_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set CHAPPIE_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveChappieRoot(), "dist", "extensionAPI.js");
