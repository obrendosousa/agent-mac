...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/chappie-discord-process-test-sessions.json"),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/chappie-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/chappie-discord-process-test-sessions.json" },
      },
    });
