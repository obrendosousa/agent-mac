...
`chappie.invoke --each`) without adding new Chappie code per workflow.
...
This extension depends on Chappie internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Chappie extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.chappie/extensions` as a standalone plugin directory.
