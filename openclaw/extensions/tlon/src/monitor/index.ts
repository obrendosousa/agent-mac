import type { RuntimeEnv, ReplyPayload, ChappieConfig } from "chappie/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "chappie/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as ChappieConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in Chappie config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your Chappie config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.chappie.ai/concepts/session#secure-dm-mode`;
