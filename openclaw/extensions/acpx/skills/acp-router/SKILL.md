---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either Chappie ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
...
Trigger this skill when the user asks Chappie to:
...
- After reading, follow `Chappie ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. Chappie ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no Chappie ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@chappie/acpx` extension.
