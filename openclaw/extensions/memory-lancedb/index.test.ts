...
const liveEnabled = HAS_OPENAI_KEY && process.env.CHAPPIE_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "chappie-memory-test-" });
...
describeLive("memory plugin live tests", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "chappie-memory-live-" });
