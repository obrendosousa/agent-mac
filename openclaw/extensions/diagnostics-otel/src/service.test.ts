...
vi.mock("chappie/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("chappie/plugin-sdk/diagnostics-otel")>(
    "chappie/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { ChappiePluginServiceContext } from "chappie/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "chappie/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/chappie-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): ChappiePluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("chappie.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chappie.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("chappie.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("chappie.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chappie.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("chappie.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("chappie.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chappie.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("chappie.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("chappie.webhook.processed");
    expect(spanNames).toContain("chappie.message.processed");
    expect(spanNames).toContain("chappie.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["chappie.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("chappie.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "chappie.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["chappie.reason"]).toBe("string");
    expect(String(attrs?.["chappie.reason"])).not.toContain(
