import type {
  ...
  ChappieConfig,
} from "chappie/plugin-sdk/googlechat";
import {
  createActionGate,
  extractToolSend,
  jsonResult,
...
function listEnabledAccounts(cfg: ChappieConfig) {
  return listEnabledGoogleChatAccounts(cfg).filter(
    (account) => account.enabled && account.credentialSource !== "none",
  );
}

function isReactionsEnabled(accounts: ReturnType<typeof listEnabledAccounts>, cfg: ChappieConfig) {
