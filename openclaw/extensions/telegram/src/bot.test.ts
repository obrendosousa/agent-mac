...
describe("createTelegramBot", () => {
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/chappie-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/chappie-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Chappie" },
        },
      },
      me: { id: 999, username: "chappie_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
