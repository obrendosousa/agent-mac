...
import type { ChappiePluginApi, ChappiePluginService } from "chappie/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: ChappiePluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ChappiePluginApi["config"]): string[] {
...
export default function register(api: ChappiePluginApi) {
  ...
  const timerService: ChappiePluginService = {
