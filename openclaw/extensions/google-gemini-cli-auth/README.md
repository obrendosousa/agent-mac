# Google Gemini CLI Auth (Chappie plugin)
...
```bash
chappie plugins enable google-gemini-cli-auth
```
...
```bash
chappie models auth login --provider google-gemini-cli --set-default
```
...
- `CHAPPIE_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `CHAPPIE_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
