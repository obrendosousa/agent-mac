...
vi.mock("chappie/plugin-sdk/google-gemini-cli-auth", () => ({
  isWSL2Sync: () => false,
...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "CHAPPIE_GEMINI_OAUTH_CLIENT_ID",
    "CHAPPIE_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.CHAPPIE_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.CHAPPIE_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@chappie.ai" });
      }
      if (url === LOAD_PROD) {
