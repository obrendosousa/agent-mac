...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CHAPPIE_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-nostr-"));
  process.env.CHAPPIE_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.CHAPPIE_STATE_DIR?.trim() || stateEnv.CHAPPIEDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".chappie");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CHAPPIE_STATE_DIR = previous;
    }
