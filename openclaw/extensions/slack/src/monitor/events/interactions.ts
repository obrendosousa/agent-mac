...
// Prefix for Chappie-generated action IDs to scope our handler
const CHAPPIE_ACTION_PREFIX = "chappie:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${CHAPPIE_ACTION_PREFIX}`);

  // Handle Chappie modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
