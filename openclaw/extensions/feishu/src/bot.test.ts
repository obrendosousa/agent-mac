import type { ChappiedbotConfig, PluginRuntime, RuntimeEnv } from "chappie/plugin-sdk/feishu";
...
async function dispatchMessage(params: { cfg: ChappiedbotConfig; event: FeishuMessageEvent }) {
...
describe("resolveBroadcastAgents", () => {
  it("returns agent list when broadcast config has the peerId", () => {
    const cfg = { broadcast: { oc_group123: ["susan", "main"] } } as unknown as ChappiedbotConfig;
  ...
  it("returns null when peerId not in broadcast", () => {
    const cfg = { broadcast: { oc_other: ["susan"] } } as unknown as ChappiedbotConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });

  it("returns null when agent list is empty", () => {
    const cfg = { broadcast: { oc_group123: [] } } as unknown as ChappiedbotConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });
});
...
describe("broadcast dispatch", () => {
  ...
  it("dispatches to all broadcast agents when bot is mentioned", async () => {
    ...
    } as unknown as ChappiedbotConfig;
