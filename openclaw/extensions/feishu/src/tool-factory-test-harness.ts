import type { AnyAgentTool, ChappiePluginApi } from "chappie/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: ChappiePluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<ChappiePluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
