...
import { withTempDownloadPath, type ChappiedbotConfig } from "chappie/plugin-sdk/feishu";
...
function createConfiguredFeishuMediaClient(params: { cfg: ChappiedbotConfig; accountId?: string }): {
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "chappie-feishu-img-",
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "chappie-feishu-resource-",
