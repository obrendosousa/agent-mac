...
import type { ChappieConfig, PluginRuntime } from "chappie/plugin-sdk/feishu";
import type { DynamicAgentCreationConfig } from "./types.js";

export type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: ChappieConfig;
  agentId?: string;
};
...
export async function maybeCreateDynamicAgent(params: {
  ...
  if (existingAgent) {
    ...
    const updatedCfg: ChappieConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.chappie/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.chappie/agents/{agentId}/agent";
