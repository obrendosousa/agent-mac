...
import type { ChappiePluginApi } from "chappie/plugin-sdk/device-pair";
import {
...
} from "chappie/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: ChappiePluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.CHAPPIE_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.CHAPPIEDBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuthLabel(cfg: ChappiePluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.CHAPPIE_GATEWAY_TOKEN,
      process.env.CHAPPIEDBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.CHAPPIE_GATEWAY_PASSWORD,
      process.env.CHAPPIEDBOT_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: ChappiePluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Chappie iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Chappie iOS app:",
            "",
            "```",
            qrAscii,
            "```",
