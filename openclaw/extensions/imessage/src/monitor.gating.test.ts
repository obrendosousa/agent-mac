...
function baseCfg(): ChappieConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
function buildDispatchContextPayload(params: { cfg: ChappieConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@chappie hello",
      bodyText: "@chappie hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
