...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    const previousStateDir = process.env.CHAPPIE_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies ChappieConfig;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.CHAPPIE_STATE_DIR = previousStateDir;
      }
