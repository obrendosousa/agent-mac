...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__chappie_channel_id";
...
type InteractionCallbackConfig = Pick<ChappieConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "chappie-mattermost-interactions").update(botToken).digest("hex");
}
