...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredChappieTmpDir(), "chappie-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub CHAPPIE_STATE_DIR.
    // Also keep it outside the Chappie temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["CHAPPIE_STATE_DIR"]);
    process.env.CHAPPIE_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "chappie-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredChappieTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Chappie state workspace and sandbox roots", async () => {
  ...
  it("rejects default Chappie state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
