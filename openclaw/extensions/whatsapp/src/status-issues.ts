    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("chappie channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (running && !connected) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("chappie doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
      }
    },
  });
