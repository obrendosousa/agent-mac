...
describe("web processMessage inbound contract", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "chappie-process-message-"));
  ...
  it("does not force an [chappie] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("../../../../../src/config/config.js").loadConfig>);
