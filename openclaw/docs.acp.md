...
This document describes how the Chappie ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`chappie acp` exposes an ACP agent over stdio and forwards prompts to a running
Chappie Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Chappie Gateway
...
- ACP client spawns `chappie acp` and speaks ACP messages over stdio.
...
`chappie acp` resolves the Gateway URL and auth from CLI flags or config:
