...
process.env.CHAPPIE_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime Chappie modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: ChappieConfig) => {
    ...
    resolveAccount: (cfg: ChappieConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: ChappieConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
