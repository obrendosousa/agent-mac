...
metadata:
  {
    "chappie": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in Chappie's own project folder!**
...
8. **NEVER start Codex in ~/.chappie/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/chappie/** - that's the LIVE Chappie instance!
...
For long-running background tasks, append a wake trigger to your prompt so Chappie gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
chappie system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: chappie system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
