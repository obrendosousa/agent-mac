---
name: node-connect
description: Diagnose Chappie node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Chappie is advertising that route, then fix pairing/auth.
...
- whether `chappie devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `chappie qr --json`. It uses the same setup-code payload Android scans.

```bash
chappie config get gateway.mode
chappie config get gateway.bind
chappie config get gateway.tailscale.mode
chappie config get gateway.remote.url
chappie config get gateway.auth.mode
chappie config get gateway.auth.allowTailscale
chappie config get plugins.entries.device-pair.config.publicUrl
chappie qr --json
chappie devices list
chappie nodes status
```

If this Chappie instance is pointed at a remote gateway, also run:

```bash
chappie qr --remote --json
```
...
If `chappie qr --json` says `Gateway is only bound to loopback`:
...
```bash
chappie devices list
chappie devices approve --latest
```
...
- `chappie devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run chappie qr again, rescan, then approve the pending device pairing.`
