---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Chappie deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Chappie cron scheduling for periodic checks, or version status checks on a machine running Chappie (laptop, workstation, Pi, VPS).
---

# Chappie Host Hardening

## Overview

Assess and harden the host running Chappie, then align it to a user-defined risk tolerance without breaking access. Use Chappie security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Chappie changes the host firewall, SSH, or OS updates; it does not.
...
5. Chappie gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Chappie can access sensitive data.
...
### 2) Run Chappie security audits (read-only)

As part of the default read-only checks, run `chappie security audit --deep`. Only offer alternatives if the user requests them:

1. `chappie security audit` (faster, non-probing)
2. `chappie security audit --json` (structured output)

Offer to apply Chappie safe defaults (numbered):

1. `chappie security audit --fix`

Be explicit that `--fix` only tightens Chappie defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Chappie version/update status (read-only)

As part of the default read-only checks, run `chappie update status`.
...
- Credential hygiene notes (location of Chappie creds, prefer disk encryption)
...
After Chappie install or first hardening pass, run at least one baseline audit and version check:

- `chappie security audit`
- `chappie security audit --deep`
- `chappie update status`

Ongoing monitoring is recommended. Use the Chappie cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `chappie cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `chappie update status`
...
Before creating, `chappie cron list` and match on exact `name`. If found, `chappie cron edit <id> ...`.
If not found, `chappie cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `chappie update status` (preferred for source checkouts and channels)
2. `npm view chappie version` (published npm version)

## Chappie command accuracy

Use only supported commands and flags:

- `chappie security audit [--deep] [--fix] [--json]`
- `chappie status` / `chappie status --deep`
- `chappie health --json`
- `chappie update status`
- `chappie cron add|list|runs|run`

Do not invent CLI flags or imply Chappie enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Chappie compaction:
