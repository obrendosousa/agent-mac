...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> ChappieChatHistoryPayload
{
    ChappieChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> ChappieChatSessionEntry {
    ChappieChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> ChappieChatSessionEntry
{
    ChappieChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> ChappieChatModelChoice {
    ChappieChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [ChappieChatHistoryPayload],
    sessionsResponses: [ChappieChatSessionsListResponse] = [],
    modelResponses: [[ChappieChatModelChoice]] = [],
    ...
    -> (TestChatTransport, ChappieChatViewModel)
{
...
private func sendUserMessage(_ vm: ChappieChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, ChappieChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ChappieChatHistoryPayload]
    private let sessionsResponses: [ChappieChatSessionsListResponse]
    private let modelResponses: [[ChappieChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<ChappieChatTransportEvent>
    private let continuation: AsyncStream<ChappieChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<ChappieChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ChappieChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ChappieChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [ChappieChatAttachmentPayload]) async throws -> ChappieChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return ChappieChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ChappieChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ChappieChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ChappieChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = ChappieChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = ChappieChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: ChappieChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = ChappieChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ChappieChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(ChappieChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == ChappieChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = ChappieChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ChappieChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = ChappieChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = ChappieChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = ChappieChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { ChappieChatViewModel(sessionKey: "main", transport: transport) }
