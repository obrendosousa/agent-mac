...
struct ChatMessageBubble: View {
    let message: ChappieChatMessage
    let style: ChappieChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ChappieChatTheme.userText : ChappieChatTheme.assistantText
    ...
    private var inlineAttachments: [ChappieChatMessageContent] {
    ...
    private var toolCalls: [ChappieChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ChappieChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ChappieChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ChappieChatTheme.onboardingAssistantBubble
        }
        return ChappieChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ChappieChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ChappieChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ChappieChatTheme.userText : ChappieChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ChappieChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ChappieChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ChappieChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ChappieChatTheme.assistantText)
            }
        }
    }
}
