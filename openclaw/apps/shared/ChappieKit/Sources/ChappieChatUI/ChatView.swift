...
@MainActor
public struct ChappieChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: ChappieChatViewModel
    ...
    public init(
        viewModel: ChappieChatViewModel,
    ...
    private var visibleMessages: [ChappieChatMessage] {
        let base: [ChappieChatMessage]
    ...
    private func mergeToolResults(in messages: [ChappieChatMessage]) -> [ChappieChatMessage] {
    ...
    private func isToolResultMessage(_ message: ChappieChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: ChappieChatMessage) -> Bool {
    ...
    private func primaryText(in message: ChappieChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: ChappieChatMessage) -> Bool {
    ...
    private func toolCalls(in message: ChappieChatMessage) -> [ChappieChatMessageContent] {
    ...
    private func inlineToolResults(in message: ChappieChatMessage) -> [ChappieChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: ChappieChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ChappieChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(ChappieChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
