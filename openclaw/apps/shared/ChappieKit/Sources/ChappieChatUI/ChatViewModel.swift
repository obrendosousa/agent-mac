...
private let chatUILogger = Logger(subsystem: "ai.chappie", category: "ChappieChatUI")

@MainActor
@Observable
public final class ChappieChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [ChappieChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [ChappieChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [ChappiePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ChappieChatPendingToolCall] = []
    public private(set) var sessions: [ChappieChatSessionEntry] = []
    private let transport: any ChappieChatTransport
    private var sessionDefaults: ChappieChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: ChappieChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any ChappieChatTransport,
    ...
    public var sessionChoices: [ChappieChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [ChappieChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ChappiePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ChappieChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ChappieChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ChappieChatMessage) -> ChappieChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ChappieChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ChappieChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: ChappieChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ChappieChatMessage],
        incoming: [ChappieChatMessage]) -> [ChappieChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ChappieChatMessage]) -> [ChappieChatMessage] {
        var result: [ChappieChatMessage] = []
    ...
    private static func dedupeKey(for message: ChappieChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ChappieChatMessageContent] = [
            ChappieChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ChappieChatAttachmentPayload in
            ChappieChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ChappieChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = ChappieChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: ChappieChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ChappieChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ChappieAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ChappieChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ChappiePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
