...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_chappie-gw._tcp", domain: "local.", interface: nil)
    ...
    private func clearTLSFingerprint(stableID: String) {
        let suite = UserDefaults(suiteName: "ai.chappie.shared") ?? .standard
    ...
    @Test @MainActor func manualConnectionsForceTLSForNonLoopbackHosts() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "chappie.local", useTLS: false) == true)
