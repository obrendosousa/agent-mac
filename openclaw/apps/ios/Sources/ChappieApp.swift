...
@MainActor
final class ChappieAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.chappie.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.chappie.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.chappie.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "chappie.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "chappie.watch.promptId"
    static let sessionKeyKey = "chappie.watch.sessionKey"
    static let actionPrimaryIDKey = "chappie.watch.action.primary.id"
    static let actionPrimaryLabelKey = "chappie.watch.action.primary.label"
    static let actionSecondaryIDKey = "chappie.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "chappie.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "chappie.watch.action.primary"
    static let actionSecondaryIdentifier = "chappie.watch.action.secondary"
    static let actionIdentifierPrefix = "chappie.watch.action."
    static let actionIDKeyPrefix = "chappie.watch.action.id."
    static let actionLabelKeyPrefix = "chappie.watch.action.label."
    static let categoryPrefix = "chappie.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ChappieWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ChappieWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ChappieWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Chappie" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ChappieWatchAction]) -> [UNNotificationAction] {
...
struct ChappieApp: App {
    ...
    @UIApplicationDelegateAdaptor(ChappieAppDelegate.self) private var appDelegate
...
extension ChappieApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Chappie: installing uncaught exception handler")
