...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__CHAPPIE_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ChappieTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.chappie", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ChappieTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ChappieTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ChappieTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ChappieTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ChappieTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ChappieTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ChappieTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ChappieTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ChappieTalkPTTStopPayload) {
