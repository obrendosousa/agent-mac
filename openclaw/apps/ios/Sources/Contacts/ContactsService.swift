...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ChappieContactsSearchParams) async throws -> ChappieContactsSearchPayload {
        ...
        return ChappieContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ChappieContactsAddParams) async throws -> ChappieContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ChappieContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ChappieContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ChappieContactPayload {
        ChappieContactPayload(
