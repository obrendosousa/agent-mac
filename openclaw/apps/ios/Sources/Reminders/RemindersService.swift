...
final class RemindersService: RemindersServicing {
    func list(params: ChappieRemindersListParams) async throws -> ChappieRemindersListPayload {
        ...
        let payload: [ChappieReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return ChappieRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ChappieRemindersAddParams) async throws -> ChappieRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ChappieReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ChappieRemindersAddPayload(reminder: payload)
    }
