...
final class MotionService: MotionServicing {
    func activities(params: ChappieMotionActivityParams) async throws -> ChappieMotionActivityPayload {
        ...
        let mapped: [ChappieMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ChappieMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ChappiePedometerParams) async throws -> ChappiePedometerPayload {
        ...
        let payload: ChappiePedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ChappiePedometerPayload(
