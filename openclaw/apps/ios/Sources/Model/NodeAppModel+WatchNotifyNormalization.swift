...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: ChappieWatchNotifyParams) -> ChappieWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [ChappieWatchAction]?,
        kind: String?,
        promptId: String?) -> [ChappieWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> ChappieWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ChappieWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                ChappieWatchAction(id: "approve", label: "Approve"),
                ChappieWatchAction(id: "decline", label: "Decline", style: "destructive"),
                ChappieWatchAction(id: "open_phone", label: "Open iPhone"),
                ChappieWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            ChappieWatchAction(id: "done", label: "Done"),
            ChappieWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            ChappieWatchAction(id: "open_phone", label: "Open iPhone"),
            ChappieWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: ChappieWatchRisk?,
        priority: ChappieNotificationPriority?) -> ChappieWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: ChappieNotificationPriority?,
        risk: ChappieWatchRisk?) -> ChappieNotificationPriority?
    {
