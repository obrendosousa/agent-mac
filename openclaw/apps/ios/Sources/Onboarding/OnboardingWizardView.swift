...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your Chappie chat, run /pair qr, then scan the code here."
    ...
    private var introStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Turn this iPhone into a secure Chappie node for chat, voice, camera, and device tools.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Label("Use Chappie from your phone", systemImage: "message.fill")
            }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        .font(.headline)
                    Text(
                        "The connected Chappie agent can use device capabilities you enable, such as camera, microphone, photos, contacts, calendar, and location. Continue only if you trust the gateway and agent you connect to.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Scan a QR code from your Chappie gateway or continue with manual setup.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 32)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `chappie devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `chappie devices approve` (or `chappie devices approve <requestId>`)\n"
                            + "2) `/pair approve` in your Chappie chat\n"
                            + "\(requestLine)\n"
                            + "Chappie will also retry automatically when you return to this app.")
                }
            }
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your Chappie chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "chappie.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your Chappie chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "chappie.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "chappie.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "chappie.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
