...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.chappie.ios", category: "LiveActivity")
    private var currentActivity: Activity<ChappieActivityAttributes>?
    ...
    func startActivity(agentName: String, sessionKey: String) {
        ...
        let attributes = ChappieActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<ChappieActivityAttributes>.activities
    ...
    private func updateCurrent(state: ChappieActivityAttributes.ContentState) {
    ...
    private func connectingState() -> ChappieActivityAttributes.ContentState {
        ChappieActivityAttributes.ContentState(
    ...
    private func idleState() -> ChappieActivityAttributes.ContentState {
        ChappieActivityAttributes.ContentState(
            statusText: "Idle",
            isIdle: true,
            isDisconnected: false,
    ...
    private func disconnectedState() -> ChappieActivityAttributes.ContentState {
        ChappieActivityAttributes.ContentState(
