# Chappie iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Chappie Gateway as a `role: node`.
...
- Beta release uses canonical `ai.chappie.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `ChappiePushTransport=relay`, `ChappiePushDistribution=official`, and `ChappiePushAPNsEnvironment=production`.
...
- `CHAPPIE_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/Chappie.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `ChappiePushTransport=direct` and `ChappiePushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `ChappiePushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `chappie.json`. `CHAPPIE_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Chappie official builds and to ensure a
