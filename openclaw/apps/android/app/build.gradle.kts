...
val androidStoreFile = providers.gradleProperty("CHAPPIE_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("CHAPPIE_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("CHAPPIE_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("CHAPPIE_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set CHAPPIE_ANDROID_STORE_FILE, " +
            "CHAPPIE_ANDROID_STORE_PASSWORD, CHAPPIE_ANDROID_KEY_ALIAS, and " +
            "CHAPPIE_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/ChappieKit/Sources/ChappieKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.chappie.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "chappie-$versionName-$buildType.apk"
