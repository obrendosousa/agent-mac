  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
    ...
    val server =
      startGatewayServer(json) { webSocket, id, method, frame ->
        when (method) {
          "connect" -> {
            webSocket.send(connectResponseFrame(id, canvasHostUrl = "http://127.0.0.1/__chappie__/cap/old-cap"))
          }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__chappie__/cap/new-cap",
