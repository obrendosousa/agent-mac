...
private const val NOTIFICATION_CHANNEL_BASE_ID = "chappie.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Chappie Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Chappie Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Chappie Active")
      }
