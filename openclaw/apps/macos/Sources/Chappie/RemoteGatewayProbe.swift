...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        switch self {
        case .tokenRequired:
            "Paste the token configured on the gateway host. On the gateway host, run `chappie config get gateway.auth.token`. If the gateway uses an environment variable instead, use `CHAPPIE_GATEWAY_TOKEN`."
        case .tokenMismatch:
            "Check `gateway.auth.token` or `CHAPPIE_GATEWAY_TOKEN` on the gateway host and try again."
        case .gatewayTokenNotConfigured:
            "This gateway is set to token auth, but no `gateway.auth.token` is configured on the gateway host. If the gateway uses an environment variable instead, set `CHAPPIE_GATEWAY_TOKEN` before starting the gateway."
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired Chappie client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired Chappie client. In your Chappie chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with `chappie doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired Chappie client yet, approve the pending request on the gateway host with `chappie devices approve`."
    ...
    var statusMessage: String {
        ...
        case .tokenMismatch:
            "Gateway token mismatch. Check gateway.auth.token or CHAPPIE_GATEWAY_TOKEN on the gateway host."
        case .gatewayTokenNotConfigured:
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired Chappie client, run /pair approve, then check the connection again."
        }
    }
}
