...
enum ChappieLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ChappieOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ChappieFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ChappieLogging.bootstrapIfNeeded()
        let label = ChappieLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ChappieOSLogHandler: AppLogLevelBackedHandler {
...
struct ChappieFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ChappieLogging.parseLabel(self.label)
