...
private let webChatSwiftLogger = Logger(subsystem: "ai.chappie", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "chappie.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: ChappieChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> ChappieChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(ChappieChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            ChappieChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return ChappieChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ChappieChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ChappieGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: ChappieProtocol.ModelChoice) -> ChappieChatModelChoice {
        ChappieChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ChappieChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ChappieChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ChappieChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: ChappieChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ChappieChatView>) -> NSViewController
    {
