...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ChappieProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.chappie", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ChappieKit.AnyCodable]()) {
                $0[$1.key] = ChappieKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ChappieProtocol.AnyCodable?) -> [String: ChappieProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ChappieProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ChappieKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ChappieProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("chappie.control.heartbeat")
    static let controlAgentEvent = Notification.Name("chappie.control.agent")
}
