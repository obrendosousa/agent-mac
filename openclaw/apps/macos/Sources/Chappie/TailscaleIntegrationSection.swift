...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.chappie/chappie.json. Prefer CHAPPIE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.chappie/chappie.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.chappie/chappie.json. Restart the gateway to apply."
        }
