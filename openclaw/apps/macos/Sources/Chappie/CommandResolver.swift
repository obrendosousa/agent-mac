...
enum CommandResolver {
    private static let projectRootDefaultsKey = "chappie.gatewayProjectRootPath"
    private static let helperName = "chappie"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let chappieEntry = root.appendingPathComponent("chappie.mjs").path
        if FileManager().isReadableFile(atPath: chappieEntry) { return chappieEntry }
        let binEntry = root.appendingPathComponent("bin/chappie.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/chappie")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let chappiePaths = self.chappieManagedPaths(home: home)
        if !chappiePaths.isEmpty {
            extras.insert(contentsOf: chappiePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + chappiePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func chappieManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".chappie"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func chappieExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectChappieExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("chappie.mjs").path,
            root.appendingPathComponent("bin/chappie.js").path,
        ]
    ...
    static func hasAnyChappieInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.chappieExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func chappieNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let chappiePath = self.projectChappieExecutable(projectRoot: root) {
            return [chappiePath, subcommand] + extraArgs
        }
        if let chappiePath = self.chappieExecutable(searchPaths: searchPaths) {
            return [chappiePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "chappie", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            chappie entrypoint missing (looked for dist/index.js or chappie.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real chappie CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/chappie"
        ...
        \(cliSection)
        \(projectSection)
        if command -v chappie >/dev/null 2>&1; then
          CLI="$(command -v chappie)"
          chappie \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/chappie.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/chappie.mjs"
            node "$PRJ/chappie.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/chappie.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/chappie.js"
            node "$PRJ/bin/chappie.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent chappie"
          pnpm --silent chappie \(quotedArgs);
        else
          echo "chappie CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ChappieConfigFile.loadDict()
