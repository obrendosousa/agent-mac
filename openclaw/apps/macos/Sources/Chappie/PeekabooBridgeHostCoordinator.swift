...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.chappie", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ChappiePeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["chappiedbot", "clawdis", "moltbot"]

    private static var chappieSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "Chappie", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = ChappiePeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.chappieSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.chappieSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.chappieSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.chappieSocketPath)
        } catch {
...
@MainActor
private final class ChappiePeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.chappie.peekaboo")
