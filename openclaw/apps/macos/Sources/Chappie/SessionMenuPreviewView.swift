...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.chappie", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ChappieSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ChappieSessionPreviewEntry,
    ...
    private static func cache(payload: ChappieSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ChappieChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ChappieKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ChappieKit.AnyCodable]) -> [ChappieChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ChappieChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ChappieChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ChappieChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ChappieChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ChappieChatMessage) -> String? {
