...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.chappie.mac`) even if the shipped
    ...
    {
        if environment["CHAPPIE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "chappie.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "chappie.nixMode") { return true }
