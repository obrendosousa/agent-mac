...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CHAPPIE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/chappie")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("chappie-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("chappie-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/chappie/ (rolling or stdout), or nil if none exist.
