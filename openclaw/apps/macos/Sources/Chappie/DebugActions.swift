...
enum DebugActions {
    private static let verboseDefaultsKey = "chappie.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "chappie.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ChappiePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Chappie", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ChappiePaths.configURL
        guard
