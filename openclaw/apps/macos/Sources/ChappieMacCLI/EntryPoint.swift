...
struct ChappieMacCLI {
    static func main() async {
        ...
        default:
            fputs("chappie-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      chappie-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      chappie-mac discover [--timeout <ms>] [--json] [--include-local]
      chappie-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      chappie-mac connect
      chappie-mac connect --url ws://127.0.0.1:18789 --json
      chappie-mac discover --timeout 3000 --json
      chappie-mac wizard --mode local
    """)
}
