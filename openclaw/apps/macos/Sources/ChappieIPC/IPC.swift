...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Chappie/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    return home
        .appendingPathComponent("Library/Application Support/Chappie/control.sock")
        .path
}()
