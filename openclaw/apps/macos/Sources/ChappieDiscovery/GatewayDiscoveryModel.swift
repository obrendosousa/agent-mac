...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.chappie", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in ChappieBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: ChappieBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.chappie.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = ChappieBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = ChappieBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = ChappieBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Chappie)", with: "")
