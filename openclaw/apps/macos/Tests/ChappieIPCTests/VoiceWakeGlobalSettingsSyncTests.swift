...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: ChappieProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: ChappieProtocol.AnyCodable(["triggers": [
            "chappie",
            "computer",
        ]]))
        ...
        #expect(updated == ["chappie", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: ChappieProtocol.AnyCodable(["unexpected": 123]))
