...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let chappiePath = tmp.appendingPathComponent("node_modules/.bin/chappie")
        try makeExecutableForTests(at: chappiePath)

        let cmd = CommandResolver.chappieCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([chappiePath.path, "gateway"]))
    }
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let chappiePath = binDir.appendingPathComponent("chappie")
        ...
        #expect(cmd.prefix(2).elementsEqual([chappiePath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "chappie", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "chappie", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("chappie@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/chappie", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "chappie@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/chappie'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("chappie"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == chappiePath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
