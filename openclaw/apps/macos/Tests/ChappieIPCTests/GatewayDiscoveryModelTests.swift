...
struct GatewayDiscoveryModelTests {
    ...
    @Test func `local gateway matches display name`() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (Chappie)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func `service name does not false positive on substring host token`() {
        let local = GatewayDiscoveryModel.LocalIdentity(
            hostTokens: ["chappie-team"],
            displayTokens: [])
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (Chappie)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "chappie-team (Chappie)",
            local: local))
    }
    ...
    @Test func `parses gateway TXT fields`() {
        ...
        #expect(parsed.cliPath == "/opt/chappie")
    }
    ...
    @Test func `dedupe key prefers resolved endpoint across sources`() {
        let wideArea = GatewayDiscoveryModel.DiscoveredGateway(
            ...
            gatewayPort: 443,
            cliPath: nil,
            stableID: "wide-area|chappie.internal.|gateway-host",
            debugID: "wide-area",
            isLocal: false)
