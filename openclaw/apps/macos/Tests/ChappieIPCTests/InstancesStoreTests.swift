...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: ChappieProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ChappieProtocol.AnyCodable] = [
            "presence": .init([ChappieProtocol.AnyCodable(entry)]),
        ]
        let payload = ChappieProtocol.AnyCodable(payloadMap)
