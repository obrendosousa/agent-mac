...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: ChappieProtocol.AnyCodable(["ok": ChappieProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ChappieProtocol.AnyCodable(["ok": ChappieProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = ChappieProtocol.AnyCodable([
            "runId": ChappieProtocol.AnyCodable("run-1"),
            "sessionKey": ChappieProtocol.AnyCodable("main"),
            "state": ChappieProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ChappieProtocol.AnyCodable(["a": ChappieProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
