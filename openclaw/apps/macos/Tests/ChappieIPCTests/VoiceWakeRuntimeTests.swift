...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["chappie"]
        let text = "hey chappie"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey chappie do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["chappie"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "chappie", start: 0.2, duration: 0.1, range: transcript.range(of: "chappie")),
