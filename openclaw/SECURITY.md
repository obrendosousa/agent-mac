...
If you believe you've found a security issue in Chappie, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [chappie/chappie](https://github.com/chappie/chappie)
- **macOS desktop app** — [chappie/chappie](https://github.com/chappie/chappie) (apps/macos)
- **iOS app** — [chappie/chappie](https://github.com/chappie/chappie) (apps/ios)
- **Android app** — [chappie/chappie](https://github.com/chappie/chappie) (apps/android)
- **ChappieHub** — [chappie/chappiehub](https://github.com/chappie/chappiehub)
- **Trust and threat model** — [chappie/trust](https://github.com/chappie/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@chappie.ai](mailto:security@chappie.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.chappie.ai).
...
- Tested version details (Chappie version and/or commit SHA).
...
- Demonstrated impact tied to Chappie's documented trust boundaries.
- For exposed-secret reports: proof the credential is Chappie-owned (or grants access to Chappie-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Chappie. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Chappie is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Chappie does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Chappie can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Chappie, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Chappie's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Chappie's trusted computing base for a gateway.
...
- Using Chappie in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.chappie`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Chappie-owned and not granting access to Chappie-operated infrastructure/services) without demonstrated Chappie impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Chappie security guidance assumes:

- The host where Chappie runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.chappie` state/config (including `chappie.json`) is effectively a trusted operator.
...
Chappie's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Chappie separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Chappie can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Chappie process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Chappie uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/chappie` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/chappie` (or `chappie-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Chappie-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Chappie temp helpers (`resolvePreferredChappieTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-chappie-dir.ts`
...
For threat model + hardening guidance (including `chappie security audit --deep` and `--fix`), see:

- `https://docs.chappie.ai/gateway/security`
...
Chappie's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `chappie gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Chappie keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `chappie security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__chappie__/canvas/`, `/__chappie__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__chappie__/canvas/`, `/__chappie__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Chappie requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Chappie in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v chappie-data:/app/data \
  chappie/chappie:latest
```
