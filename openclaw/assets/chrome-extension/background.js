  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === 'connected') {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Chappie Browser Relay: relay reconnecting…',
      })
    }
  }
...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    if (!valid) {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Chappie Browser Relay (click to attach/detach)',
      })
      continue
    }
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Chappie Browser Relay: attached (click to detach)',
      })
    } catch {
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'Chappie Browser Relay: connecting to local relay…',
    })
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Chappie Browser Relay: relay not running (open options for setup)',
      })
...
async function onDebuggerDetach(source, reason) {
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'Chappie Browser Relay: re-attaching after navigation…',
  })
  ...
  for (let attempt = 0; attempt < delays.length; attempt++) {
    ...
    try {
      ...
      if (!relayUp) {
        ...
        void chrome.action.setTitle({
          tabId,
          title: 'Chappie Browser Relay: attached, waiting for relay reconnect…',
        })
      }
      return
    } catch {
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'Chappie Browser Relay: re-attach failed (click to retry)',
  })
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  const { url, token } = msg
  const headers = token ? { 'x-chappie-relay-token': token } : {}
