...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/chappie/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/chappie");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/chappie/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CHAPPIE_CONTROL_UI_BASE_PATH__ = "/chappie";
    const app = mountApp("/chappie/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/chappie");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/chappie/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("chappie.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "chappie.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/chappie" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("chappie.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/chappie",
    });
    expect(JSON.parse(localStorage.getItem("chappie.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/chappie";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/chappie");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/chappie#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/chappie");
    expect(app.settings.token).toBe("");
