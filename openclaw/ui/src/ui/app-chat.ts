...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as ChappieApp);
}
...
async function sendChatMessageNow(
  ...
  const runId = await sendChatMessage(host as unknown as ChappieApp, message, opts?.attachments);
...
async function clearChatHistory(host: ChatHost) {
  ...
  try {
    ...
    host.chatRunId = null;
    await loadChatHistory(host as unknown as ChappieApp);
  } catch (err) {
    host.lastError = String(err);
  }
  scheduleChatScroll(host as unknown as Parameters<typeof scheduleChatScroll>[0]);
...
export async function refreshChat(host: ChatHost, opts?: { scheduleScroll?: boolean }) {
  await Promise.all([
    loadChatHistory(host as unknown as ChappieApp),
    loadSessions(host as unknown as ChappieApp, {
      activeMinutes: 0,
      limit: 0,
      includeGlobal: true,
      includeUnknown: true,
