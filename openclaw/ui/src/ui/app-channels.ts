...
export async function handleWhatsAppStart(host: ChappieApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ChappieApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ChappieApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ChappieApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ChappieApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ChappieApp): string {
...
function resolveGatewayHttpAuthHeader(host: ChappieApp): string | null {
...
function buildGatewayHttpHeaders(host: ChappieApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ChappieApp) {
...
export function handleNostrProfileToggleAdvanced(host: ChappieApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ChappieApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ChappieApp) {
  const state = host.nostrProfileFormState;
