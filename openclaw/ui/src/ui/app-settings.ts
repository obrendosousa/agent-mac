...
import type { ChappieApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as ChappieApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as ChappieApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as ChappieApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as ChappieApp);
    await loadConfig(host as unknown as ChappieApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as ChappieApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as ChappieApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as ChappieApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as ChappieApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as ChappieApp);
    await loadDevices(host as unknown as ChappieApp);
    await loadConfig(host as unknown as ChappieApp);
    await loadExecApprovals(host as unknown as ChappieApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as ChappieApp);
    await loadConfig(host as unknown as ChappieApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as ChappieApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as ChappieApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__CHAPPIE_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as ChappieApp;
...
async function loadOverviewLogs(host: ChappieApp) {
...
function buildAttentionItems(host: ChappieApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.chappie.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ChappieApp, true),
    loadConfigSchema(host as unknown as ChappieApp),
    loadConfig(host as unknown as ChappieApp),
  ]);
}
