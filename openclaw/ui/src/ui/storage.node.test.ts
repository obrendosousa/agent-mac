  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __CHAPPIE_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__CHAPPIE_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__CHAPPIE_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /chappie/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/chappie"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/chappie/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/chappie"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("chappie.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "chappie.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/chappie",
    ...
    expect(JSON.parse(localStorage.getItem("chappie.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/chappie",
      sessionKey: "agent",
  ...
  it("does not reuse a session token for a different gatewayUrl", async () => {
    ...
    localStorage.setItem(
      "chappie.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example:8443/chappie",
        sessionKey: "main",
        lastActiveSessionKey: "main",
        theme: "claw",
        themeMode: "system",
